...
def _candidate(candidate_id: str, source: str) -> RepositoryPatchCandidate:
    return RepositoryPatchCandidate(
        ...
        macro_ids=(f'behavior:{candidate_id}',),
        files=(('candidate.py', source),),
        support_score=1,
        edit_count=1,
    )
...
def _initial_tests(oracle: Callable[[int, int], object]) -> tuple[PatchTest, ...]:
    cases = ((6, 9), (8, 12))
    return tuple(
        PatchTest(f'initial-{index}', args, oracle(*args))
        for index, args in enumerate(cases)
    )


def _initial_survivors(
    candidates: tuple[RepositoryPatchCandidate, ...],
    tests: tuple[PatchTest, ...],
) -> tuple[tuple[RepositoryPatchCandidate, Callable[..., object]], ...]:
    survivors = []
...
def _target_independent_one_probe_baseline(
    survivors: tuple[tuple[RepositoryPatchCandidate, Callable[..., object]], ...],
    probes: tuple[RepositoryProbe, ...],
    oracle: Callable[[int, int], object],
) -> dict[str, object]:
    # The probe is selected strictly from content identity; target outputs are not
    # consulted until after selection. This is the matched one-query causal baseline.
    ...
    for candidate, fn in survivors:
        try:
            ...
            continue
    return {
        'exact': len(kept) == 1 and kept[0].candidate_id == 'euclidean-gcd',
...
def run_external_transfer(
    ...
    *,
    ...
    active = solve_repository_patch_with_active_probes(
        ...
        probes,
        oracle,
        verification_inputs=probes,
        max_selection_oracle_calls=1,
    )
    selected_is_correct = bool(
        active.candidate is not None and active.candidate.candidate_id == 'euclidean-gcd'
    )
    ...
    oracle_calls_total = (
        initial_oracle_calls
        + int(random_baseline['selection_oracle_calls'])
        + active.oracle_calls_total
    )

    passed = bool(
        active_exact
        and active.false_terminal_accepts == 0
        and active.selection_oracle_calls == 1
        and selected_partition_count == 4
        and selected_largest_partition == 1
        and verification_exact == len(probes) == 625
        and not random_exact
        and len(survivors) > 1
    )
    return {
        'passed': passed,
        ...
        'claim_boundary': (
            'I/O-only transfer to one independently sourced integer ufunc over a '
            'host-authored finite four-hypothesis behavior set; not general repository coding, '
            'open-ended test invention, unrestricted program synthesis, or AGI.'
        ),
    }
