    ...
    *,
    ...
    while len(survivors) > 1:
        ...
        if probe is None or partitions is None:
            # No remaining probe splits the current version space. That does
            # not imply the version space contains the oracle: an unanimous
            # (or singleton) prediction can still be contradicted. Fall
            # through to target-independent misspecification probing below.
            break
    ...
    if selected_probe is None and survivors:
        # Once entropy-based discrimination is exhausted (including a
        # singleton version space), remaining diagnostics still serve a
        # different purpose: detect that every surviving hypothesis predicts
        # an outcome absent from the oracle. This is bounded by the same oracle
        # budget and receives no target output before candidate generation.
        for probe in diagnostics:
            ...
            fallback_partitions: dict[tuple[str, object], list[object]] = {}
