...
from .arc_current_plus import program_set
from .arc_eval import ParsedTask, TaskScore, load_task, parse_task
from .arc_ops import apply_program


def score_plus(task: ParsedTask, *, max_attempts: int=2, max_programs: int=64) -> TaskScore:
    ...
    return TaskScore(
        ...
        len(task.test_inputs),correct,len(programs),attempts,
    )


__all__=['ParsedTask','TaskScore','load_task','parse_task','score_plus']
