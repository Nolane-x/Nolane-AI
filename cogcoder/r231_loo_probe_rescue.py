...
from .r231_loo_correction_value import rank_loo_correction_candidates

_SELECTION_MODES = {'loo_loss', 'r227_targeted', 'r230_entropy', 'oldest', 'random', 'no_reverify'}


@dataclass(frozen=True)
class LOOProbeRescueDecision:
    status: str
...
def _convert(
    ...
    *,
    ...
    ledger: EvidenceLedger,
    trust_posterior: ReliabilityPosterior,
) -> LOOProbeRescueDecision:
    return LOOProbeRescueDecision(
...
def select_probe_observation(
    ...
    *,
    ...
    if not eligible:
        return None
    if mode == 'loo_loss':
        ranked = rank_loo_correction_candidates(
            proposals,
            ledger,
            trust_posterior,
            regime=regime,
...
def route_with_loo_probe_rescue(
    proposals: Sequence[OperatorProposal],
    ...
    *,
    ...
    # Adaptive leave-one-out historical correction. Only the selected row is superseded;
