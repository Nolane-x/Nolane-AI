...
class LongHorizonStressObservation:
    ...
    def payload(self) -> dict[str, Any]:
        return {
            'observation_id': self.observation_id, 'scenario': self.scenario.value,
            'regime_digest': self.regime_digest, 'initial_state_digest': self.initial_state_digest,
            'final_state_digest': self.final_state_digest, 'checkpoint_anchor': self.checkpoint_anchor,
            'event_anchor': self.event_anchor, 'plan_revision_before': self.plan_revision_before,
            'plan_revision_after': self.plan_revision_after, 'contamination_count': self.contamination_count,
            'stale_context_count': self.stale_context_count, 'false_accepts': self.false_accepts,
            'regressions': self.regressions, 'recovered': self.recovered,
            'elapsed_logical_epochs': self.elapsed_logical_epochs, 'evidence': self.evidence.to_state(),
    ...
    def from_state(cls, state: Mapping[str, Any]) -> 'LongHorizonStressObservation':
        row = cls(
            observation_id=str(state['observation_id']), scenario=StressScenarioKind(str(state['scenario'])),
            regime_digest=str(state['regime_digest']), initial_state_digest=str(state['initial_state_digest']),
            final_state_digest=str(state['final_state_digest']), checkpoint_anchor=str(state['checkpoint_anchor']),
            event_anchor=str(state['event_anchor']), plan_revision_before=str(state['plan_revision_before']),
            plan_revision_after=str(state['plan_revision_after']), contamination_count=int(state['contamination_count']),
            stale_context_count=int(state['stale_context_count']), false_accepts=int(state['false_accepts']),
            regressions=int(state['regressions']), recovered=bool(state['recovered']),
            elapsed_logical_epochs=int(state['elapsed_logical_epochs']), evidence=EvidenceRecord.from_state(state['evidence']),
    ...
    def _validate(self) -> None:
        for value in (
            self.observation_id, self.regime_digest, self.initial_state_digest, self.final_state_digest,
            self.checkpoint_anchor, self.event_anchor, self.plan_revision_before, self.plan_revision_after,
        ):
            if not str(value).strip():
                raise ValueError('stress observation anchors and digests must be explicit')
        if min(self.contamination_count, self.stale_context_count, self.false_accepts, self.regressions, self.elapsed_logical_epochs) < 0:
...
class StressSuiteAssessment:
    ...
    def payload(self) -> dict[str, Any]:
        return {
            'assessment_id': self.assessment_id, 'observation_ids': list(self.observation_ids),
    ...
    def from_state(cls, state: Mapping[str, Any]) -> 'StressSuiteAssessment':
        row = cls(
            assessment_id=str(state['assessment_id']), observation_ids=tuple(str(x) for x in state.get('observation_ids', ())),
            ...
            passed=bool(state['passed']), reasons=tuple(str(x) for x in state.get('reasons', ())), digest=str(state['digest']),
        )
        if canonical_digest(row.payload()) != row.digest:
...
class LongHorizonStressLedger:
    ...
    def record_stress(self, **kwargs: Any) -> LongHorizonStressObservation:
        row0 = LongHorizonStressObservation(
            observation_id=str(kwargs['observation_id']), scenario=StressScenarioKind(kwargs['scenario']),
            regime_digest=str(kwargs['regime_digest']), initial_state_digest=str(kwargs['initial_state_digest']),
            final_state_digest=str(kwargs['final_state_digest']), checkpoint_anchor=str(kwargs['checkpoint_anchor']),
            event_anchor=str(kwargs['event_anchor']), plan_revision_before=str(kwargs['plan_revision_before']),
            plan_revision_after=str(kwargs['plan_revision_after']), contamination_count=int(kwargs['contamination_count']),
            stale_context_count=int(kwargs['stale_context_count']), false_accepts=int(kwargs['false_accepts']),
            regressions=int(kwargs['regressions']), recovered=bool(kwargs['recovered']),
            elapsed_logical_epochs=int(kwargs['elapsed_logical_epochs']), evidence=kwargs['evidence'],
            subject_agent_id=None if kwargs.get('subject_agent_id') is None else str(kwargs['subject_agent_id']), digest='',
        )
        row = LongHorizonStressObservation(
            observation_id=row0.observation_id, scenario=row0.scenario, regime_digest=row0.regime_digest,
            initial_state_digest=row0.initial_state_digest, final_state_digest=row0.final_state_digest,
            checkpoint_anchor=row0.checkpoint_anchor, event_anchor=row0.event_anchor,
            plan_revision_before=row0.plan_revision_before, plan_revision_after=row0.plan_revision_after,
            contamination_count=row0.contamination_count, stale_context_count=row0.stale_context_count,
            false_accepts=row0.false_accepts, regressions=row0.regressions, recovered=row0.recovered,
            elapsed_logical_epochs=row0.elapsed_logical_epochs, evidence=row0.evidence,
            subject_agent_id=row0.subject_agent_id, digest=canonical_digest(row0.payload()),
        )
        self._validate_external(row)
    ...
    def assess_suite(self, observation_ids: tuple[str, ...]) -> StressSuiteAssessment:
        ...
        payload0 = {
            ...
            'covered_scenarios': [x.value for x in covered], 'missing_scenarios': [x.value for x in missing],
        ...
        result = StressSuiteAssessment(
            assessment_id=assessment_id, observation_ids=tuple(payload0['observation_ids']), covered_scenarios=covered,
            missing_scenarios=missing, passed=not reasons, reasons=tuple(reasons), digest=canonical_digest(payload),
        )
