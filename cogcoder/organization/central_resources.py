...
class ResourceAllocationReceipt:
    ...
    def from_state(cls, state: Mapping[str, Any]) -> 'ResourceAllocationReceipt':
        return cls(
            allocation_id=str(state['allocation_id']), beneficiary=str(state['beneficiary']),
            resource=str(state['resource']), amount=int(state['amount']), reason=str(state['reason']),
            evidence_refs=tuple(str(x) for x in state.get('evidence_refs', ())),
            before_available=int(state['before_available']), after_available=int(state['after_available']),
        )
...
class ResourceReleaseReceipt:
    ...
    def to_state(self) -> dict[str, Any]:
        return {
            'release_id': self.release_id, 'allocation_id': self.allocation_id,
            'beneficiary': self.beneficiary, 'resource': self.resource, 'amount': self.amount,
            'reason': self.reason, 'evidence_refs': list(self.evidence_refs),
            'before_leased': self.before_leased, 'after_leased': self.after_leased,
            'before_available': self.before_available, 'after_available': self.after_available,
        }

    @classmethod
    def from_state(cls, state: Mapping[str, Any]) -> 'ResourceReleaseReceipt':
        return cls(
            release_id=str(state['release_id']), allocation_id=str(state['allocation_id']),
            beneficiary=str(state['beneficiary']), resource=str(state['resource']), amount=int(state['amount']),
            reason=str(state['reason']), evidence_refs=tuple(str(x) for x in state.get('evidence_refs', ())),
            before_leased=int(state['before_leased']), after_leased=int(state['after_leased']),
            before_available=int(state['before_available']), after_available=int(state['after_available']),
        )
...
class _LeaseState:
    ...
    def to_state(self) -> dict[str, Any]:
        return {
            'allocation_id': self.allocation_id, 'beneficiary': self.beneficiary,
            'resource': self.resource, 'allocated_amount': self.allocated_amount,
            'remaining_amount': self.remaining_amount,
        }

    @classmethod
    def from_state(cls, state: Mapping[str, Any]) -> '_LeaseState':
        return cls(str(state['allocation_id']), str(state['beneficiary']), str(state['resource']),
                   int(state['allocated_amount']), int(state['remaining_amount']))
...
class CentralResourceArbiter:
    ...
    def leased_to(self, beneficiary: str, resource: str) -> int:
        ...
        return sum(row.remaining_amount for row in self._leases.values()
                   if row.beneficiary == str(beneficiary) and row.resource == str(resource))

    def allocate(self, *, beneficiary: str, resource: str, amount: int, reason: str,
        ...
        receipt = ResourceAllocationReceipt(allocation_id, beneficiary, resource, amount, reason, evidence, before, after)
    ...
    def release(self, allocation_id: str, *, amount: int, reason: str,
        ...
        receipt = ResourceReleaseReceipt(
            f'release-{counter:08d}', lease.allocation_id, lease.beneficiary, lease.resource,
            amount, reason, evidence, before_leased, after_leased, before_available, after_available,
        )
    ...
    def to_state(self) -> dict[str, Any]:
        return {
            'capacity': dict(self._capacity), 'available': dict(self._available),
            ...
            'allocation_counter': self._allocation_counter, 'release_counter': self._release_counter,
        }
    ...
    def from_state(cls, state: Mapping[str, Any]) -> 'CentralResourceArbiter':
        ...
        arbiter._allocation_receipts = [ResourceAllocationReceipt.from_state(x) for x in state.get('allocation_receipts', ())]
        arbiter._release_receipts = [ResourceReleaseReceipt.from_state(x) for x in state.get('release_receipts', ())]
        arbiter._allocation_counter = int(state.get('allocation_counter', len(arbiter._allocation_receipts)))
        arbiter._release_counter = int(state.get('release_counter', len(arbiter._release_receipts)))
        if arbiter._allocation_counter < len(arbiter._allocation_receipts) or arbiter._release_counter < len(arbiter._release_receipts):
