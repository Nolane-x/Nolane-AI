...
class ExperienceRecord:
    ...
    def to_state(self) -> dict[str, Any]:
        return {
            'experience_id': self.experience_id, 'agent_id': self.agent_id,
            'region': self.region, 'domain': self.domain, 'outcome': self.outcome.value,
    ...
    def from_state(cls, state: Mapping[str, Any]) -> 'ExperienceRecord':
        return cls(
            experience_id=str(state['experience_id']), agent_id=str(state['agent_id']),
            region=str(state['region']), domain=str(state['domain']),
            outcome=ExperienceOutcome(str(state['outcome'])), summary=str(state['summary']),
...
class AttributionRecord:
    ...
    def to_state(self) -> dict[str, Any]:
        return {
            'attribution_id': self.attribution_id, 'experience_id': self.experience_id,
            'agent_id': self.agent_id, 'learning_layer': self.learning_layer.value,
            'lesson': self.lesson, 'positive': self.positive,
            'verifier_agent_id': self.verifier_agent_id, 'evidence': self.evidence.to_state(),
        }

    @classmethod
    def from_state(cls, state: Mapping[str, Any]) -> 'AttributionRecord':
        return cls(
            attribution_id=str(state['attribution_id']), experience_id=str(state['experience_id']),
            agent_id=str(state['agent_id']), learning_layer=LearningLayer(str(state['learning_layer'])),
            lesson=str(state['lesson']), positive=bool(state['positive']),
...
class ExperienceLedger:
    ...
    def record(
        ...
        *,
        ...
        digest_payload = {
            'agent_id': identity.agent_id, 'region': identity.region, 'domain': domain,
            'outcome': normalized.value, 'summary': summary, 'task_id': task,
            'object_refs': list(objects), 'evidence_refs': list(evidence),
        }
        experience_id = 'experience-' + canonical_digest(digest_payload)[:24]
        row = ExperienceRecord(
            experience_id=experience_id, agent_id=identity.agent_id, region=identity.region,
            domain=domain, outcome=normalized, summary=summary, task_id=task,
            object_refs=objects, evidence_refs=evidence,
        )
    ...
    def experiences_for(self, agent_id: str) -> tuple[ExperienceRecord, ...]:
        ...
        return tuple(sorted((row for row in self._experiences.values() if row.agent_id == str(agent_id)), key=lambda row: row.experience_id))
    ...
    def attribute(
        ...
        *,
        ...
        payload = {
            'experience_id': experience.experience_id, 'agent_id': experience.agent_id,
            'learning_layer': layer.value, 'lesson': lesson, 'positive': positive,
            'verifier_agent_id': evidence.verifier_agent_id, 'evidence': evidence.to_state(),
        }
        attribution_id = 'attribution-' + canonical_digest(payload)[:24]
        row = AttributionRecord(
            attribution_id=attribution_id, experience_id=experience.experience_id,
            agent_id=experience.agent_id, learning_layer=layer, lesson=lesson,
            positive=positive, verifier_agent_id=evidence.verifier_agent_id, evidence=evidence,
        )
    ...
    def attributions_for(self, agent_id: str) -> tuple[AttributionRecord, ...]:
        self.registry.get(agent_id)
        return tuple(sorted((row for row in self._attributions.values() if row.agent_id == str(agent_id)), key=lambda row: row.attribution_id))
