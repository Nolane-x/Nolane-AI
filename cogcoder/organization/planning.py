...
class PlanNode:
    ...
    def to_state(self) -> dict[str, Any]:
        return {
            ...
            'dependencies': list(self.dependencies), 'requirement_refs': list(self.requirement_refs),
    ...
    def from_state(cls, state: Mapping[str, Any]) -> 'PlanNode':
        return cls(
            str(state['node_id']), str(state['title']),
...
class PlanRevision:
    ...
    def to_state(self) -> dict[str, Any]:
        return {
            ...
            'evidence_refs': list(self.evidence_refs), 'graph_digest': self.graph_digest,
            'changed_node_ids': list(self.changed_node_ids), 'source_revision': self.source_revision,
        }
    ...
    def from_state(cls, state: Mapping[str, Any]) -> 'PlanRevision':
        return cls(
            ...
            str(state['graph_digest']), tuple(str(x) for x in state.get('changed_node_ids', ())),
...
class PlanDelta:
    ...
    def to_state(self) -> dict[str, Any]:
        return {
            ...
            'added_nodes': list(self.added_nodes), 'removed_nodes': list(self.removed_nodes),
            'changed_nodes': list(self.changed_nodes), 'affected_tasks': list(self.affected_tasks),
        }

    @classmethod
    def from_state(cls, state: Mapping[str, Any]) -> 'PlanDelta':
        return cls(
            int(state['from_version']), int(state['to_version']),
            tuple(str(x) for x in state.get('added_nodes', ())), tuple(str(x) for x in state.get('removed_nodes', ())),
            tuple(str(x) for x in state.get('changed_nodes', ())), tuple(str(x) for x in state.get('affected_tasks', ())),
        )
...
class MasterPlanGraph:
    ...
    def apply(self, *, actor_agent_id: str, reason: str, evidence_refs: tuple[str, ...], upsert_nodes: tuple[PlanNode, ...], milestones: tuple[Milestone, ...] = (), risks: tuple[PlanRisk, ...] = ()) -> PlanRevision:
        ...
        revision = PlanRevision(next_version, self.version or None, str(actor_agent_id), reason, evidence, digest, tuple(sorted(set(changed))))
    ...
    def rollback(self, *, actor_agent_id: str, source_revision: int, reason: str, evidence_refs: tuple[str, ...]) -> PlanRevision:
        ...
        next_version = self.version + 1
        digest = canonical_digest({'version': next_version, **payload})
        changed = tuple(sorted(set(self._nodes) ^ set(nodes) | {k for k in set(self._nodes) & set(nodes) if self._nodes[k] != nodes[k]}))
        revision = PlanRevision(next_version, self.version or None, str(actor_agent_id), reason, evidence, digest, changed, source_revision)
    ...
    def ready_nodes(self) -> tuple[str, ...]:
        ...
        return tuple(sorted(x.node_id for x in self._nodes.values() if x.status in {PlanNodeStatus.PLANNED, PlanNodeStatus.READY} and all(dep in done for dep in x.dependencies)))
...
class PlanningControlPlane:
    def __init__(self, *, registry: Any, authority: Any, ledger: Any, tasks: Any, requirements: RequirementsControlPlane, graph: MasterPlanGraph | None = None, task_links: Mapping[str, str] | None = None, deltas: tuple[PlanDelta, ...] = ()) -> None:
    ...
    def apply_revision(self, *, actor_agent_id: str, reason: str, evidence_refs: tuple[str, ...], upsert_nodes: tuple[PlanNode, ...], milestones: tuple[Milestone, ...] = (), risks: tuple[PlanRisk, ...] = ()) -> PlanRevision:
        self.registry.get(actor_agent_id)
        self.authority.require_write(actor_agent_id, 'master-plan')
        old = self.graph.version
        rev = self.graph.apply(actor_agent_id=actor_agent_id, reason=reason, evidence_refs=evidence_refs, upsert_nodes=upsert_nodes, milestones=milestones, risks=risks)
        self._delta(old, rev.version, ())
        self.ledger.append(
            EventKind.PLAN_AMENDED, source_agent_id=actor_agent_id, target_agent_id='planning.chief',
            region='planning-program', evidence_refs=rev.evidence_refs, object_refs=rev.changed_node_ids,
    ...
    def rollback(self, *, actor_agent_id: str, source_revision: int, reason: str, evidence_refs: tuple[str, ...]) -> PlanRevision:
        ...
        self.authority.require_write(actor_agent_id, 'master-plan')
        old = self.graph.version
        rev = self.graph.rollback(actor_agent_id=actor_agent_id, source_revision=source_revision, reason=reason, evidence_refs=evidence_refs)
        self._delta(old, rev.version, ())
        self.ledger.append(
            EventKind.PLAN_ROLLED_BACK, source_agent_id=actor_agent_id, target_agent_id='planning.chief',
            region='planning-program', evidence_refs=rev.evidence_refs,
            payload={'plan_action': 'rollback', 'old_version': old, 'new_version': rev.version, 'source_revision': source_revision, 'affected_tasks': []},
        )
        return rev
    ...
    def apply_gap(self, *, proposal_event_id: str, actor_agent_id: str, added_nodes: tuple[PlanNode, ...], evidence_refs: tuple[str, ...], affected_tasks: tuple[str, ...]) -> GapApplication:
        ...
        event = self.ledger.append(
            EventKind.PLAN_AMENDED, source_agent_id=actor_agent_id, target_agent_id=proposal.source_agent_id,
            region='planning-program', causal_parent_ids=(proposal.event_id,), evidence_refs=tuple(str(x) for x in evidence_refs),
            object_refs=tuple(x.node_id for x in added_nodes),
            payload={'plan_action': 'gap_applied', 'proposal_event_id': proposal.event_id, 'old_version': old, 'new_version': rev.version, 'affected_tasks': list(affected_tasks)},
        )
    ...
    def plan_delta(self, from_version: int) -> PlanDelta:
        ...
        removed = tuple(sorted(set(before) - set(after)))
        changed = tuple(sorted(k for k in set(before) & set(after) if before[k] != after[k]))
        affected = tuple(sorted({task for delta in self._deltas if delta.to_version > from_version for task in delta.affected_tasks}))
    ...
    def to_state(self) -> dict[str, Any]:
        return {'graph': self.graph.to_state(), 'task_links': dict(sorted(self._task_links.items())), 'deltas': [x.to_state() for x in self._deltas]}

    @classmethod
    def from_state(cls, *, registry: Any, authority: Any, ledger: Any, tasks: Any, requirements: RequirementsControlPlane, state: Mapping[str, Any]) -> 'PlanningControlPlane':
        graph = MasterPlanGraph.from_state(requirements, state.get('graph', {}))
        return cls(registry=registry, authority=authority, ledger=ledger, tasks=tasks, requirements=requirements, graph=graph, task_links=state.get('task_links', {}), deltas=tuple(PlanDelta.from_state(v) for v in state.get('deltas', ())))
