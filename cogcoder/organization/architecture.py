...
class ArchitectureComponent:
    ...
    def to_state(self) -> dict[str, Any]:
        return {
            'component_id': self.component_id, 'title': self.title, 'kind': self.kind.value,
            'owner_region': self.owner_region, 'trust_zone': self.trust_zone,
            'requirement_refs': list(self.requirement_refs), 'plan_refs': list(self.plan_refs),
    ...
    def from_state(cls, state: Mapping[str, Any]) -> 'ArchitectureComponent':
        return cls(
            str(state['component_id']), str(state['title']), ComponentKind(str(state['kind'])),
            str(state['owner_region']), str(state['trust_zone']),
...
class InterfaceContract:
    ...
    def __post_init__(self) -> None:
        if not all(str(x).strip() for x in (self.interface_id, self.producer_component_id, self.semantic_version, self.signature_digest)):
            raise ValueError('interface identity/producer/version/signature must be non-empty')

    def to_state(self) -> dict[str, Any]:
        return {
            'interface_id': self.interface_id, 'producer_component_id': self.producer_component_id,
            'interface_class': self.interface_class.value, 'semantic_version': self.semantic_version,
            'signature_digest': self.signature_digest, 'stability': self.stability.value,
            'consumer_scope': list(self.consumer_scope), 'compatibility_policy': self.compatibility_policy,
    ...
    def from_state(cls, state: Mapping[str, Any]) -> 'InterfaceContract':
        return cls(
            str(state['interface_id']), str(state['producer_component_id']), InterfaceClass(str(state['interface_class'])),
            str(state['semantic_version']), str(state['signature_digest']), InterfaceStability(str(state['stability'])),
            tuple(str(x) for x in state.get('consumer_scope', ())),
            str(state.get('compatibility_policy', 'backward')), str(state.get('trust_classification', 'internal')),
        )
...
class ArchitectureEdge:
    ...
    def to_state(self) -> dict[str, str]:
        return {
            'edge_id': self.edge_id, 'source_component_id': self.source_component_id,
            'target_component_id': self.target_component_id, 'kind': self.kind.value,
        }

    @classmethod
    def from_state(cls, state: Mapping[str, Any]) -> 'ArchitectureEdge':
        return cls(str(state['edge_id']), str(state['source_component_id']), str(state['target_component_id']), EdgeKind(str(state['kind'])))
...
class ArchitectureRevision:
    ...
    def to_state(self) -> dict[str, Any]:
        return {
            'version': self.version, 'parent_version': self.parent_version, 'actor_agent_id': self.actor_agent_id,
            'reason': self.reason, 'evidence_refs': list(self.evidence_refs),
            'changed_refs': list(self.changed_refs), 'graph_digest': self.graph_digest,
        }

    @classmethod
    def from_state(cls, state: Mapping[str, Any]) -> 'ArchitectureRevision':
        return cls(
            int(state['version']), None if state.get('parent_version') is None else int(state['parent_version']),
            str(state['actor_agent_id']), str(state['reason']), tuple(str(x) for x in state.get('evidence_refs', ())),
            tuple(str(x) for x in state.get('changed_refs', ())), str(state['graph_digest']),
        )
...
class ArchitectureGraph:
    ...
    @staticmethod
    def _validate(components: Mapping[str, ArchitectureComponent], interfaces: Mapping[str, InterfaceContract], edges: Mapping[str, ArchitectureEdge]) -> None:
    ...
    def apply(self, *, actor_agent_id: str, reason: str, evidence_refs: tuple[str, ...], upsert_components: tuple[ArchitectureComponent, ...] = (), upsert_interfaces: tuple[InterfaceContract, ...] = (), upsert_edges: tuple[ArchitectureEdge, ...] = ()) -> ArchitectureRevision:
        ...
        for row in upsert_components:
            components[row.component_id] = row; changed.append(row.component_id)
        for row in upsert_interfaces:
            interfaces[row.interface_id] = row; changed.append(row.interface_id)
        for row in upsert_edges:
            edges[row.edge_id] = row; changed.append(row.edge_id)
        ...
        revision = ArchitectureRevision(next_version, self.version or None, str(actor_agent_id), reason, evidence, tuple(sorted(set(changed))), digest)
    ...
    def from_state(cls, state: Mapping[str, Any]) -> 'ArchitectureGraph':
        graph = cls()
        graph._components = {x.component_id: x for x in (ArchitectureComponent.from_state(v) for v in state.get('components', ())) }
        graph._interfaces = {x.interface_id: x for x in (InterfaceContract.from_state(v) for v in state.get('interfaces', ())) }
        graph._edges = {x.edge_id: x for x in (ArchitectureEdge.from_state(v) for v in state.get('edges', ())) }
...
class ArchitectureControlPlane:
    ...
    def apply_revision(self, *, actor_agent_id: str, reason: str, evidence_refs: tuple[str, ...], upsert_components: tuple[ArchitectureComponent, ...] = (), upsert_interfaces: tuple[InterfaceContract, ...] = (), upsert_edges: tuple[ArchitectureEdge, ...] = ()) -> ArchitectureRevision:
        self.registry.get(actor_agent_id)
        self.authority.require_write(actor_agent_id, 'architecture-graph')
        revision = self.graph.apply(actor_agent_id=actor_agent_id, reason=reason, evidence_refs=evidence_refs, upsert_components=upsert_components, upsert_interfaces=upsert_interfaces, upsert_edges=upsert_edges)
        self.ledger.append(
            EventKind.ARCHITECTURE_CONCERN, source_agent_id=actor_agent_id, target_agent_id='architecture.chief',
            region='architecture-system', object_refs=revision.changed_refs, evidence_refs=revision.evidence_refs,
            payload={'architecture_action': 'changed', 'version': revision.version, 'reason': revision.reason},
        )
        return revision

    def propose_concern(self, *, source_agent_id: str, component_refs: tuple[str, ...], observation: str, alternatives: tuple[str, ...], evidence_refs: tuple[str, ...], severity: int):
        ...
        return self.ledger.append(
            EventKind.ARCHITECTURE_CONCERN, source_agent_id=source_agent_id, target_agent_id='architecture.chief',
            region='architecture-system', object_refs=component_refs, evidence_refs=tuple(str(x) for x in evidence_refs),
            payload={'architecture_action': 'concern', 'observation': observation, 'alternatives': list(alternatives), 'severity': int(severity)},
        )
    ...
    @classmethod
    def from_state(cls, *, registry: Any, authority: Any, ledger: Any, state: Mapping[str, Any]) -> 'ArchitectureControlPlane':
        return cls(registry=registry, authority=authority, ledger=ledger, graph=ArchitectureGraph.from_state(state.get('graph', {})))
