...
from .types import AgentRank, EventKind, canonical_digest
...
class StaleAgentReceipt:
    ...
    def from_state(cls, state: Mapping[str, Any]) -> 'StaleAgentReceipt':
        row = cls(
            stale_id=str(state['stale_id']), lease_id=str(state['lease_id']),
            task_id=str(state['task_id']), agent_id=str(state['agent_id']),
            ...
            status=str(state.get('status', 'detected')), event_id=str(state['event_id']),
            digest=str(state['digest']),
        )
        if canonical_digest(row.payload()) != row.digest:
...
class LeaseCoordinator:
    def __init__(
        self, *, registry: AgentRegistry, tasks: TaskGraph, events: EventLedger,
        receipts: tuple[TaskLeaseReceipt, ...] = (),
        stale_receipts: tuple[StaleAgentReceipt, ...] = (),
        lease_counter: int = 0, stale_counter: int = 0,
    ) -> None:
    ...
    def grant(
        ...
        row = self._digest_row(
            lease_id=f'lease-{self._lease_counter:08d}', task_id=task.task_id,
            agent_id=identity.agent_id, epoch=epoch, status=LeaseStatus.ACTIVE,
            granted_event_id=event.event_id, supersedes_lease_id=prior_id,
            evidence_refs=tuple(str(x) for x in evidence_refs),
            last_heartbeat_token=int(token), stale_after_tokens=int(stale_after_tokens),
            renewal_count=0,
        )
        self._receipts[row.lease_id] = row
        self._active_by_task[row.task_id] = row.lease_id
    ...
    def heartbeat(
        ...
        self.events.append(
            ...
            source_agent_id=current.agent_id, target_agent_id=current.agent_id,
        ...
        row = self._digest_row(
            lease_id=current.lease_id, task_id=current.task_id, agent_id=current.agent_id,
            ...
            last_heartbeat_token=int(token), stale_after_tokens=current.stale_after_tokens,
    ...
    def revoke(
        ...
        self.events.append(
            ...
            source_agent_id=actor.agent_id, target_agent_id=current.agent_id,
            region=holder.region, object_refs=(current.task_id,),
            evidence_refs=tuple(str(x) for x in evidence_refs),
            payload={'lease_id': current.lease_id, 'epoch': current.epoch, 'reason': reason},
        )
        row = self._digest_row(
            lease_id=current.lease_id, task_id=current.task_id, agent_id=current.agent_id,
            epoch=current.epoch, status=LeaseStatus.REVOKED,
            granted_event_id=current.granted_event_id,
            supersedes_lease_id=current.supersedes_lease_id,
            evidence_refs=current.evidence_refs,
    ...
    def complete(
        ...
        row = self._digest_row(
            lease_id=current.lease_id, task_id=current.task_id, agent_id=current.agent_id,
            epoch=current.epoch, status=LeaseStatus.COMPLETED,
            granted_event_id=current.granted_event_id,
            supersedes_lease_id=current.supersedes_lease_id,
            evidence_refs=current.evidence_refs,
            last_heartbeat_token=current.last_heartbeat_token,
    ...
    def detect_stale(self, current_token: int) -> tuple[StaleAgentReceipt, ...]:
        ...
        for task_id in sorted(self._active_by_task):
            ...
            event = self.events.append(
                ...
                region=identity.region, object_refs=(lease.task_id,),
                payload={
                    'lease_id': lease.lease_id, 'agent_id': lease.agent_id,
            ...
            temp = StaleAgentReceipt(
                stale_id=f'stale-{self._stale_counter:08d}', lease_id=lease.lease_id,
                task_id=lease.task_id, agent_id=lease.agent_id,
                detection_token=int(current_token), last_heartbeat_token=lease.last_heartbeat_token,
                region_chief_id=identity.region_chief_id,
                escalation_recipients=recipients, status='detected',
    ...
    @classmethod
    def from_state(
        cls, *, registry: AgentRegistry, tasks: TaskGraph, events: EventLedger,
        ...
        if not state:
            ...
            for task in tasks.tasks():
                ...
                temp = TaskLeaseReceipt(
                    lease_id=f'lease-{counter:08d}', task_id=task.task_id,
                    agent_id=task.leased_to, epoch=1, status=LeaseStatus.ACTIVE,
                    granted_event_id=None, supersedes_lease_id=None, evidence_refs=(),
                    last_heartbeat_token=0, stale_after_tokens=3, renewal_count=0, digest='',
                )
                rows.append(replace(temp, digest=canonical_digest(temp.payload())))
            return cls(
                registry=registry, tasks=tasks, events=events,
                receipts=tuple(rows), lease_counter=counter,
            )
        ...
        return cls(
            registry=registry, tasks=tasks, events=events,
            receipts=receipts, stale_receipts=stale,
