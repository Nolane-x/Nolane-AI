...
from .coordination_conflicts import ConflictCoordinator, ConflictPacket, ConflictClaim, ConflictResolutionReceipt
...
from .types import AgentRank, AgentStatus, EventKind, canonical_digest
...
class CoordinationBudget:
    ...
    def __post_init__(self) -> None:
        values = (
            self.max_active_agents, self.max_region_active_agents,
            self.high_severity_max_active_agents, self.max_pending_acks,
            self.max_unresolved_conflicts, self.max_coordination_events_per_window,
        )
...
class WakeReservation:
    ...
    def payload(self) -> dict[str, Any]:
        return {
            'reservation_id': self.reservation_id, 'event_id': self.event_id,
            'agent_id': self.agent_id, 'region': self.region,
            'disposition': self.disposition.value, 'priority': self.priority,
            'reason': self.reason, 'coordination_event_id': self.coordination_event_id,
        }
    ...
    def from_state(cls, state: Mapping[str, Any]) -> 'WakeReservation':
        row = cls(
            reservation_id=str(state['reservation_id']), event_id=str(state['event_id']),
            agent_id=str(state['agent_id']), region=str(state['region']),
            disposition=WakeDisposition(str(state['disposition'])),
            priority=int(state['priority']), reason=str(state['reason']),
...
class CoordinationEscalation:
    ...
    def payload(self) -> dict[str, Any]:
        return {
            'escalation_id': self.escalation_id, 'reason': self.reason,
            'target_agent_id': self.target_agent_id,
            'source_event_id': self.source_event_id, 'event_id': self.event_id,
        }
    ...
    def from_state(cls, state: Mapping[str, Any]) -> 'CoordinationEscalation':
        row = cls(
            escalation_id=str(state['escalation_id']), reason=str(state['reason']),
            ...
            event_id=str(state['event_id']), digest=str(state['digest']),
        )
        if canonical_digest(row.payload()) != row.digest:
...
_COORDINATION_EVENT_KINDS = {
    EventKind.TASK_LEASE_GRANTED, EventKind.TASK_LEASE_RENEWED, EventKind.TASK_LEASE_REVOKED,
    EventKind.COORDINATION_ACK, EventKind.COORDINATION_ESCALATED,
    EventKind.CONFLICT_OPENED, EventKind.CONFLICT_CLAIM_ADDED, EventKind.CONFLICT_RESOLVED,
    EventKind.WAKE_RESERVED, EventKind.WAKE_DEFERRED, EventKind.STALE_AGENT_DETECTED,
...
class CoordinationControlPlane:
    def __init__(
        self, *, registry: AgentRegistry, events: EventLedger, authority: AuthorityGraph,
        tasks: TaskGraph, scheduler: WakeSleepScheduler,
        ...
        reservation_counter: int = 0, escalation_counter: int = 0,
        ...
        self.conflicts_state = conflicts or ConflictCoordinator(
            registry=registry, authority=authority, events=events,
        )
    ...
    def _rank_order(rank: AgentRank) -> int:
        return {
            AgentRank.CENTRAL: 0, AgentRank.CHIEF: 1,
            AgentRank.SENIOR_SPECIALIST: 2, AgentRank.SPECIALIST: 3,
        }[rank]
    ...
    def grant_lease(
        self, task_id: str, agent_id: str, *, token: int = 0,
        stale_after_tokens: int = 3, evidence_refs: tuple[str, ...] = (),
    ) -> TaskLeaseReceipt:
        row = self.leases.grant(
            task_id, agent_id, token=token, stale_after_tokens=stale_after_tokens,
    ...
    def open_conflict(
        self, opener_agent_id: str, subject_artifact_id: str, *, proposition: str,
        requested_action: str, evidence_refs: tuple[str, ...] = (),
        causal_event_ids: tuple[str, ...] = (),
    ) -> ConflictPacket:
        packet = self.conflicts_state.open(
            opener_agent_id, subject_artifact_id, proposition=proposition,
            requested_action=requested_action, evidence_refs=evidence_refs,
    ...
    def add_claim(
        self, conflict_id: str, claimant_agent_id: str, *, proposition: str,
        requested_action: str, evidence_refs: tuple[str, ...] = (),
    ) -> ConflictClaim:
        return self.conflicts_state.add_claim(
            conflict_id, claimant_agent_id, proposition=proposition,
            requested_action=requested_action, evidence_refs=evidence_refs,
        )

    def resolve_conflict(
        self, conflict_id: str, resolver_agent_id: str, *, decision: str,
        evidence_refs: tuple[str, ...], override_id: str | None = None,
    ) -> ConflictResolutionReceipt:
        return self.conflicts_state.resolve(
            conflict_id, resolver_agent_id, decision=decision,
            evidence_refs=evidence_refs, override_id=override_id,
        )
    ...
    def propose_cross_region_change(
        self, *, proposer_agent_id: str, subject_artifact_id: str,
        proposition: str, requested_action: str, evidence_refs: tuple[str, ...],
    ) -> ConflictPacket:
        return self.open_conflict(
            proposer_agent_id, subject_artifact_id, proposition=proposition,
            requested_action=requested_action, evidence_refs=evidence_refs,
        )
    ...
    def plan_wakes(self, event_id: str, *, mode: str = 'normal') -> tuple[WakeReservation, ...]:
        ...
        global_limit = self.budget.high_severity_max_active_agents if mode == 'high_severity' else self.budget.max_active_agents
        ...
        for aid in self._recipients(event.event_id):
            ...
            coord_event = self.events.append(
                kind, source_agent_id='nolane.central', target_agent_id=identity.agent_id,
                region=identity.region, causal_parent_ids=(event.event_id,), priority=event.priority,
                payload={'source_event_id': event.event_id, 'mode': mode,
                         'disposition': disposition.value, 'reason': reason},
            )
            self._reservation_counter += 1
            temp = WakeReservation(
                reservation_id=f'wake-reservation-{self._reservation_counter:08d}',
                event_id=event.event_id, agent_id=identity.agent_id, region=identity.region,
                disposition=disposition, priority=event.priority, reason=reason,
                coordination_event_id=coord_event.event_id, digest='',
            )
    ...
    def escalate_stale(self, *, current_token: int) -> tuple[StaleAgentReceipt, ...]:
        ...
        for row in rows:
            if not any(x.source_event_id == row.event_id for x in self._escalations.values()):
                ...
                self._record_escalation(reason='stale_agent', target_agent_id=target, source_event_id=row.event_id)
        return rows
    ...
    def _record_escalation(
        self, *, reason: str, target_agent_id: str | None, source_event_id: str | None,
        ...
        event = self.events.append(
            ...
            source_agent_id='nolane.central', target_agent_id=target_agent_id,
            region=None if target_agent_id is None else self.registry.get(target_agent_id).region,
            causal_parent_ids=causal,
            payload={'reason': str(reason), 'source_event_id': source_event_id}, priority=100,
        )
        self._escalation_counter += 1
        temp = CoordinationEscalation(
            escalation_id=f'coord-escalation-{self._escalation_counter:08d}',
            reason=str(reason), target_agent_id=target_agent_id,
            source_event_id=source_event_id, event_id=event.event_id, digest='',
        )
        row = replace(temp, digest=canonical_digest(temp.payload()))
        self._escalations[row.escalation_id] = row
        return row
    ...
    def _check_ack_budget(self, source_event_id: str) -> None:
        ...
        if pending > self.budget.max_pending_acks and not any(x.reason == 'pending_ack_budget' for x in self._escalations.values()):
            self._record_escalation(reason='pending_ack_budget', target_agent_id='nolane.central', source_event_id=source_event_id)
    ...
    def _check_conflict_budget(self, source_event_id: str) -> None:
        ...
        if unresolved > self.budget.max_unresolved_conflicts and not any(x.reason == 'conflict_budget' for x in self._escalations.values()):
            self._record_escalation(reason='conflict_budget', target_agent_id='nolane.central', source_event_id=source_event_id)
    ...
    def _maybe_coordination_budget(self, source_event_id: str | None) -> None:
        ...
        if generated >= self.budget.max_coordination_events_per_window and not any(x.reason == 'coordination_event_budget' for x in self._escalations.values()):
            self._record_escalation(reason='coordination_event_budget', target_agent_id='nolane.central', source_event_id=source_event_id)
    ...
    def metrics(self) -> CoordinationMetrics:
        ...
        return CoordinationMetrics(
            ...
            lease_transition_count=sum(
                row.kind in {EventKind.TASK_LEASE_GRANTED, EventKind.TASK_LEASE_RENEWED, EventKind.TASK_LEASE_REVOKED}
                for row in events
            ),
    ...
    @classmethod
    def from_state(
        cls, *, registry: AgentRegistry, events: EventLedger, authority: AuthorityGraph,
        tasks: TaskGraph, scheduler: WakeSleepScheduler, state: Mapping[str, Any],
    ) -> 'CoordinationControlPlane':
        leases = LeaseCoordinator.from_state(
            registry=registry, tasks=tasks, events=events, state=state.get('leases', {}),
        )
        deliveries = DeliveryCoordinator.from_state(
            registry=registry, events=events, state=state.get('deliveries', {}),
        )
        ...
        return cls(
            registry=registry, events=events, authority=authority, tasks=tasks,
            scheduler=scheduler, leases=leases, deliveries=deliveries, conflicts=conflicts,
            budget=CoordinationBudget.from_state(state.get('budget', {})),
            reservations=reservations, escalations=escalations,
