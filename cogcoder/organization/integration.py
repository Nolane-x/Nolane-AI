...
from .compatibility import CompatibilityAssessment, CompatibilityClass
...
class ChangeCandidate:
    ...
    def to_state(self) -> dict[str, Any]:
        return {
            'candidate_id': self.candidate_id, 'producer_agent_id': self.producer_agent_id,
            'task_refs': list(self.task_refs), 'plan_refs': list(self.plan_refs),
    ...
    def from_state(cls, state: Mapping[str, Any]) -> 'ChangeCandidate':
        return cls(
            str(state['candidate_id']), str(state['producer_agent_id']),
            tuple(str(x) for x in state.get('task_refs', ())), tuple(str(x) for x in state.get('plan_refs', ())),
            tuple(str(x) for x in state.get('requirement_refs', ())), int(state['architecture_version_expected']),
...
class IntegrationReceipt:
    ...
    def to_state(self) -> dict[str, Any]:
        return {
            'receipt_id': self.receipt_id, 'candidate_id': self.candidate_id,
            'actor_agent_id': self.actor_agent_id, 'status': self.status.value,
            'evidence_refs': list(self.evidence_refs), 'architecture_version': self.architecture_version,
            'digest': self.digest,
        }
    ...
    def from_state(cls, state: Mapping[str, Any]) -> 'IntegrationReceipt':
        return cls(
            str(state['receipt_id']), str(state['candidate_id']), str(state['actor_agent_id']),
            ChangeCandidateStatus(str(state['status'])), tuple(str(x) for x in state.get('evidence_refs', ())),
            int(state['architecture_version']), str(state['digest']),
        )
...
class IntegrationGraph:
    ...
    def update(self, candidate: ChangeCandidate) -> ChangeCandidate:
        ...
        rows = dict(self._candidates); rows[candidate.candidate_id] = candidate
        self._validate(rows)
        self._candidates = rows
        self._version += 1
        return candidate
    ...
    def integration_order(self) -> tuple[str, ...]:
        ...
        for key, row in self._candidates.items():
            for dep in row.dependency_candidate_ids:
                indegree[key] += 1; forward[dep].append(key)
    ...
    def from_state(cls, state: Mapping[str, Any]) -> 'IntegrationGraph':
        graph = cls()
        graph._candidates = {x.candidate_id: x for x in (ChangeCandidate.from_state(v) for v in state.get('candidates', ())) }
...
class IntegrationControlPlane:
    def __init__(self, *, registry: Any, authority: Any, architecture: Any, graph: IntegrationGraph | None = None, receipts: tuple[IntegrationReceipt, ...] = ()) -> None:
    ...
    def add_candidate(self, *, actor_agent_id: str, candidate: ChangeCandidate, replace: bool = False) -> ChangeCandidate:
    ...
    def integrate(self, candidate_id: str, *, actor_agent_id: str, evidence_refs: tuple[str, ...]) -> IntegrationReceipt:
        self.registry.get(actor_agent_id)
        self.authority.require_write(actor_agent_id, 'integration-state')
        ...
        if not candidate.compatibility_assessments or any((not row.integration_safe) or row.compatibility in {CompatibilityClass.UNKNOWN, CompatibilityClass.BREAKING} for row in candidate.compatibility_assessments):
        ...
        integrated_ids = {row.candidate_id for row in self.graph.candidates() if row.status is ChangeCandidateStatus.INTEGRATED}
        ...
        payload = {
            'receipt_index': self._receipt_counter, 'candidate_id': candidate_id,
            'actor_agent_id': actor_agent_id, 'status': ChangeCandidateStatus.INTEGRATED.value,
            'evidence_refs': list(evidence), 'architecture_version': self.architecture.graph.version,
        }
        digest = canonical_digest(payload)
        receipt = IntegrationReceipt(
            f'integration-{self._receipt_counter:08d}', candidate_id, actor_agent_id,
            ChangeCandidateStatus.INTEGRATED, evidence, self.architecture.graph.version, digest,
        )
    ...
    def to_state(self) -> dict[str, Any]:
        return {'graph': self.graph.to_state(), 'receipt_counter': self._receipt_counter, 'receipts': [x.to_state() for x in self.receipts()]}

    @classmethod
    def from_state(cls, *, registry: Any, authority: Any, architecture: Any, state: Mapping[str, Any]) -> 'IntegrationControlPlane':
        ...
        plane = cls(registry=registry, authority=authority, architecture=architecture, graph=graph, receipts=receipts)
