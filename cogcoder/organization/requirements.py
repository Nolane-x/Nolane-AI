...
# Keep the Part-I event serialization schema stable while exposing precise
# Part-III names. Payloads carry `requirements_action` so aliases remain auditable.
for _name in (
    'REQUIREMENT_AMBIGUITY', 'REQUIREMENT_CHANGE_PROPOSED',
    'REQUIREMENT_CHANGED', 'ACCEPTANCE_GAP',
):
...
class AcceptanceCriterion:
    ...
    def from_state(cls, state: Mapping[str, Any]) -> 'AcceptanceCriterion':
        return cls(
            str(state['criterion_id']), str(state['statement']),
...
class RequirementRevision:
    ...
    def from_state(cls, state: Mapping[str, Any]) -> 'RequirementRevision':
        return cls(
            ...
            str(state['actor_agent_id']), str(state['reason']),
...
class RequirementGraph:
    ...
    def apply(self, *, actor_agent_id: str, reason: str, evidence_refs: tuple[str, ...], upserts: Iterable[RequirementNode]) -> RequirementRevision:
        ...
        graph_digest = canonical_digest({'version': next_version, 'nodes': [candidate[k].to_state() for k in sorted(candidate)]})
        revision = RequirementRevision(
            next_version, self.version or None, str(actor_agent_id), reason, evidence,
            tuple(sorted(set(changed))), graph_digest,
        )
    ...
    def to_state(self) -> dict[str, Any]:
        return {'nodes': [x.to_state() for x in self.nodes()], 'revisions': [x.to_state() for x in self._revisions]}

    @classmethod
    def from_state(cls, state: Mapping[str, Any]) -> 'RequirementGraph':
        graph = cls()
        graph._nodes = {x.requirement_id: x for x in (RequirementNode.from_state(v) for v in state.get('nodes', ()))}
...
class RequirementsControlPlane:
    def __init__(self, *, registry: Any, authority: Any, ledger: Any, graph: RequirementGraph | None = None) -> None:
        self.registry = registry
        self.authority = authority
        self.ledger = ledger
        self.graph = graph or RequirementGraph()

    def apply_revision(self, *, actor_agent_id: str, reason: str, evidence_refs: tuple[str, ...], upserts: tuple[RequirementNode, ...]) -> RequirementRevision:
        self.registry.get(actor_agent_id)
        self.authority.require_write(actor_agent_id, 'requirements')
        revision = self.graph.apply(actor_agent_id=actor_agent_id, reason=reason, evidence_refs=evidence_refs, upserts=upserts)
        self.ledger.append(
            EventKind.REQUIREMENT_CHANGED, source_agent_id=actor_agent_id, target_agent_id='requirements.chief',
            region='requirements-product', evidence_refs=revision.evidence_refs,
            object_refs=revision.changed_requirement_ids,
            payload={'requirements_action': 'changed', 'version': revision.version, 'reason': revision.reason},
        )
        return revision

    def _proposal(self, kind: EventKind, *, source_agent_id: str, requirement_id: str, text: str, evidence_refs: tuple[str, ...], action: str):
        ...
        return self.ledger.append(
            kind, source_agent_id=source_agent_id, target_agent_id='requirements.chief',
            region='requirements-product', evidence_refs=tuple(str(x) for x in evidence_refs),
            object_refs=(requirement_id,), payload={'requirements_action': action, 'requirement_id': requirement_id, 'text': text},
        )

    def propose_ambiguity(self, *, source_agent_id: str, requirement_id: str, question: str, evidence_refs: tuple[str, ...]):
        return self._proposal(EventKind.REQUIREMENT_AMBIGUITY, source_agent_id=source_agent_id, requirement_id=requirement_id, text=question, evidence_refs=evidence_refs, action='ambiguity')

    def propose_change(self, *, source_agent_id: str, requirement_id: str, proposal: str, evidence_refs: tuple[str, ...]):
        return self._proposal(EventKind.REQUIREMENT_CHANGE_PROPOSED, source_agent_id=source_agent_id, requirement_id=requirement_id, text=proposal, evidence_refs=evidence_refs, action='change_proposed')

    def propose_acceptance_gap(self, *, source_agent_id: str, requirement_id: str, gap: str, evidence_refs: tuple[str, ...]):
        return self._proposal(EventKind.ACCEPTANCE_GAP, source_agent_id=source_agent_id, requirement_id=requirement_id, text=gap, evidence_refs=evidence_refs, action='acceptance_gap')

    def to_state(self) -> dict[str, Any]:
        return {'graph': self.graph.to_state()}

    @classmethod
    def from_state(cls, *, registry: Any, authority: Any, ledger: Any, state: Mapping[str, Any]) -> 'RequirementsControlPlane':
        return cls(registry=registry, authority=authority, ledger=ledger, graph=RequirementGraph.from_state(state.get('graph', {})))
