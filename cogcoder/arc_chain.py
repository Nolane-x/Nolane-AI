...
def _prefixes(pairs, max_prefixes):
    ...
    for program in seeds:
        ...
        shape_gap = sum(abs(r.h - y.h) + abs(r.w - y.w) for r, (_, y) in zip(rendered, pairs))
        key = (shape_gap, program.cost, len(program.steps), repr(program.signature))
        current = best.get(signature)
        if current is None or key < current[0]:
            best[signature] = (key, program, rendered)
    rows = sorted(best.values(), key=lambda item: item[0])
    return tuple((program, rendered) for _, program, rendered in rows[:max_prefixes])
...
def _suffixes(grid, target):
    ...
    if target.h % grid.h == 0 and target.w % grid.w == 0:
        ...
        if (rf, cf) != (1, 1):
            rows.append((Step('scale', (rf, cf)), 2))
            rows.append((Step('tile', (rf, cf)), 3))
...
def chain_programs(pairs, *, max_prefixes=24, limit=32):
    pairs = tuple(pairs)
    if not pairs or max_prefixes < 1 or limit < 1:
        return ()
    accepted = {}
    for prefix, rendered in _prefixes(pairs, max_prefixes):
        first_rendered = rendered[0]
        for suffix, suffix_cost in _suffixes(first_rendered, pairs[0][1]):
            program = Program(prefix.steps + (suffix,), prefix.cost + suffix_cost)
            outputs = tuple(_apply(program, x) for x, _ in pairs)
            if any(out is None or out.shape != y.shape for out, (_, y) in zip(outputs, pairs)):
                continue
            if all(out == y for out, (_, y) in zip(outputs, pairs)):
                accepted[program.signature] = program
                continue
            table = _merge_color_maps(
                [infer_color_map(out, y) for out, (_, y) in zip(outputs, pairs)]
            )
            if table is None or not any(a != b for a, b in table):
                continue
            mapped = Program(
                program.steps + (Step('color_map', (table,)),),
                program.cost + 1 + len(table) // 4,
            )
    ...
    return tuple(
        sorted(
            accepted.values(),
            key=lambda p: (p.cost, len(p.steps), repr(p.signature)),
        )[:limit]
    )
