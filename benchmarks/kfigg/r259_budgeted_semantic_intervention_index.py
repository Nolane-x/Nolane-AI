...
_ROLES = ('x', 'a', 'b', 'fa', 'fb')
_R258_FROZEN_SYNTHESIS_CANDIDATES = 261_169
...
class _OpaqueConfiguration:
    ...
    def __post_init__(self) -> None:
        ...
        if set(self.role_order) != set(_ROLES) or len(self.role_order) != len(_ROLES):
            raise ValueError('role_order must be a permutation of latent roles')
...
def _run_configuration(config: _OpaqueConfiguration):
    ...
    return discover_budgeted_intervention(
        oracle=_oracle(config),
        ordered_field_names=config.field_names,
        probe_training_contexts=tuple(_context(config, case) for case in _PROBE_TRAIN_CASES),
        probe_validation_contexts=tuple(_context(config, case) for case in _PROBE_VALID_CASES),
        vocabulary=vocabulary,
        downstream_need=need,
        downstream_examples=_downstream_examples(config),
        probe_max_depth=2,
...
def _replay_under_rename(source_config: _OpaqueConfiguration, target_config: _OpaqueConfiguration, source_receipt):
    ...
    target_probe = target_schema.externalize_expr(source_schema.canonicalize_expr(selected.probe_expression))
    target_full = target_schema.externalize_expr(source_schema.canonicalize_expr(selected.seeded_downstream_expression))
    vocabulary, _lifecycle, _selected = build_promoted_vocabulary()
...
def run_benchmark_part(part: str) -> dict[str, object]:
    ...
    if part == 'rename':
        a = _OpaqueConfiguration('rename-a', ('q7', 'q2', 'q9', 'q4', 'q1'), _ROLES)
        b = _OpaqueConfiguration('rename-b', ('zeta', 'theta', 'mu', 'rho', 'eta'), _ROLES)
        receipt = _run_configuration(a)
        replay = _replay_under_rename(a, b, receipt)
        selected = receipt.selected
        wrong, noncausal = _candidate_audit(a, receipt)
        selected_id = selected.intervention.intervention_id if selected is not None else ''
        selected_positions = sorted(position for position, _value in selected.intervention.bindings) if selected else []
        replay_positions = sorted(replay.get('position_set', frozenset()))
        return {
            'part': 'rename',
            'discovered': int(receipt.passed) + int(bool(replay.get('passed'))),
            'no_seed_failures': 2 * int(not receipt.no_seed_passed),
            'seeded_successes': int(selected is not None and selected.seeded_downstream_passed) + int(bool(replay.get('passed'))),
            'probe_validation_exact': (selected.probe_validation_exact if selected else 0) + int(replay.get('probe_exact', 0)),
            'probe_validation_cases': (selected.probe_validation_cases if selected else len(_PROBE_VALID_CASES)) + int(replay.get('probe_cases', len(_PROBE_VALID_CASES))),
            'position_rename_invariant': bool(selected_id and selected_id == replay.get('intervention_id') and selected_positions == replay_positions),
            'wrong_role_false_accepts': wrong,
            'noncausal_candidates_rejected': noncausal,
            'selected_intervention_ids': [selected_id, str(replay.get('intervention_id', ''))],
            'selected_position_sets': [selected_positions, replay_positions],
            'oracle_calls': receipt.oracle_calls + int(replay.get('probe_cases', 0)) + int(replay.get('downstream_cases', 0)),
            'synthesis_candidates_considered': receipt.total_synthesis_candidates,
            ...
            'budget_respected': receipt.total_synthesis_candidates <= receipt.max_total_synthesis_candidates,
        }
    if part == 'reordered':
        config = _OpaqueConfiguration('reordered', ('k0', 'k1', 'k2', 'k3', 'k4'), ('fa', 'x', 'fb', 'a', 'b'))
        receipt = _run_configuration(config)
        selected = receipt.selected
        selected_positions = sorted(position for position, _value in selected.intervention.bindings) if selected else []
        wrong, noncausal = _candidate_audit(config, receipt)
        return {
            'part': 'reordered',
            ...
            'probe_validation_exact': selected.probe_validation_exact if selected else 0,
            'probe_validation_cases': selected.probe_validation_cases if selected else len(_PROBE_VALID_CASES),
            'argument_permutation_tracks_roles': bool(set(selected_positions) == set(config.endpoint_positions)),
            'wrong_role_false_accepts': wrong,
            'noncausal_candidates_rejected': noncausal,
            'selected_intervention_ids': [selected.intervention.intervention_id if selected else ''],
...
def merge_benchmark_parts(rename: dict[str, object], reordered: dict[str, object]) -> dict[str, object]:
    ...
    r259_candidates = int(rename['synthesis_candidates_considered']) + int(reordered['synthesis_candidates_considered'])
    return {
        'milestone': 'R2.59',
        'capability': 'budgeted-semantic-intervention-index',
        'claim_boundary': 'Budgeted semantic-index reuse for bounded pure-input intervention discovery; not open-ended experiment, anchor-language, or representation invention.',
        'configurations': 3,
        ...
        'projection_index_builds': int(rename['projection_index_builds']) + int(reordered['projection_index_builds']),
        'projection_index_reuses': int(rename['projection_index_reuses']) + int(reordered['projection_index_reuses']),
        'derived_anchor_values': list(rename['derived_anchor_values']),
        'global_budget_respected': bool(rename['budget_respected'] and reordered['budget_respected']),
        'max_total_synthesis_candidates_per_full_search': _GLOBAL_SYNTHESIS_BUDGET,
        'r258_frozen_synthesis_candidates': _R258_FROZEN_SYNTHESIS_CANDIDATES,
        'r259_synthesis_candidates_considered': r259_candidates,
        'synthesis_reduction_ratio': round(_R258_FROZEN_SYNTHESIS_CANDIDATES / max(1, r259_candidates), 6),
