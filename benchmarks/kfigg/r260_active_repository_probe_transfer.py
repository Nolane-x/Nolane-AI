...
def _all_probe_inputs() -> tuple[RepositoryProbe, ...]:
    return tuple(RepositoryProbe(tuple(args)) for args in itertools.product(_PROBE_VALUES_XYALT, _PROBE_VALUES_XYALT, _PROBE_VALUES_CAP, _PROBE_VALUES_XYALT))
...
def _random_one_probe_baseline(seed, survivors, probe_inputs, oracle, verification_inputs):
    if len(survivors) <= 1:
        if len(survivors) == 1:
            ...
            return {'exact': exact, 'false_accepts': 0, 'selection_oracle_calls': 0, 'verification_oracle_calls': verify_calls, 'survivors_after': 1, 'probe_args': None}
        return {'exact': False, 'false_accepts': 0, 'selection_oracle_calls': 0, 'verification_oracle_calls': 0, 'survivors_after': 0, 'probe_args': None}
    # Preserve the preregistered baseline identity from the original capability branch
    # while moving the accepted mechanism to the non-colliding R2.60 namespace.
    probe = min(probe_inputs, key=lambda row: hashlib.sha256(f'r259-random|{seed}|{row.args}'.encode()).hexdigest())
    ...
    if len(kept) != 1:
        return {'exact': False, 'false_accepts': 0, 'selection_oracle_calls': 1, 'verification_oracle_calls': 0, 'survivors_after': len(kept), 'probe_args': list(probe.args)}
    exact, verify_calls = _verify_candidate(kept[0][0], oracle, verification_inputs)
    return {'exact': exact, 'false_accepts': 0, 'selection_oracle_calls': 1, 'verification_oracle_calls': verify_calls, 'survivors_after': 1, 'probe_args': list(probe.args)}


def _candidate_identity_invariance(candidates, initial_tests, probes, oracle, verification_inputs, reference_receipt):
    renamed = tuple(replace(candidate, candidate_id=f'opaque-{index:03d}') for index, candidate in enumerate(reversed(candidates)))
    receipt = solve_repository_patch_with_active_probes(renamed, initial_tests, probes, oracle, verification_inputs=verification_inputs, max_selection_oracle_calls=1)
    if not reference_receipt.rounds or not receipt.rounds:
        return False
    return bool(receipt.status == reference_receipt.status == 'accept' and receipt.rounds[0].probe.args == reference_receipt.rounds[0].probe.args and receipt.rounds[0].partition_signature == reference_receipt.rounds[0].partition_signature and receipt.selection_oracle_calls == reference_receipt.selection_oracle_calls == 1)
...
def run_heldout_episode(seed: int) -> dict[str, object]:
    ...
    active = solve_repository_patch_with_active_probes(candidates, initial_tests, probe_inputs, oracle, verification_inputs=all_inputs, max_selection_oracle_calls=1)
    ...
    if seed == HELDOUT_EPISODES[0]:
        identity_invariant = _candidate_identity_invariance(candidates, initial_tests, probe_inputs, oracle, all_inputs, active)
    return {
        'seed': seed,
...
def run_frozen_heldout() -> dict[str, object]:
    ...
    return {
        'schema_version': 1,
        'milestone': 'R2.60 Active Diagnostic Repository Probe Synthesis',
