...
def test_post_selection_singleton_can_still_expose_out_of_space_counterexample() -> None:
    ...
    # Both probes split Add from Sub. The deterministic active-probe tie-break
    # chooses (2, 2) first; multiplication accidentally agrees with Add there,
    # reducing the repository version space to one survivor. The second probe
    # then proves that singleton is still misspecified and must be allowed to
    # trigger closed-grammar primitive induction.
