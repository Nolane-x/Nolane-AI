...
def _case():
    ...
    verification = tuple(
        RepositoryProbe((x,))
        for x in (-17, -11, -8, -4, 1, 2, 7, 10, 13, 22)
    )
...
def test_r263_preserves_every_public_oracle_observation_across_later_expansion() -> None:
    ...
    result = solve_repository_patch_with_compositional_expansion(
        (source, wrong), (), diagnostics, traced_oracle,
        refinement_inputs=refinement,
        verification_inputs=verification,
    ...
    assert result.observed_probe_ids == (
        RepositoryProbe((5,)).probe_id,
        RepositoryProbe((-5,)).probe_id,
    )
    assert calls[:2] == [5, -5]
...
def test_r263_requires_disjoint_final_verification_pool() -> None:
    ...
    try:
        solve_repository_patch_with_compositional_expansion(
            (source, wrong), (), diagnostics, _oracle,
            refinement_inputs=refinement,
            verification_inputs=(RepositoryProbe((-5,)), *verification),
            expansion_seeds=(source,), expansion_macros=(_floor_to_mod(),),
        )
...
def test_r263_depth_budget_prevents_second_edit_and_abstains() -> None:
    ...
    result = solve_repository_patch_with_compositional_expansion(
        ...
        expansion_seeds=(source,), expansion_macros=(_floor_to_mod(),),
        max_selection_oracle_calls=1, max_refinement_oracle_calls=1,
        max_expansion_rounds=2, max_composition_depth=1,
        max_generated_candidates_per_round=8, max_sites_per_macro=8,
    )
...
def test_r263_unexpressible_second_edit_abstains_without_final_verification() -> None:
    ...
    result = solve_repository_patch_with_compositional_expansion(
        ...
        refinement_inputs=refinement,
        verification_inputs=verification,
        expansion_seeds=(source,), expansion_macros=(only_pos_macro,),
        max_selection_oracle_calls=1, max_refinement_oracle_calls=1,
        max_expansion_rounds=2, max_composition_depth=2,
        max_generated_candidates_per_round=8, max_sites_per_macro=8,
    )
