...
def test_completion_rejects_transient_pause_revoke_restore_during_inference(
    tmp_path: Path,
) -> None:
    runtime = OrganizationRuntime.first_generation()
    identity = runtime.registry.identities()[0]
    ...
    task_id = "task-post-inference-transient-pause-authority"
    runtime.tasks.add_task(task_id, title="transient pause authority", plan_node_id="P1")
    runtime.tasks.lease(task_id, identity.agent_id)

    workspace = _workspace(tmp_path)

    class _TransientPauseCompletionBackend:
        backend_id = "post-inference-transient-pause-backend-v1"
        checkpoint_digest = "post-inference-transient-pause-checkpoint-v1"
        ...
        def decide(self, request: InferenceRequest) -> AgentDecisionReceipt:
            ...
            return AgentDecisionReceipt.create(
                ...
                request=request,
                action=ExecutionAction.complete(
                    reason="stale completion after transient pause revocation",
                ),
            )

    runtime.execution.bind_backend(identity.agent_id, _TransientPauseCompletionBackend())
    session = runtime.execution.start(
        agent_id=identity.agent_id,
        task_id=task_id,
        workspace=workspace,
    ...
    try:
        ...
        with pytest.raises(
            PermissionError,
            match="lifecycle.*authority|pause.*authority|execution.*authority",
        ):
            runtime.execution.step(session.session_id)
