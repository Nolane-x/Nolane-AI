...
def _rows() -> tuple[dict[str, float], ...]:
    ...
    for lo,hi,left,middle,right in configs:
        for x in (lo-3.0,lo,(lo+hi)/2.0,hi,hi+3.0):
            out.append({'x':x,'lo':lo,'hi':hi,'left':left,'middle':middle,'right':right})
    return tuple(out)


def test_terminal_context_cannot_reuse_a_context_queried_during_intervention_profiling() -> None:
    rows = _rows(); discovery = rows[:18]; validation = rows[18:24]
    # R2.66 profiles every legal one-field 0.0 intervention on discovery rows.
    # Therefore this exact context is already queried as learning evidence when
    # profiling the authorized intervention `middle := 0.0`, even though it is not
    # one of the original discovery/validation rows.
    terminal_alias = dict(discovery[0])
    terminal_alias['middle'] = 0.0
    assert terminal_alias not in discovery
    assert terminal_alias not in validation

    need = OperatorInventionNeed(
        'R2.66 terminal intervention-evidence disjointness',
        ('x','lo','hi','left','middle','right'), 'out',
        constants=(0.0,), max_depth=3, max_candidates=25000,
    )
    with pytest.raises(ValueError, match='disjoint'):
        synthesize_contextual_composition_program(
            ...
            discovery,
            validation,
            terminal_contexts=(terminal_alias,),
