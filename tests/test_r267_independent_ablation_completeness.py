...
def test_budget_exhausted_lower_order_search_is_not_causal_falsification() -> None:
    """Ablation exhaustion must be inconclusive, never evidence of necessity.

    The normal authored R2.67 witness is discoverable with the ordinary full-triplet
    budget.  Here only the *ablation* budget is collapsed to one candidate.  Each
    proper-subset search therefore cannot possibly establish absence across the
    declared depth/grammar.  The runtime must fail closed instead of treating six
    resource-exhausted searches as proof that all three probes are causally needed.
    """
    rows = _rows()
