...
def test_a6_advances_current_audit_lane_without_rewriting_a5_admission_artifacts() -> None:
    assert integration.COMPONENT_ID == "external.integration"
    assert integration.COMPONENT_VERSION == "0.0.5"
    assert compatibility.SEMANTIC_SURFACE_VERSION == "0.0.5"
    assert admission_bundle.COMPONENT_ID == "external.integration"
    assert admission_bundle.COMPONENT_VERSION == "0.0.5"
    assert component_revision_map()["external.integration"] == 5

    assert admission_bundle.ADMISSION_AUDIT_PROTOCOL == "external-integration-admission-audit-v2"
...
def test_a6_audit_report_identity_uses_v2_protocol_and_digest_namespace() -> None:
    report = run_canonical_admission_audit(observed_epoch=11)

    assert report.protocol == "external-integration-admission-audit-v2"
    assert report.digest.startswith("admission-audit-v2-")
