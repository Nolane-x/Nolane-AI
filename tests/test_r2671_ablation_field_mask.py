...
_PARTNER = {'a':'b','b':'a','c':'d','d':'c','e':'f','f':'e'}


def _rows() -> tuple[dict[str, float], ...]:
    return tuple({field: float(value) for field, value in zip(ROLES, values, strict=True)} for values in CONFIGS[:18])


def _oracle(row: Mapping[str, object]) -> float:
    return float(row['a'])*float(row['b']) + float(row['c'])*float(row['d']) + float(row['e'])*float(row['f'])
...
def test_three_probe_authority_rejects_triplet_when_pair_specific_free_fields_suffice() -> None:
    ...
    structure = discover_three_probe_structure(
        _oracle, ROLES, (0.0,), discovery, validation,
    ...
    for left_index, right_index in itertools.combinations(range(3), 2):
        ...
        fixed_positions = {position for profile in pair_profiles for position, _value in profile.intervention.bindings}
        free_fields = tuple(ROLES[position] for position in range(len(ROLES)) if position not in fixed_positions)
        omitted_index = ({0,1,2} - {left_index,right_index}).pop()
        omitted_profile = selected.profiles[omitted_index]
        omitted_field = ROLES[omitted_profile.intervention.bindings[0][0]]
        partner_field = _PARTNER[omitted_field]
        assert omitted_field in free_fields and partner_field in free_fields
        expression = Binary('sub', Binary('add', Field('__p0'), Field('__p1')), Binary('mul', Field(omitted_field), Field(partner_field)))
        ...
        for index, context in enumerate(contexts):
            env = {field: context[field] for field in free_fields}
            env['__p0'] = left_outputs[index]
            env['__p1'] = right_outputs[index]
            exact += int(float(evaluate_expr(expression, env)) == float(_oracle(context)))
