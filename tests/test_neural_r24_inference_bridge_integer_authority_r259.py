...
def _build_request(
    *,
    ...
    return CognitiveStateEncoder().build_request(
        ...
        checkpoint_digest="checkpoint-r259",
...
def test_r259_encoder_rejects_non_exact_integer_step_frontier(step_index) -> None:
...
def test_r259_encoder_rejects_non_exact_integer_execution_lineage_version(lineage_version) -> None:
...
def test_r259_encoder_rejects_modern_lineage_alias_before_session_binding(lineage_version) -> None:
    with pytest.raises(ValueError, match="execution lineage version must be an exact integer"):
        _build_request(
            execution_lineage_version=lineage_version,
            execution_session_id="session-r259",
            workspace_epoch_id="epoch-r259",
        )


def test_r259_encoder_preserves_canonical_integer_frontiers() -> None:
