...
from cogcoder.r260_complementary_experiment_program import (
...
def test_r260_discovers_complementary_pair_and_rejects_singletons():
    rows = _contexts()
    result = discover_complementary_experiment_structure(
        ...
        rows[:18],
        rows[18:],
        context_validator=_valid_deadzone_context,
        intervention_arity=1,
    )
    assert result.passed
    assert result.selected is not None
    assert result.selected.program.composition_op == 'add'
    bindings = {spec.bindings for spec in result.selected.program.interventions}
    assert bindings == {((1, -10.0),), ((2, 10.0),)}
...
def test_r260_fails_closed_on_invalid_interventions_and_is_rename_invariant():
    rows = _contexts()
    ...
    renamed = discover_complementary_experiment_structure(
        ...
        tuple(rename_context(row) for row in rows[18:]),
        context_validator=renamed_valid,
        intervention_arity=1,
    )
...
def test_r260_hierarchical_probe_synthesis_beats_matched_flat_local_budget():
    rows = _contexts()
    need = OperatorInventionNeed(
        'bounded complementary deadzone decomposition',
        ('x', 'low', 'high'),
        'out',
        constants=(-10.0, 10.0),
        max_depth=2,
