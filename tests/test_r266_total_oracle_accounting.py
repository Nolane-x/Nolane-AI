...
def test_public_synthesis_receipt_accounts_for_every_oracle_call_including_terminal() -> None:
    ...
    receipt=synthesize_contextual_composition_program(
        oracle,('x','lo','hi','left','middle','right'),need,
        rows[:18],rows[18:24],terminal_contexts=rows[24:30],
    ...
    # `structure.oracle_calls` stops at structure discovery. The public synthesis
    # contract also makes later oracle calls during inherited final validation and
    # independent terminal verification, so the public receipt needs an auditable
    # total that exactly matches the oracle side-effect count.
