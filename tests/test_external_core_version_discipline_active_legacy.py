...
def test_legacy_owner_adoption_does_not_launder_old_transitive_helper_ownership(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    _run(repo, "init", "-q")
    _run(repo, "config", "user.email", "version-gate@example.invalid")
    ...
    _write(
        ...
        "nolane/metadata/_component_specs.py",
        'COMPONENT_SPECS = ('
        '("external.architecture", "external_core", "architecture + ADR", "architecture-v1", ()),'
        '("organization.runtime", "organization", "runtime", "runtime-v1", ()),'
        ')\n',
    )
    _write(
        repo,
        "nolane/metadata/component_versions.py",
        '_COMPONENT_REVISIONS = {component_id: 0 for component_id, *_ in COMPONENT_SPECS}\n'
        '_COMPONENT_REVISIONS.update({"external.architecture": 5, "organization.runtime": 1})\n',
    )
    _write(
        repo,
        "nolane/external_core/architecture.py",
    ...
    _write(
        repo,
        "nolane/runtime.py",
        'import cogcoder.organization.adr\nCOMPONENT_ID = "organization.runtime"\nCOMPONENT_VERSION = "0.0.1"\nVALUE = 1\n',
    )
    _write(repo, "cogcoder/organization/adr.py", "VALUE = 1\n")
    base = _commit(repo, "base before ADR owner declaration")
    ...
    _write(
        ...
        "nolane/metadata/component_versions.py",
        '_COMPONENT_REVISIONS = {component_id: 0 for component_id, *_ in COMPONENT_SPECS}\n'
        '_COMPONENT_REVISIONS.update({"external.architecture": 6, "organization.runtime": 1})\n',
    )
    head = _commit(repo, "adopt explicit ADR owner")
