...
def test_handler_binding_authority_revision_is_per_tool_and_idempotent() -> None:
    ...
    assert execution.external_core_handler_binding_authority_revision(tool_a) == 0
    assert execution.external_core_handler_binding_authority_revision(tool_b) == 0

    executor.register_handler(tool_a, _handler_a)
    assert execution.external_core_handler_binding_authority_revision(tool_a) == 1
    assert execution.external_core_handler_binding_authority_revision(tool_b) == 0
    ...
    assert execution.external_core_handler_binding_authority_revision(tool_a) == 1
    assert execution.external_core_handler_binding_authority_revision(tool_b) == 1
...
def test_handler_binding_authority_revision_is_not_advanced_by_rejected_replacement() -> None:
    ...
    revision = execution.external_core_handler_binding_authority_revision(tool_id)
    assert revision == 1
    ...
    assert execution.external_core_handler_binding_authority_revision(tool_id) == revision
