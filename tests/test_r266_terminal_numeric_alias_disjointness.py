...
def test_terminal_context_cannot_alias_learning_context_by_numeric_json_type() -> None:
    ...
    # discovery[0] is numerically integral.  Re-encode every scalar as int: Python
    # oracle semantics and R2.66 numeric equivalence treat this as the same point,
    # while raw JSON encodes -6 and -6.0 differently. Terminal independence must be
    # semantic, not bypassable through a numeric representation alias.
    ...
    with pytest.raises(ValueError, match='disjoint'):
        synthesize_contextual_composition_program(
            _oracle,('x','lo','hi','left','middle','right'),need,
            discovery,validation,terminal_contexts=(terminal_alias,),
            intervention_arity=1,composition_constants=(0.0,),composition_max_depth=2,
            composition_max_candidates_per_pair=12000,max_composition_candidates_total=120000,
            probe_constants=(0.0,),probe_max_depth=3,probe_max_candidates=20000,
        )
