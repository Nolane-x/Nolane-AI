...
def test_round_robin_result_is_hypothesis_fair() -> None:
    source = _candidate('caller:source', 'Add')
...
def test_global_generation_cap_bounds_upstream_generation_work(monkeypatch) -> None:
    # max_generated_candidates is exposed by the public solver as one global
    # generation budget. Returning only <= budget rows is insufficient if the
    # scheduler asks each hypothesis to independently generate the full budget
    # first: that silently multiplies actual generation work by hypothesis count
    # while the receipt reports only the post-truncation row count.
