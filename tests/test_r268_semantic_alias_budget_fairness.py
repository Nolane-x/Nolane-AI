...
def test_behavior_identical_intervention_aliases_do_not_change_tight_budget_outcome() -> None:
    # For this oracle, fixing either field to -1 or +1 produces exactly the
    # same intervention behavior because abs(-1) == abs(+1).  Adding +1 is
    # therefore only an identity-level alias expansion, not new causal
    # information.  It must not dilute a global proposal/search budget.
    control = _run((-1.0,))
