...
def test_probe_synthesis_midstream_failure_reports_only_observed_validation_cases(monkeypatch) -> None:
    """A partial synthesis failure must not claim validation/final cases that never ran."""
    validation_cases = 6
    ...
    assert receipt.probe_validation_exact == validation_cases
    assert receipt.probe_validation_cases == validation_cases
...
def test_terminal_probe_error_reports_only_attempted_terminal_cases() -> None:
    """A terminal error on the first selected probe must not claim the whole planned ledger."""
    ...
    def terminal_failing_oracle(row: Mapping[str, object]) -> float:
        ...
        nonzero_matches = sum(
            values[field] == float(first_terminal[field])
            for field in FIELDS
            if values[field] != 0.0
        )
    ...
    receipt = _run(
        terminal_failing_oracle,
        need=_need('R2.67.1 terminal partial receipt accounting'),
    )
