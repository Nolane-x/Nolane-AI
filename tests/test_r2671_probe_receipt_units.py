...
def _rows() -> tuple[dict[str, float], ...]:
    return tuple({field: float(value) for field, value in zip(ROLES, values, strict=True)} for values in CONFIGS[:24])
...
def test_success_receipt_counts_each_probe_validation_observation_as_one_case() -> None:
    ...
    need = OperatorInventionNeed('R2.67.1 probe receipt accounting', ROLES, 'out', constants=(0.0, 2.0), max_depth=4, max_candidates=50_000)
    receipt = synthesize_three_probe_causal_program(
        _oracle, ROLES, need, discovery, validation,
