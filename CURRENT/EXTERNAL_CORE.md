...
`external.candidate_synthesis` is canonical-native at v0.0.1 as the stateless proposal-generation boundary upstream of Capability Acquisition. Its first bounded mode composes two or more existing unary learned abstractions in semantic source order. Composition is built as transient canonical `AbstractionCall` IR, expanded against the exact Cognitive Library vocabulary, and emitted as a standalone one-parameter `LearnedAbstraction` before conversion through the existing `CapabilityCandidate` contract. This keeps candidate decoding self-contained and does not widen Capability Acquisition to carry hidden source dependencies.

Candidate Synthesis accepts discovery-phase evidence only. Independent-challenge and final-Assurance evidence are forbidden from generation, while experiment-receipt and causal-program IDs are provenance references without authority. Requests and receipts are immutable/content-addressed; set-like provenance is normalized, source order remains semantic, generation is explicitly budgeted, and missing/non-unary/duplicate/already-installed cases fail closed or abstain. The component stores no ledger, verifies that Cognitive Library digest is unchanged across synthesis, and has no API that can admit, probation, promote, self-assure, mutate the library, or mutate the frozen neural asset. A proposal enters lifecycle state only when a caller separately invokes Capability Acquisition.
