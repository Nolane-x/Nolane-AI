...
def patch_execution() -> None:
    ...
    text = replace_once(text, 'COMPONENT_VERSION = "0.0.11"', 'COMPONENT_VERSION = "0.0.12"', label="execution component version")
    text = replace_once(
        text,
...
def patch_versions() -> None:
    ...
    text = replace_once(text, '        "external.execution.control": 11,', '        "external.execution.control": 12,', label="metadata execution-control revision")
    metadata.write_text(text, encoding="utf-8")

    tests = ROOT / "tests/test_refoundation_component_versions.py"
    text = tests.read_text(encoding="utf-8")
    text = replace_once(text, '    "external.execution.control": 11,', '    "external.execution.control": 12,', label="accepted execution-control revision")
    text = replace_once(text, '    assert str(component_version("external.execution.control")) == "0.0.11"', '    assert str(component_version("external.execution.control")) == "0.0.12"', label="execution-control current version assertion")
    text = replace_once(text, '    assert str(next_component_version("external.execution.control")) == "0.0.12"', '    assert str(next_component_version("external.execution.control")) == "0.0.13"', label="execution-control next version assertion")
    tests.write_text(text, encoding="utf-8")

    wave5aa = ROOT / "tests/test_refoundation_wave5aa_native_execution_control.py"
    text = wave5aa.read_text(encoding="utf-8")
    text = replace_once(text, '    assert canonical.COMPONENT_VERSION == "0.0.11"', '    assert canonical.COMPONENT_VERSION == "0.0.12"', label="Wave 5AA canonical execution-control version")
    text = replace_once(
        text,
