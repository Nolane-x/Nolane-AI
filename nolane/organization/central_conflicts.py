...
MIGRATED_FROM = "cogcoder.organization.central_conflicts"
...
class CentralConflictPacket:
    ...
    def from_state(cls, state: Mapping[str, Any]) -> "CentralConflictPacket":
        return cls(
            ...
            resolver_agent_id=(
                None if state.get("resolver_agent_id") is None else str(state["resolver_agent_id"])
            ),
            ...
            resolution_evidence_refs=tuple(
                str(x) for x in state.get("resolution_evidence_refs", ())
            ),
        )
...
class CentralConflictRegistry:
    ...
    def open(
        ...
        *,
        ...
        packet = CentralConflictPacket(
            conflict_id=f"conflict-{counter:08d}",
    ...
    def escalate(
        ...
        *,
        ...
        evidence = tuple(str(x).strip() for x in evidence_refs if str(x).strip())
        if (
            old.status is not ConflictStatus.OPEN
            or not str(resolver_agent_id).strip()
            or not str(rationale).strip()
            or not evidence
        ):
        ...
        row = replace(
            ...
            resolver_agent_id=str(resolver_agent_id),
