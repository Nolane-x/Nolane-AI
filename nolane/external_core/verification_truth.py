...
class TruthVerificationReceipt:
    ...
    def create(cls, *, receipt_id: str, claim_id: str, verifier_id: str, source_family: str,
        ...
        if scope:
            ...
            return cls(
                ...
                "", "", evidence_ids, canonical_digest(payload), SCOPED_BINDING_MODE, scope,
            )
    ...
    def from_state(cls, state: Mapping[str, Any]) -> "TruthVerificationReceipt":
        ...
        if has_scope_keys:
            if str(state.get("binding_mode", "")) != SCOPED_BINDING_MODE:
...
class TruthVerificationLedger:
    ...
    Raw receipts remain available for audit. V1 receipts bind complete Knowledge/Epistemic state;
    A8 scoped receipts bind one live canonical dependency-scope digest. Provenance validity remains
    an independent requirement before any passing receipt contributes to Assurance.
    """
    ...
    def scoped_receipts(self, claim_id: str, *, scope_digest: str) -> tuple[TruthVerificationReceipt, ...]:
        ...
        return tuple(
            row for row in self.receipts(claim_id)
            if row.is_scoped and row.scope_digest == scope_digest
        )
