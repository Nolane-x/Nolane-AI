...
class TruthVerificationReceipt:
    ...
    def create(cls, *, receipt_id: str, claim_id: str, verifier_id: str, source_family: str,
        ...
        payload = {
            "receipt_id": str(receipt_id).strip(), "claim_id": str(claim_id).strip(),
        ...
        return cls(
            payload["receipt_id"], payload["claim_id"], payload["verifier_id"], payload["source_family"],
    ...
    def payload(self) -> dict[str, Any]:
        return {
            "receipt_id": self.receipt_id, "claim_id": self.claim_id, "verifier_id": self.verifier_id,
...
class TruthVerificationLedger:
    """Append-only challenge receipts; negative results are retained."""
    ...
    def record(self, row: TruthVerificationReceipt) -> TruthVerificationReceipt:
        previous = self._receipts.get(row.receipt_id)
    ...
    def receipts(self, claim_id: str | None = None) -> tuple[TruthVerificationReceipt, ...]:
        rows = self._receipts.values()
        if claim_id is not None:
            rows = (row for row in rows if row.claim_id == str(claim_id))
    ...
    def independent_passing_channels(self, claim_id: str, *, knowledge_digest: str, epistemic_digest: str) -> int:
        rows = self.bound_receipts(claim_id, knowledge_digest=knowledge_digest, epistemic_digest=epistemic_digest)
        return len({row.source_family for row in rows if row.passed})

    def distinct_passing_channel_kinds(self, claim_id: str, *, knowledge_digest: str, epistemic_digest: str) -> int:
        rows = self.bound_receipts(claim_id, knowledge_digest=knowledge_digest, epistemic_digest=epistemic_digest)
        return len({row.channel for row in rows if row.passed})

    @property
    def digest(self) -> str:
        return truth_digest({"protocol": "truth-verification-ledger-v1", "receipts": [row.to_state() for row in self.receipts()]})
