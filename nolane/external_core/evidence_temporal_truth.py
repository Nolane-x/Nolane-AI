...
class EvidenceTemporalBinding:
    ...
    def create(
        ...
        *,
        ...
        return cls(evidence.evidence_id, evidence.content_digest, interval, canonical_digest(payload))
    ...
    def from_state(cls, state: Mapping[str, Any]) -> "EvidenceTemporalBinding":
        _unexpected(state, {"protocol", "evidence_id", "evidence_digest", "interval", "digest"}, "evidence temporal binding")
        ...
        row = cls(evidence_id, evidence_digest, interval, canonical_digest(payload))
...
class TemporalEvidenceView:
    """Append-only temporal applicability sidecar for canonical TruthEvidence rows."""

    def __init__(self) -> None:
        self._bindings: dict[str, EvidenceTemporalBinding] = {}
    ...
    def record(self, row: EvidenceTemporalBinding) -> EvidenceTemporalBinding:
        ...
        old = self._bindings.get(row.evidence_id)
        if old is not None and old != row:
            raise ValueError("evidence temporal binding rebind collision")
        self._bindings[row.evidence_id] = row
        return row
    ...
    def binding(self, evidence_id: str) -> EvidenceTemporalBinding | None:
        return self._bindings.get(str(evidence_id))
    ...
    def from_state(cls, state: Mapping[str, Any]) -> "TemporalEvidenceView":
        ...
        for value in state.get("bindings", ()):
            ...
            if row.evidence_id in seen:
                raise ValueError("duplicate serialized evidence temporal binding")
