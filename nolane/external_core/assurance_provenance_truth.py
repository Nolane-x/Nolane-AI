...
from .epistemic_provenance_truth import (
    PROVENANCE_BINDING_MODE,
    ProvenanceEpistemicJudge,
)
from .epistemic_temporal_truth import TemporalEpistemicJudge, TemporalTruthRelationAwareScope
...
from .verification_provenance_truth import ProvenanceTruthVerificationLedger


PARENT_COMPONENT_ID = "external.assurance"
TRUTH_PROTOCOL = "truth-assurance-provenance-lineage-temporal-v5"
...
class ProvenanceTruthClosureCertificate:
    certificate_id: str
    claim_id: str
    risk: KnowledgeRisk
    scope_digest: str
    ...
    def create(
        ...
        *,
        ...
        closed: bool,
        reasons: tuple[str, ...],
    ) -> "ProvenanceTruthClosureCertificate":
        claim_id = _explicit(claim_id, "provenance closure claim_id")
        scope_digest = _explicit(scope_digest, "provenance closure scope digest")
        verification_scope_digest = _explicit(
            verification_scope_digest,
            "provenance verification projection digest",
        )
        temporal_context_digest = _explicit(
            temporal_context_digest,
            "provenance closure context digest",
        )
        context = TemporalContext.create(as_of=as_of)
        if context.digest != temporal_context_digest:
            raise ValueError("provenance closure temporal context digest mismatch")
        verification_receipt_ids = _ids(
            tuple(verification_receipt_ids),
            "provenance verification receipt ids",
        )
        epistemic_debt_ids = _ids(
            tuple(epistemic_debt_ids),
            "provenance epistemic debt ids",
        )
        ...
        if any(not value for value in reasons) or len(set(reasons)) != len(reasons):
            raise ValueError("provenance closure reasons must be explicit and unique")
        closed = bool(closed)
        if closed != (not reasons):
            raise ValueError("provenance closure decision and reasons are inconsistent")
        payload = {
        ...
        return cls(
            f"truth-provenance-closure-{digest[:24]}",
            claim_id,
            KnowledgeRisk(risk),
            scope_digest,
            verification_scope_digest,
    ...
    @classmethod
    def from_state(cls, state: Mapping[str, Any]) -> "ProvenanceTruthClosureCertificate":
        _unexpected(
            state,
            {
                "certificate_id",
        ...
        if str(state.get("protocol", "")) != TRUTH_PROTOCOL:
            raise ValueError("unsupported provenance assurance protocol")
        if str(state.get("binding_mode", "")) != PROVENANCE_BINDING_MODE:
            raise ValueError("unsupported provenance assurance binding mode")
        row = cls.create(
        ...
        if (
            ...
            raise ValueError("provenance assurance certificate digest mismatch")
        return row


class ProvenanceTruthAssuranceGate:
    """Risk-sensitive v5 closure over live temporal truth and source lineage."""

    @staticmethod
    def _lineage_conflict(
    ...
    def close(
        ...
        *,
        ...
        if not isinstance(source_provenance, SourceProvenanceRegistry):
            raise TypeError("provenance assurance requires SourceProvenanceRegistry")
        if not isinstance(verification, ProvenanceTruthVerificationLedger):
            raise TypeError("provenance assurance requires ProvenanceTruthVerificationLedger")
        ...
        if provenance_scope.temporal_scope_digest != temporal_scope.digest:
            raise ValueError("provenance scope does not bind canonical temporal scope")

        claim = knowledge.get(str(claim_id))
        target = temporal_scope.assessment(claim.claim_id)
        ...
        if self._target_conflict(temporal_scope, claim.claim_id):
            reasons.add("epistemic_claim_conflicted")
        if self._lineage_conflict(temporal_scope, claim.claim_id):
            reasons.add("epistemic_lineage_conflicted")

        lineage = set(temporal_scope.lineage_claim_ids)
        for debt in temporal_scope.debts:
        ...
        for lineage_claim_id in temporal_scope.lineage_claim_ids:
            if lineage_claim_id == claim.claim_id:
                continue
            if (
                temporal_scope.assessment(lineage_claim_id).disposition
                is not EpistemicDisposition.SUPPORTED
            ):
                reasons.add("epistemic_lineage_not_supported")

        if any(
            source_provenance.current(source_id) is None
            for source_id in provenance_scope.source_ids
        ):
        ...
        return ProvenanceTruthClosureCertificate.create(
            claim_id=claim.claim_id,
            risk=claim.risk,
            scope_digest=provenance_scope.digest,
            verification_scope_digest=verification_scope_digest,
            temporal_context_digest=temporal_context.digest,
            as_of=temporal_context.as_of,
            verification_receipt_ids=passing_ids,
            epistemic_debt_ids=tuple(row.debt_id for row in temporal_scope.debts),
            closed=not reasons,
    ...
    def validate_certificate(
        self,
        certificate: ProvenanceTruthClosureCertificate,
        *,
        temporal_context: TemporalContext,
        knowledge: KnowledgeLedger,
        evidence: EvidenceLedger,
        ...
        if not isinstance(certificate, ProvenanceTruthClosureCertificate):
            return False
        if certificate.binding_mode != PROVENANCE_BINDING_MODE:
            return False
        if (
