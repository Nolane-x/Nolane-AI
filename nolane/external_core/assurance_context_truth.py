...
from .epistemic_context_truth import (
...
from .verification_context_truth import ContextTruthVerificationLedger


PARENT_COMPONENT_ID = "external.assurance"
TRUTH_PROTOCOL = (
    "truth-assurance-context-dependence-defeasible-justification-provenance-"
    "lineage-temporal-v9"
)
...
class ContextTruthClosureCertificate:
    certificate_id: str
    claim_id: str
    risk: KnowledgeRisk
    scope_digest: str
    ...
    def create(
        ...
        *,
        ...
        if any(not value for value in reasons) or len(set(reasons)) != len(reasons):
            raise ValueError("context closure reasons must be explicit and unique")
        closed = bool(closed)
        if closed != (not reasons):
            raise ValueError("context closure decision and reasons are inconsistent")
        payload = {
        ...
        return cls(
            f"truth-context-closure-{digest[:24]}",
            claim_id,
            KnowledgeRisk(risk),
            scope_digest,
            verification_scope_digest,
    ...
    @classmethod
    def from_state(cls, state: Mapping[str, Any]) -> "ContextTruthClosureCertificate":
        _unexpected(
            state,
            {
                "certificate_id",
            ...
            "context assurance certificate",
        )
        if str(state.get("protocol", "")) != TRUTH_PROTOCOL:
            raise ValueError("unsupported context assurance protocol")
        if str(state.get("binding_mode", "")) != CONTEXT_BINDING_MODE:
            raise ValueError("unsupported context assurance binding mode")
        row = cls.create(
        ...
        if (
            ...
            raise ValueError("context assurance certificate digest mismatch")
        return row


class ContextTruthAssuranceGate:
    """Risk-sensitive A15 closure over exact context-qualified A14 truth."""

    @staticmethod
    def _supporting_lineage_claim_ids(scope: ContextTruthScope) -> frozenset[str]:
    ...
    @staticmethod
    def _relation_target_conflict(scope: ContextTruthScope, claim_id: str) -> bool:
        return any(str(claim_id) in row.claim_ids for row in scope.contradictions)

    @classmethod
    def _relation_lineage_conflict(cls, scope: ContextTruthScope, claim_id: str) -> bool:
    ...
    def close(
        ...
        *,
        ...
        verification: ContextTruthVerificationLedger,
    ) -> ContextTruthClosureCertificate:
        if not isinstance(truth_context, TruthContext):
            raise TypeError("context assurance requires TruthContext")
        if not isinstance(source_dependence, SourceDependenceRegistry):
            raise TypeError("context assurance requires SourceDependenceRegistry")
        if not isinstance(justifications, KnowledgeJustificationRegistry):
            raise TypeError("context assurance requires KnowledgeJustificationRegistry")
        if not isinstance(undercutters, JustificationUndercutterRegistry):
            raise TypeError("context assurance requires JustificationUndercutterRegistry")
        if not isinstance(claim_context, ClaimContextBindingRegistry):
            raise TypeError("context assurance requires ClaimContextBindingRegistry")
        if not isinstance(evidence_context, EvidenceContextBindingRegistry):
            raise TypeError("context assurance requires EvidenceContextBindingRegistry")
        if not isinstance(verification, ContextTruthVerificationLedger):
            raise TypeError("context assurance requires v9 verification ledger")

        scope = ContextEpistemicJudge().relation_aware_temporal_scope(
            str(claim_id),
        ...
        other_issues = tuple(
            ...
            if "context" not in issue and "dependence" not in issue
        )
        if context_issues:
            reasons.add("verification_context_invalid")
        if dependence_issues:
            reasons.add("verification_dependence_invalid")
        if coverage.invalid_receipt_ids and not context_issues and not dependence_issues:
        ...
        return ContextTruthClosureCertificate.create(
    ...
    def validate_certificate(
        self,
        certificate: ContextTruthClosureCertificate,
        *,
        truth_context: TruthContext,
        temporal_context: TemporalContext,
        knowledge: KnowledgeLedger,
        ...
        if not isinstance(certificate, ContextTruthClosureCertificate):
            return False
        if certificate.binding_mode != CONTEXT_BINDING_MODE:
            return False
        if not isinstance(truth_context, TruthContext):
            return False
        if certificate.truth_context_digest != truth_context.digest:
            return False
        if (
