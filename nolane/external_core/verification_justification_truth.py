...
from .epistemic_justification_truth import (
...
TRUTH_PROTOCOL = "truth-verification-justification-provenance-lineage-temporal-v6"
PROJECTION_PROTOCOL = "truth-verification-justification-provenance-projection-v6"
...
class JustificationTruthVerificationReceipt:
    receipt_id: str
    claim_id: str
    verifier_id: str
    ...
    def create(
        ...
        *,
        ...
        receipt_id = _explicit(receipt_id, "justification receipt id")
        claim_id = _explicit(claim_id, "justification receipt claim id")
        verifier_id = _explicit(verifier_id, "justification verifier id")
        scope_digest = _explicit(scope_digest, "justification scope digest")
        temporal_context_digest = _explicit(
            temporal_context_digest,
            "justification verification temporal context digest",
        )
        context = TemporalContext.create(as_of=as_of)
        if context.digest != temporal_context_digest:
            raise ValueError("justification verification temporal context digest mismatch")
        evidence_ids = _ids(tuple(evidence_ids), "justification verification evidence ids")
        source_provenance_digest = _explicit(
            source_provenance_digest,
            "justification verifier provenance digest",
        )
    ...
    @classmethod
    def from_state(cls, state: Mapping[str, Any]) -> "JustificationTruthVerificationReceipt":
        _unexpected(
            state,
            {
                "protocol",
            ...
            "justification verification receipt",
        )
        if str(state.get("protocol", "")) != TRUTH_PROTOCOL:
            raise ValueError("unsupported justification verification protocol")
        if str(state.get("binding_mode", "")) != JUSTIFICATION_BINDING_MODE:
            raise ValueError("unsupported justification verification binding mode")
        row = cls.create(
        ...
        if str(state["digest"]) != row.digest:
            raise ValueError("justification verification receipt digest mismatch")
        return row


@dataclass(frozen=True, slots=True)
class JustificationTruthVerificationCoverage:
    receipts: tuple[JustificationTruthVerificationReceipt, ...]
...
class JustificationTruthVerificationLedger:
    """Exact v6 verification bound to justification-aware truth state."""

    def __init__(self) -> None:
        self._receipts: dict[str, JustificationTruthVerificationReceipt] = {}

    def record(
        self,
        row: JustificationTruthVerificationReceipt,
    ) -> JustificationTruthVerificationReceipt:
        if not isinstance(row, JustificationTruthVerificationReceipt):
            raise TypeError("justification verification ledger accepts v6 receipts only")
        old = self._receipts.get(row.receipt_id)
        if old is not None and old != row:
            raise ValueError("justification verification receipt id collision")
    ...
    ) -> tuple[JustificationTruthVerificationReceipt, ...]:
    ...
    def receipt_is_current(
        receipt: JustificationTruthVerificationReceipt,
        *,
        scope: JustificationTruthScope,
        temporal_context: TemporalContext,
    ) -> bool:
        if not isinstance(receipt, JustificationTruthVerificationReceipt):
            return False
        ...
        if not isinstance(temporal_context, TemporalContext):
            return False
        return (
            receipt.binding_mode == JUSTIFICATION_BINDING_MODE
    ...
    def coverage(
        ...
        *,
        ...
        evidence_temporal: TemporalEvidenceView,
        source_provenance: SourceProvenanceRegistry,
    ) -> JustificationTruthVerificationCoverage:
        rows = self.current_receipts(
            str(claim_id),
            scope=scope,
            temporal_context=temporal_context,
        )
        valid: list[JustificationTruthVerificationReceipt] = []
        invalid: list[JustificationTruthVerificationReceipt] = []
        issues: list[str] = []
        for row in rows:
        ...
        for source_id in scope.supporting_source_ids:
            try:
        ...
        return JustificationTruthVerificationCoverage(
            receipts=rows,
    ...
    @classmethod
    def from_state(cls, state: Mapping[str, Any]) -> "JustificationTruthVerificationLedger":
        _unexpected(state, {"protocol", "receipts"}, "justification verification ledger")
        if str(state.get("protocol", "")) != TRUTH_PROTOCOL:
            raise ValueError("unsupported justification verification protocol")
        ledger = cls()
        seen: set[str] = set()
        for value in state.get("receipts", ()):
            row = JustificationTruthVerificationReceipt.from_state(value)
            if row.receipt_id in seen:
                raise ValueError("duplicate serialized justification verification receipt")
