...
from .epistemic_provenance_truth import PROVENANCE_BINDING_MODE, ProvenanceTruthScope
...
TRUTH_PROTOCOL = "truth-verification-provenance-lineage-temporal-v5"
PROJECTION_PROTOCOL = "truth-verification-provenance-lineage-projection-v5"
...
class ProvenanceTruthVerificationReceipt:
    receipt_id: str
    claim_id: str
    verifier_id: str
    ...
    def create(
        ...
        *,
        ...
        receipt_id = _explicit(receipt_id, "provenance receipt_id")
        claim_id = _explicit(claim_id, "provenance claim_id")
        verifier_id = _explicit(verifier_id, "provenance verifier_id")
        scope_digest = _explicit(scope_digest, "provenance scope digest")
        temporal_context_digest = _explicit(
            temporal_context_digest,
            "provenance verification temporal context digest",
        )
        context = TemporalContext.create(as_of=as_of)
        if context.digest != temporal_context_digest:
            raise ValueError("provenance verification temporal context digest mismatch")
        evidence_ids = _ids(tuple(evidence_ids), "provenance verification evidence ids")
        source_provenance_digest = _explicit(
            source_provenance_digest,
            "provenance verification source provenance digest",
        )
    ...
    @classmethod
    def from_state(cls, state: Mapping[str, Any]) -> "ProvenanceTruthVerificationReceipt":
        _unexpected(
            state,
            {
                "protocol",
            ...
            "provenance verification binding",
        )
        if str(state.get("protocol", "")) != TRUTH_PROTOCOL:
            raise ValueError("unsupported provenance verification protocol")
        if str(state.get("binding_mode", "")) != PROVENANCE_BINDING_MODE:
            raise ValueError("unsupported provenance verification binding mode")
        row = cls.create(
        ...
        if str(state["digest"]) != row.digest:
            raise ValueError("provenance verification receipt digest mismatch")
        return row


@dataclass(frozen=True, slots=True)
class ProvenanceTruthVerificationCoverage:
    receipts: tuple[ProvenanceTruthVerificationReceipt, ...]
...
class ProvenanceTruthVerificationLedger:
    """Append-only v5 verification with lineage-derived independence."""

    def __init__(self) -> None:
        self._receipts: dict[str, ProvenanceTruthVerificationReceipt] = {}

    def record(
        self,
        row: ProvenanceTruthVerificationReceipt,
    ) -> ProvenanceTruthVerificationReceipt:
        if not isinstance(row, ProvenanceTruthVerificationReceipt):
            raise TypeError("provenance verification ledger accepts v5 receipts only")
        old = self._receipts.get(row.receipt_id)
        if old is not None and old != row:
            raise ValueError("provenance verification receipt id collision")
    ...
    ) -> tuple[ProvenanceTruthVerificationReceipt, ...]:
    ...
    def receipt_is_current(
        receipt: ProvenanceTruthVerificationReceipt,
        *,
        scope: ProvenanceTruthScope,
        temporal_context: TemporalContext,
    ) -> bool:
        if not isinstance(receipt, ProvenanceTruthVerificationReceipt):
            return False
        if not isinstance(scope, ProvenanceTruthScope) or not isinstance(
            temporal_context,
            TemporalContext,
        ):
            return False
        return (
            receipt.binding_mode == PROVENANCE_BINDING_MODE
    ...
    def _provenance_issue(
        ...
        *,
        ...
        if (
            ...
            != source_provenance.projection_digest((row.verifier_id,))
        ):
    ...
    def coverage(
        ...
        *,
        ...
        evidence_temporal: TemporalEvidenceView,
        source_provenance: SourceProvenanceRegistry,
    ) -> ProvenanceTruthVerificationCoverage:
        rows = self.current_receipts(
            str(claim_id),
            scope=scope,
            temporal_context=temporal_context,
        )
        valid: list[ProvenanceTruthVerificationReceipt] = []
        invalid: list[ProvenanceTruthVerificationReceipt] = []
        issues: list[str] = []
        for row in rows:
        ...
        return ProvenanceTruthVerificationCoverage(
            receipts=rows,
    ...
    @classmethod
    def from_state(cls, state: Mapping[str, Any]) -> "ProvenanceTruthVerificationLedger":
        _unexpected(state, {"protocol", "receipts"}, "provenance verification ledger")
        if str(state.get("protocol", "")) != TRUTH_PROTOCOL:
            raise ValueError("unsupported provenance verification ledger protocol")
        ledger = cls()
        seen: set[str] = set()
        for value in state.get("receipts", ()):
            row = ProvenanceTruthVerificationReceipt.from_state(value)
            if row.receipt_id in seen:
                raise ValueError("duplicate serialized provenance verification receipt")
