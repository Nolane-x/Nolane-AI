...
from .verification_justification_truth import JustificationTruthVerificationLedger


PARENT_COMPONENT_ID = "external.assurance"
TRUTH_PROTOCOL = "truth-assurance-justification-provenance-lineage-temporal-v6"
...
class JustificationTruthClosureCertificate:
    certificate_id: str
    claim_id: str
    risk: KnowledgeRisk
    scope_digest: str
    ...
    def create(
        ...
        *,
        ...
        closed: bool,
        reasons: tuple[str, ...],
    ) -> "JustificationTruthClosureCertificate":
        claim_id = _explicit(claim_id, "justification closure claim id")
        scope_digest = _explicit(scope_digest, "justification closure scope digest")
        verification_scope_digest = _explicit(
            verification_scope_digest,
            "justification verification projection digest",
        )
        temporal_context_digest = _explicit(
            temporal_context_digest,
            "justification closure context digest",
        )
        context = TemporalContext.create(as_of=as_of)
        if context.digest != temporal_context_digest:
            raise ValueError("justification closure temporal context digest mismatch")
        verification_receipt_ids = _ids(
            tuple(verification_receipt_ids),
            "justification verification receipt ids",
        )
        epistemic_debt_ids = _ids(
            tuple(epistemic_debt_ids),
            "justification epistemic debt ids",
        )
        ...
        if any(not value for value in reasons) or len(set(reasons)) != len(reasons):
            raise ValueError("justification closure reasons must be explicit and unique")
        closed = bool(closed)
        if closed != (not reasons):
            raise ValueError("justification closure decision and reasons are inconsistent")
        payload = {
        ...
        return cls(
            f"truth-justification-closure-{digest[:24]}",
            claim_id,
            KnowledgeRisk(risk),
            scope_digest,
            verification_scope_digest,
    ...
    @classmethod
    def from_state(cls, state: Mapping[str, Any]) -> "JustificationTruthClosureCertificate":
        _unexpected(
            state,
            {
                "certificate_id",
        ...
        if str(state.get("protocol", "")) != TRUTH_PROTOCOL:
            raise ValueError("unsupported justification assurance protocol")
        if str(state.get("binding_mode", "")) != JUSTIFICATION_BINDING_MODE:
            raise ValueError("unsupported justification assurance binding mode")
        row = cls.create(
        ...
        if (
            ...
            raise ValueError("justification assurance certificate digest mismatch")
        return row


class JustificationTruthAssuranceGate:
    ...
    def close(
        ...
        *,
        ...
        if not isinstance(justifications, KnowledgeJustificationRegistry):
            raise TypeError("justification assurance requires KnowledgeJustificationRegistry")
        if not isinstance(verification, JustificationTruthVerificationLedger):
            raise TypeError("justification assurance requires v6 verification ledger")
        ...
        return JustificationTruthClosureCertificate.create(
    ...
    def validate_certificate(
        self,
        certificate: JustificationTruthClosureCertificate,
        *,
        temporal_context: TemporalContext,
        knowledge: KnowledgeLedger,
        evidence: EvidenceLedger,
        ...
        if not isinstance(certificate, JustificationTruthClosureCertificate):
            return False
        if certificate.binding_mode != JUSTIFICATION_BINDING_MODE:
            return False
        if (
...
__all__ = (
    ...
    "TRUTH_PROTOCOL",
    "JustificationTruthClosureCertificate",
    "JustificationTruthAssuranceGate",
)
