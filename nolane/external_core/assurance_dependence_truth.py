...
from .verification_dependence_truth import DependenceTruthVerificationLedger


PARENT_COMPONENT_ID = "external.assurance"
TRUTH_PROTOCOL = (
    "truth-assurance-dependence-defeasible-justification-provenance-lineage-temporal-v8"
)
...
class DependenceTruthClosureCertificate:
    certificate_id: str
    claim_id: str
    risk: KnowledgeRisk
    scope_digest: str
    ...
    def create(
        ...
        *,
        ...
        closed: bool,
        reasons: tuple[str, ...],
    ) -> "DependenceTruthClosureCertificate":
        claim_id = _explicit(claim_id, "dependence closure claim id")
        scope_digest = _explicit(scope_digest, "dependence closure scope digest")
        verification_scope_digest = _explicit(
            verification_scope_digest,
            "dependence verification projection digest",
        )
        temporal_context_digest = _explicit(
            temporal_context_digest,
            "dependence closure temporal context digest",
        )
        context = TemporalContext.create(as_of=as_of)
        if context.digest != temporal_context_digest:
            raise ValueError("dependence closure temporal context digest mismatch")
        verification_receipt_ids = _ids(
            tuple(verification_receipt_ids),
            "dependence verification receipt ids",
        )
        epistemic_debt_ids = _ids(
            tuple(epistemic_debt_ids),
            "dependence epistemic debt ids",
        )
        ...
        if any(not value for value in reasons) or len(set(reasons)) != len(reasons):
            raise ValueError("dependence closure reasons must be explicit and unique")
        closed = bool(closed)
        if closed != (not reasons):
            raise ValueError("dependence closure decision and reasons are inconsistent")
        payload = {
        ...
        return cls(
            f"truth-dependence-closure-{digest[:24]}",
            claim_id,
            KnowledgeRisk(risk),
            scope_digest,
            verification_scope_digest,
    ...
    @classmethod
    def from_state(cls, state: Mapping[str, Any]) -> "DependenceTruthClosureCertificate":
        _unexpected(
            state,
            {
                "certificate_id",
            ...
            "dependence assurance certificate",
        )
        if str(state.get("protocol", "")) != TRUTH_PROTOCOL:
            raise ValueError("unsupported dependence assurance protocol")
        if str(state.get("binding_mode", "")) != DEPENDENCE_BINDING_MODE:
            raise ValueError("unsupported dependence assurance binding mode")
        row = cls.create(
        ...
        if (
            ...
            raise ValueError("dependence assurance certificate digest mismatch")
        return row


class DependenceTruthAssuranceGate:
    """Risk-sensitive A14 closure over exact A13 truth plus source dependence."""

    @staticmethod
    def _supporting_lineage_claim_ids(scope: DependenceTruthScope) -> frozenset[str]:
        base = scope.defeasible_scope
        by_claim: dict[str, list[Any]] = {}
        for status in base.justification_statuses:
        ...
        visit(base.target_claim_id)
        return frozenset(seen)

    @staticmethod
    def _relation_target_conflict(scope: DependenceTruthScope, claim_id: str) -> bool:
        return any(str(claim_id) in row.claim_ids for row in scope.defeasible_scope.contradictions)

    @classmethod
    def _relation_lineage_conflict(cls, scope: DependenceTruthScope, claim_id: str) -> bool:
        lineage = set(cls._supporting_lineage_claim_ids(scope))
        lineage.discard(str(claim_id))
        return any(
            set(row.claim_ids) & lineage
            for row in scope.defeasible_scope.contradictions
        )
    ...
    def close(
        ...
        *,
        ...
        verification: DependenceTruthVerificationLedger,
    ) -> DependenceTruthClosureCertificate:
        if not isinstance(source_dependence, SourceDependenceRegistry):
            raise TypeError("dependence assurance requires SourceDependenceRegistry")
        if not isinstance(justifications, KnowledgeJustificationRegistry):
            raise TypeError("dependence assurance requires KnowledgeJustificationRegistry")
        if not isinstance(undercutters, JustificationUndercutterRegistry):
            raise TypeError("dependence assurance requires JustificationUndercutterRegistry")
        if not isinstance(verification, DependenceTruthVerificationLedger):
            raise TypeError("dependence assurance requires v8 verification ledger")

        scope = DependenceEpistemicJudge().relation_aware_temporal_scope(
            str(claim_id),
        ...
        target = base.assessment(claim.claim_id)
        ...
        for lineage_claim_id in sorted(supporting_lineage):
            if lineage_claim_id == claim.claim_id:
                continue
            disposition = base.assessment(lineage_claim_id).disposition
        ...
        provenance_issues = tuple(issue for issue in coverage.issues if "dependence" not in issue)
        if dependence_issues:
            reasons.add("verification_dependence_invalid")
        if coverage.invalid_receipt_ids or provenance_issues:
        ...
        return DependenceTruthClosureCertificate.create(
            ...
            epistemic_debt_ids=tuple(row.debt_id for row in base.debts),
            closed=not reasons,
    ...
    def validate_certificate(
        self,
        certificate: DependenceTruthClosureCertificate,
        *,
        temporal_context: TemporalContext,
        knowledge: KnowledgeLedger,
        evidence: EvidenceLedger,
        ...
        if not isinstance(certificate, DependenceTruthClosureCertificate):
            return False
        if certificate.binding_mode != DEPENDENCE_BINDING_MODE:
            return False
        if (
