...
from .epistemic_observation_truth import OBSERVATION_BINDING_MODE, ObservationTruthScope
...
TRUTH_PROTOCOL = (
    "truth-verification-observation-context-dependence-defeasible-justification-"
    "provenance-lineage-temporal-v10"
)
PROJECTION_PROTOCOL = (
    "truth-verification-observation-context-dependence-defeasible-justification-"
    "provenance-projection-v10"
)
_COMPONENT_PROTOCOL = "truth-verification-observation-dependence-component-v10"
...
class ObservationTruthVerificationReceipt:
    receipt_id: str
    claim_id: str
    verifier_id: str
    ...
    def create(
        ...
        *,
        ...
        receipt_id = _explicit(receipt_id, "observation verification receipt id")
        claim_id = _explicit(claim_id, "observation verification claim id")
        verifier_id = _explicit(verifier_id, "observation verifier id")
        scope_digest = _explicit(scope_digest, "observation verification scope digest")
        truth_context_digest = _explicit(
            truth_context_digest,
            "observation verification truth-context digest",
        )
        temporal_context_digest = _explicit(
            temporal_context_digest,
            "observation verification temporal-context digest",
        )
        temporal = TemporalContext.create(as_of=as_of)
        if temporal.digest != temporal_context_digest:
            raise ValueError("observation verification temporal context digest mismatch")
        observation_requirement_digest = _explicit(
            observation_requirement_digest,
            "observation verification requirement projection digest",
        )
        observation_result_digest = _explicit(
            observation_result_digest,
            "observation verification result projection digest",
        )
        evidence_ids = _ids(tuple(evidence_ids), "observation verification evidence ids")
        source_provenance_digest = _explicit(
            source_provenance_digest,
            "observation verifier provenance digest",
        )
        source_dependence_digest = _explicit(
            source_dependence_digest,
            "observation verifier source-dependence digest",
        )
        evidence_context_digest = _explicit(
            evidence_context_digest,
            "observation verifier evidence-context digest",
        )
    ...
    @classmethod
    def from_state(cls, state: Mapping[str, Any]) -> "ObservationTruthVerificationReceipt":
        _unexpected(
            state,
            {
                "protocol",
        ...
        if str(state.get("protocol", "")) != TRUTH_PROTOCOL:
            raise ValueError("unsupported observation verification protocol")
        if str(state.get("binding_mode", "")) != OBSERVATION_BINDING_MODE:
            raise ValueError("unsupported observation verification binding mode")
        row = cls.create(
        ...
        if str(state["digest"]) != row.digest:
            raise ValueError("observation verification receipt digest mismatch")
        return row


@dataclass(frozen=True, slots=True)
class ObservationTruthVerificationCoverage:
    receipts: tuple[ObservationTruthVerificationReceipt, ...]
...
class ObservationTruthVerificationLedger:
    """Dedicated A16 verifier ledger; observation state is coverage, never independence."""

    def __init__(self) -> None:
        self._receipts: dict[str, ObservationTruthVerificationReceipt] = {}

    def record(
        self,
        row: ObservationTruthVerificationReceipt,
    ) -> ObservationTruthVerificationReceipt:
        if not isinstance(row, ObservationTruthVerificationReceipt):
            raise TypeError("observation verification ledger accepts v10 receipts only")
        old = self._receipts.get(row.receipt_id)
        if old is not None and old != row:
            raise ValueError("observation verification receipt id collision")
    ...
    ) -> tuple[ObservationTruthVerificationReceipt, ...]:
    ...
    def receipt_is_current(
        receipt: ObservationTruthVerificationReceipt,
        *,
        scope: ObservationTruthScope,
        truth_context: TruthContext,
        temporal_context: TemporalContext,
    ) -> bool:
        if not isinstance(receipt, ObservationTruthVerificationReceipt):
            return False
        ...
        if not isinstance(temporal_context, TemporalContext):
            return False
        return (
            receipt.binding_mode == OBSERVATION_BINDING_MODE
            and scope.binding_mode == OBSERVATION_BINDING_MODE
            ...
            and receipt.observation_requirement_digest == scope.observation_requirement_digest
            and receipt.observation_result_digest == scope.observation_result_digest
        )
    ...
    def _lineage_requirements(
        ...
        rows = tuple(
            sorted(
                (
                    requirement
                    for claim_id in scope.lineage_claim_ids
    ...
    def _scope_issue(
        ...
        *,
        ...
        requirements = cls._lineage_requirements(scope, observation_requirements)
        if (
            scope.observation_requirement_digest
            != observation_requirements.projection_digest(scope.lineage_claim_ids)
            or scope.requirement_digests != tuple(sorted(row.digest for row in requirements))
        ):
            return "verification_observation_requirements_stale"
        if scope.observation_result_digest != observation_results.projection_digest(requirements):
            return "verification_observation_results_stale"
        return None
    ...
    @staticmethod
    def _component_keys(
        rows: tuple[ObservationTruthVerificationReceipt, ...],
        *,
        source_provenance: SourceProvenanceRegistry,
        source_dependence: SourceDependenceRegistry,
    ) -> tuple[str, ...]:
    ...
    def coverage(
        ...
        *,
        ...
        if not isinstance(scope, ObservationTruthScope):
            raise TypeError("observation verification requires ObservationTruthScope")
        if not isinstance(truth_context, TruthContext):
            raise TypeError("observation verification requires TruthContext")
        if not isinstance(claim_context, ClaimContextBindingRegistry):
            raise TypeError("observation verification requires ClaimContextBindingRegistry")
        if not isinstance(evidence_context, EvidenceContextBindingRegistry):
            raise TypeError("observation verification requires EvidenceContextBindingRegistry")
        if not isinstance(observation_requirements, ObservationRequirementRegistry):
            raise TypeError("observation verification requires ObservationRequirementRegistry")
        if not isinstance(observation_results, ObservationResultLedger):
            raise TypeError("observation verification requires ObservationResultLedger")
        ...
        valid: list[ObservationTruthVerificationReceipt] = []
        invalid: list[ObservationTruthVerificationReceipt] = []
        issues: list[str] = []
        for row in rows:
        ...
        eligible: list[ObservationTruthVerificationReceipt] = []
        ...
        return ObservationTruthVerificationCoverage(
            receipts=rows,
    ...
    @classmethod
    def from_state(cls, state: Mapping[str, Any]) -> "ObservationTruthVerificationLedger":
        _unexpected(state, {"protocol", "receipts"}, "observation verification ledger")
        if str(state.get("protocol", "")) != TRUTH_PROTOCOL:
            raise ValueError("unsupported observation verification protocol")
        ledger = cls()
        seen: set[str] = set()
        for value in state.get("receipts", ()):
            row = ObservationTruthVerificationReceipt.from_state(value)
            if row.receipt_id in seen:
                raise ValueError("duplicate serialized observation verification receipt")
