...
from runtime import evaluate_r19
...
def main():
    ...
    baseline=evaluate_r11(
        ...
        indices=tuple(lock["benchmark"]["development_indices"]),horizon=1,
    )
    rows=[]
    for c in lock["selection"]["candidates"]:
        ev=evaluate_r19(
            parent,make_task=make_r18_task,families=FAMILIES,split="dev",
            indices=tuple(lock["benchmark"]["development_indices"]),alpha=float(c["alpha"]),
        )
    ...
    manifest={
      "status":"R19_DEV_ELIGIBLE_FRESH_UNOPENED" if selected else "R19_DEV_REJECTED_FRESH_UNOPENED",
      "fresh_opened":False,
      "parent_checkpoint_sha256":meta["checkpoint_sha256"],
      "parent_state_dict_sha256":meta["state_dict_sha256"],
    ...
    (args.output/"r19.dev.json").write_text(json.dumps(manifest,indent=2,sort_keys=True)+"\n")
    print(json.dumps({
      "status":manifest["status"],"parent_dev_solved":baseline["solved"],
      ...
      "candidates":[{
        ...
        "overrides":r["evaluation"]["overrides_vs_r11"],
        "delta":r["total_solved_delta_vs_parent"],"family_delta":r["family_solved_delta_vs_parent"],
        "eligible":r["eligible"]
