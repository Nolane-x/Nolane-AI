...
from decision_set_ranker_core import NativeR36DecisionSetRankerEnsemble
from decision_set_ranker_training import (
...
def main() -> int:
    ...
    if lock.get("candidate") != "Neural-vNext-Native-R36-DecisionSetRanker":
        raise ValueError("unexpected R36 candidate")
    if (
        lock["fresh_isolation"]["status"] != "UNOPENED"
        or lock["benchmark"]["fresh_indices"] != [280, 319]
    ):
        raise ValueError("R36 fresh isolation changed")
    ...
    print(
        json.dumps(
            {
                "status": "R36_R11_BASELINE_FRESH_UNOPENED",
    ...
    if model.goal_parameter_count() != int(architecture["expected_goal_successor_parameters"]):
        raise AssertionError("R36 goal parameter count changed")
    if model.set_parameter_count() != int(architecture["expected_set_successor_parameters"]):
        raise AssertionError("R36 set parameter count changed")
    if model.parameter_count() != int(architecture["expected_successor_parameters"]):
        raise AssertionError("R36 total parameter count changed")
    ...
    status = (
        "R36_DEV_ELIGIBLE_CONFIRMATION_UNOPENED_FRESH_UNOPENED"
        if eligibility["eligible"]
        else "R36_DEV_REJECTED_CONFIRMATION_FRESH_UNOPENED"
    )
    selected_config = {
        "temperature": temperature,
    ...
    manifest = {
        **metadata,
        ...
        "selected_candidate": "decision_set_listwise_rescue_ranker",
