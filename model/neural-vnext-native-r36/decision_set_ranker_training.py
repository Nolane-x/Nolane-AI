...
R11_HORIZON = 1
CHECKPOINT_FORMAT = "nolane-neural-vnext-native-r36-decision-set-ranker-v1"
        ...
        for epoch in range(int(epochs)):
            ...
            for position, group_index in enumerate(order):
                ...
                if bool(rescue_mask.any().item()):
                    rescue_indices = torch.nonzero(rescue_mask, as_tuple=False).flatten() + 1
                    listwise_loss = -torch.logsumexp(log_probs[rescue_indices], dim=0)
                else:
                ...
                harm_penalty = -torch.log((1.0 - harm_mass).clamp_min(1.0e-6))
                ...
                (loss / float(max(1, int(batch_size)))).backward()
                pending += 1
                if pending >= int(batch_size) or position == len(order) - 1:
                    torch.nn.utils.clip_grad_norm_(head.parameters(), 1.0)
                    optimizer.step()
                    optimizer.zero_grad(set_to_none=True)
                    pending = 0
                ...
                losses.append(float(loss.detach().item()))
                outcome_correct += int(
                    (outcome_logits.detach().argmax(dim=-1) == labels).sum().item()
                )
            ...
            history.append(
                {
                    ...
                    "null_accuracy_no_rescue": selected_null_correct / max(1, no_rescue_groups),
                }
            )
        summaries.append({"head": head_index, "history": history})
    model.eval()
...
def _score_decision_set(
    ...
    return {
        ...
        "min_rescue": float(outcome_probs[:, candidate_index, RESCUE_CLASS].min().item()),
        "mean_rescue": float(outcome_probs[:, candidate_index, RESCUE_CLASS].mean().item()),
...
def _select_action_row(
    ...
    *,
    ...
    if selected_index is None or int(selected_index) == 0:
        return None, score
    if (
        float(score["min_selection"]) < float(rescue_threshold)
...
def evaluate_r36(
    ...
    *,
    ...
    if split not in {"dev", "fresh"}:
        raise ValueError("R36 evaluation split must be dev/fresh")
    rows: list[dict[str, Any]] = []
