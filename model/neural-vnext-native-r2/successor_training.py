...
CHECKPOINT_FORMAT = "nolane-neural-vnext-native-r2-transition-v1"
...
def load_lock(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(fresh, dict) or fresh.get("status") != "UNOPENED":
        raise ValueError("successor PREDEV lock must keep fresh unopened")
    return payload
...
def _task_order(
    *,
    ...
    if start < 0 or end < start:
        raise ValueError("invalid successor training index range")
    rows = [
        (str(family), index)
...
def train_episode(
    ...
    *,
    ...
    trace = PublicTransitionTrace(max_length=model.trace_length)
    ...
    if not trainable:
        raise ValueError("successor exposes no gradients")
...
def train_successor_policy(
    ...
    *,
    ...
    if expert_epochs < 1 and not dagger_teacher_mix:
        raise ValueError("at least one successor training epoch is required")
...
def evaluate_successor(
    ...
    *,
    ...
    if split not in {"dev", "fresh"}:
        raise ValueError("successor evaluation split must be dev or fresh")
...
def save_successor_checkpoint(
    ...
    *,
    ...
    if parent_state_sha != str(parent_state_dict_sha256):
        raise ValueError("frozen parent state changed before successor checkpoint save")
    payload = {
...
def load_successor_checkpoint(
    path: str | Path,
) -> tuple[NativeR2TransitionPolicy, dict[str, Any]]:
    payload = torch.load(Path(path), map_location="cpu", weights_only=True)
    if not isinstance(payload, dict) or payload.get("format") != CHECKPOINT_FORMAT:
        raise ValueError("unsupported successor checkpoint")
    architecture = payload.get("architecture")
    state = payload.get("state_dict")
    if not isinstance(architecture, dict) or not isinstance(state, dict):
        raise ValueError("successor checkpoint is missing architecture/state")

    parent = NativeRecurrentPolicy(
        global_dim=int(architecture["parent_global_dim"]),
        action_dim=int(architecture["parent_action_dim"]),
        hidden_dim=int(architecture["parent_hidden_dim"]),
        attention_heads=int(architecture["parent_attention_heads"]),
    )
    model = NativeR2TransitionPolicy(
        parent,
        trace_token_dim=int(architecture["trace_token_dim"]),
        trace_hidden_dim=int(architecture["trace_hidden_dim"]),
        trace_length=int(architecture["trace_length"]),
    )
    model.load_state_dict(state, strict=True)
    if successor_state_dict_sha256(model) != payload.get("state_dict_sha256"):
    ...
    if state_dict_sha256(model.parent.state_dict()) != payload.get(
        ...
        raise ValueError("successor checkpoint embedded parent digest mismatch")
    if model.full_parameter_count() != int(payload.get("parameters", -1)):
        raise ValueError("successor checkpoint parameter audit mismatch")
    if model.successor_parameter_count() != int(
        payload.get("successor_parameters", -1)
    ):
        raise ValueError("successor-owned parameter audit mismatch")
    model.eval()
    metadata = {
        key: value
        for key, value in payload.items()
