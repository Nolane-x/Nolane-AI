...
from dual_evidence_set_ranker_core import NativeR38DualEvidenceSetRankerEnsemble
...
def main() -> int:
    ...
    if lock.get("candidate") != "Neural-vNext-Native-R38-DualEvidenceSetRanker":
        raise ValueError("unexpected R38 candidate")
    if (
        lock["fresh_isolation"]["status"] != "UNOPENED"
        or lock["benchmark"]["fresh_indices"] != [280, 319]
    ):
        raise ValueError("R38 fresh isolation changed")
    ...
    print(
        json.dumps(
            {
                "status": "R38_R11_BASELINE_FRESH_UNOPENED",
    ...
    if model.goal_parameter_count() != int(architecture["expected_goal_successor_parameters"]):
        raise AssertionError("R38 goal parameter count changed")
    if model.set_parameter_count() != int(architecture["expected_set_successor_parameters"]):
        raise AssertionError("R38 set parameter count changed")
    if model.parameter_count() != int(architecture["expected_successor_parameters"]):
        raise AssertionError("R38 total parameter count changed")
    ...
    status = (
        "R38_DEV_ELIGIBLE_CONFIRMATION_UNOPENED_FRESH_UNOPENED"
        if eligibility["eligible"]
        else "R38_DEV_REJECTED_CONFIRMATION_FRESH_UNOPENED"
    )
    selected_config = {
        "temperature": temperature,
    ...
    manifest = {
        **metadata,
        ...
        "private_boundary": {
            ...
            "direct_rescue_probability_gate": True,
        ...
        "selected_candidate": "dual_evidence_decision_set_ranker",
