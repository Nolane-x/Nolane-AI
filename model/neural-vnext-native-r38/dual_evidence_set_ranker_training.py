...
R11_HORIZON = 1
CHECKPOINT_FORMAT = "nolane-neural-vnext-native-r38-dual-evidence-set-ranker-v1"
...
def _training_stratum(
    ...
    *,
    ...
    if not group:
        raise ValueError("empty R38 decision group")
...
def train_decision_set_ensemble(
    ...
    *,
    ...
    if int(stratum_count) < 2:
        raise ValueError("R38 stratum_count must be >=2")
...
def evaluate_r38(
    ...
    *,
    ...
    if split not in {"dev", "fresh"}:
        raise ValueError("R38 evaluation split must be dev/fresh")
    rows: list[dict[str, Any]] = []
