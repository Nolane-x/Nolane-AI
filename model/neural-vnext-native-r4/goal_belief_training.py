...
from attributed_training import load_r3_checkpoint
...
CHECKPOINT_FORMAT = "nolane-neural-vnext-native-r4-public-goal-belief-v1"
        ...
        stages.append(
            {
                ...
                "mean_episode_residual_mse": (
                    total_residual_mse / max(1, episodes)
                ),
            }
        )
    return {
...
def rollout_r4(
    ...
    return {
        ...
        "actions": actions,
        "final_belief_candidates": belief.candidate_count,
    }
            ...
            rows.append(
                {
                    ...
                    "steps": int(result["steps"]),
                    "final_belief_candidates": int(
                        result["final_belief_candidates"]
                    ),
                }
            )
...
def save_r4_checkpoint(
    ...
    *,
    ...
    if parent_state_sha != str(parent_state_dict_sha256):
        raise ValueError("frozen R3 parent changed before R4 checkpoint save")
    payload = {
...
def load_r4_checkpoint(
    path: str | Path,
) -> tuple[NativeR4PublicGoalBeliefPolicy, dict[str, Any]]:
    ...
    r3 = NativeR3AttributedBeliefPolicy(
        r2,
        attribution_token_dim=int(architecture["r3_attribution_token_dim"]),
        attribution_hidden_dim=int(
            architecture["r3_attribution_hidden_dim"]
        ),
        attribution_length=int(architecture["r3_attribution_length"]),
    )
    model = NativeR4PublicGoalBeliefPolicy(
        r3,
