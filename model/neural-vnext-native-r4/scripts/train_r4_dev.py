...
from latent_goal_core import NativeR4LatentGoalBeliefPolicy  # noqa: E402
from latent_goal_training import (  # noqa: E402
    evaluate_r4,
    load_lock,
    save_r4_checkpoint,
    train_r4_policy,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Train R4 latent hidden-goal belief on frozen accepted R3. "
            "Fresh remains forbidden."
        )
    )
    parser.add_argument(
        "--lock",
        type=Path,
        default=ROOT / "PREDEV_LOCK.json",
    )
    parser.add_argument(
        "--parent-checkpoint",
        type=Path,
    ...
    if benchmark["fresh_indices"] != [120, 159]:
        raise ValueError("R4 must reserve fresh:120..159")
    ...
    for candidate_index, cfg in enumerate(training["candidates"]):
        ...
        model = NativeR4LatentGoalBeliefPolicy(
            ...
            belief_hidden_dim=int(cfg["belief_hidden_dim"]),
            goal_embedding_dim=int(cfg["goal_embedding_dim"]),
        )
        if state_dict_sha256(model.parent.state_dict()) != actual_parent_state:
            raise ValueError(
                f"candidate {name} changed frozen R3 parent at init"
            )
        ...
        train_summary = train_r4_policy(
            ...
            goal_aux_weight=float(cfg["goal_aux_weight"]),
        )
        if state_dict_sha256(model.parent.state_dict()) != actual_parent_state:
            raise ValueError(
                f"candidate {name} mutated frozen R3 parent"
            )
