...
from belief_tree_planner import choose_public_belief_tree_action
...
def rollout_r14(
    ...
    *,
    ...
    while not task.done:
        ...
        action, decision = choose_public_belief_tree_action(
        ...
        belief_tree_decisions += int(
            decision.get("reason") == "certified_belief_tree_advantage"
        )
    ...
    return {
        ...
        "belief_tree_decisions": belief_tree_decisions,
...
def evaluate_r14(
    ...
    *,
    ...
    if split not in {"dev", "fresh"}:
        raise ValueError("R14 evaluation split must be dev or fresh")
    ...
    for family in families:
        ...
        for index in range(int(indices[0]), int(indices[1]) + 1):
            ...
            fdec += int(result["belief_tree_decisions"])
            solved += int(result["solved"])
            steps += int(result["steps"])
            overrides += int(result["overrides_vs_r11"])
            decisions += int(result["belief_tree_decisions"])
            episodes += 1
            rows.append({
                "family": str(family),
                "split": str(split),
    ...
    return {
        ...
        "belief_tree_decisions": decisions,
