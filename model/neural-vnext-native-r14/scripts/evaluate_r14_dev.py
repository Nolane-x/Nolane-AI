...
from belief_tree_runtime import evaluate_r14
...
def main() -> int:
    ...
    if lock.get("candidate") != "Neural-vNext-Native-R14-BeliefTreeCausal":
        raise ValueError("unexpected R14 candidate identity")
    if lock["fresh_isolation"]["status"] != "UNOPENED":
        raise ValueError("R14 fresh must remain unopened")
    if lock["benchmark"]["development_indices"] != [736, 767]:
        raise ValueError("R14 dev identities changed")
    if lock["benchmark"]["fresh_indices"] != [280, 319]:
        raise ValueError("R14 fresh reservation changed")
    ...
    print(json.dumps({
        "status": "R14_ACCEPTED_R11_BASELINE_FRESH_UNOPENED",
    ...
    for cfg in lock["selection"]["candidates"]:
        ...
        if eligibility["eligible"] and (
            selected is None or rank > selected["rank"]
        ):
    ...
    payload = {
        "schema_version": 1,
        "status": (
            "R14_DEV_ELIGIBLE_FRESH_UNOPENED"
            if selected
            else "R14_DEV_NO_ELIGIBLE_CANDIDATE_FRESH_UNOPENED"
        ),
