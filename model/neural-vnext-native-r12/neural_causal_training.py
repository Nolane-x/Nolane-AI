...
from neural_causal_core import (
    CAUSAL_FEATURE_DIM,
    NativeR12NeuralCausalResidualPolicy,
)

CHECKPOINT_FORMAT = "nolane-neural-vnext-native-r12-neural-causal-residual-v1"
...
def load_lock(path: str | Path) -> dict[str, Any]:
    ...
    if payload.get("schema_version") != 1:
        raise ValueError("unsupported R12 PREDEV lock")
    if payload.get("candidate") != "Neural-vNext-Native-R12-NeuralCausalResidual":
        raise ValueError("unexpected R12 candidate")
    if payload["fresh_isolation"]["status"] != "UNOPENED":
        raise ValueError("R12 fresh must remain unopened")
    if payload["benchmark"]["fresh_indices"] != [280, 319]:
        raise ValueError("R12 must reserve fresh:280..319")
    return payload
...
def build_causal_action_features(
    *,
    ...
    if tensor.shape != (actions, CAUSAL_FEATURE_DIM):
        raise AssertionError("unexpected R12 causal feature shape")
    return tensor
...
def train_episode(
    ...
    *,
    ...
    if getattr(task, "split", None) != "train":
        raise ValueError("R12 training is train-split only")
    if getattr(task, "family", None) != "implicit_goal_regimes":
        raise ValueError("R12 training is hidden-goal-family only")
    ...
    if not trainable:
        raise ValueError("R12 exposes no trainable gradients")
...
def evaluate_r12(
    ...
    *,
    ...
    if split not in {"dev", "fresh"}:
        raise ValueError("R12 evaluation split must be dev or fresh")
...
def load_r12_checkpoint(
    path: str | Path,
    *,
    parent_checkpoint_path: str | Path,
) -> tuple[NativeR12NeuralCausalResidualPolicy, dict[str, Any]]:
    payload = torch.load(Path(path), map_location="cpu", weights_only=True)
    if not isinstance(payload, dict) or payload.get("format") != CHECKPOINT_FORMAT:
        raise ValueError("unsupported R12 checkpoint")
    ...
    if not isinstance(architecture, dict) or not isinstance(successor_state, dict):
        raise ValueError("R12 checkpoint missing architecture/state")
    ...
    if parent_metadata["checkpoint_sha256"] != payload["parent_checkpoint_sha256"]:
        raise ValueError("R12 embedded parent checkpoint authority mismatch")
    if parent_metadata["state_dict_sha256"] != payload["parent_state_dict_sha256"]:
        raise ValueError("R12 embedded parent state authority mismatch")

    model = NativeR12NeuralCausalResidualPolicy(
        parent,
        posterior_embedding_dim=int(architecture["r12_posterior_embedding_dim"]),
        residual_hidden_dim=int(architecture["r12_residual_hidden_dim"]),
        max_support=int(architecture["r12_max_support"]),
        parent_margin=float(architecture["r12_parent_margin"]),
    )
    model.load_successor_state_dict(successor_state)
    if model.successor_state_sha256() != payload["successor_state_dict_sha256"]:
        raise ValueError("R12 successor state digest mismatch")
    if model.full_parameter_count() != int(payload["parameters"]):
        raise ValueError("R12 parameter audit mismatch")
    if model.successor_parameter_count() != int(payload["successor_parameters"]):
        raise ValueError("R12 successor parameter audit mismatch")
    model.eval()
    metadata = {
        key: value for key, value in payload.items()
        if key != "successor_state_dict"
