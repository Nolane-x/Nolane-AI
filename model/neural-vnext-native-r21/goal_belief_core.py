...
def encode_public_goal_features(
    *,
    ...
    feedback = list(float(v) for v in previous_feedback)
    if len(feedback) != 3:
    ...
    if features.shape != (PUBLIC_GOAL_FEATURE_DIM,):
        raise AssertionError("unexpected R21 public goal feature shape")
    return features
...
class NativeR21GoalBeliefEnsemble(nn.Module):
    """Dedicated public hidden-goal classifier.

    The model is trained with private goal labels only on train identities.
    At inference, exact public consistency remains a hard support mask.
    """
    ...
    def architecture(self) -> dict[str, Any]:
        return {
            "r21_ensemble_size": self.ensemble_size,
            "r21_hidden_dim": self.hidden_dim,
            "r21_public_goal_feature_dim": PUBLIC_GOAL_FEATURE_DIM,
            "r21_goal_hypothesis_count": GOAL_HYPOTHESIS_COUNT,
        }
    ...
    def ensemble_logits(self, features: Tensor) -> Tensor:
        if features.ndim != 2 or features.shape[-1] != PUBLIC_GOAL_FEATURE_DIM:
            raise ValueError("R21 features must be [batch,dim]")
    ...
    def posterior(
        ...
        *,
        ...
        if features.ndim != 1 or features.shape[0] != PUBLIC_GOAL_FEATURE_DIM:
            raise ValueError("R21 posterior expects one public feature row")
        if support_mask.shape != (GOAL_HYPOTHESIS_COUNT,):
        ...
        probs = probs * support_mask.float()
        mass = probs.sum()
...
def fuse_public_and_neural_posterior(
    public_posterior: Tensor,
    neural_posterior: Tensor,
    *,
    beta: float,
) -> Tensor:
    if public_posterior.shape != (GOAL_HYPOTHESIS_COUNT,):
        raise ValueError("public posterior must be [125]")
    if neural_posterior.shape != (GOAL_HYPOTHESIS_COUNT,):
        raise ValueError("neural posterior must be [125]")
    beta = float(beta)
    if not 0.0 <= beta <= 1.0:
        raise ValueError("beta must lie in [0,1]")
    support = public_posterior > 0.0
    fused = (1.0 - beta) * public_posterior + beta * neural_posterior
    fused = fused * support.float()
    mass = fused.sum()
    if float(mass.item()) <= 0.0:
        return public_posterior / public_posterior.sum()
    return fused / mass
