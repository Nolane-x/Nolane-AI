...
from consistency_core import (
    NativeR5PublicConsistencyPolicy,
    PublicGoalConsistencyBelief,
    r5_state_dict_sha256,
)

CHECKPOINT_FORMAT = (
    "nolane-neural-vnext-native-r5-public-consistency-v1"
)
...
def load_lock(path: str | Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("unsupported R5 PREDEV lock")
    if (
        payload.get("candidate")
        != "Neural-vNext-Native-R5-PublicConsistency"
    ):
        raise ValueError("unexpected R5 candidate identity")
    ...
    if (
        ...
        raise ValueError(
            "R5 PREDEV lock must keep fresh unopened"
        )
    benchmark = payload.get("benchmark")
    if (
        not isinstance(benchmark, dict)
        or benchmark.get("fresh_indices") != [160, 199]
    ):
        raise ValueError(
            "R5 must reserve untouched fresh:160..199"
        )
    return payload
...
def _task_order(
    *,
    ...
    if start < 0 or end < start:
        raise ValueError("invalid R5 training index range")
    rows = [
        (str(family), index)
...
def train_episode(
    ...
    *,
    ...
    if getattr(task, "split", None) != "train":
        raise ValueError("R5 training is train-split only")
    if (
        getattr(task, "family", None)
        != "implicit_goal_regimes"
    ):
        raise ValueError(
            "R5 training is hidden-goal-family only"
        )
    ...
    if not trainable:
        raise ValueError(
            "R5 exposes no trainable successor gradients"
        )
...
def train_r5_policy(
    ...
    *,
    ...
    if tuple(families) != (
        ...
        raise ValueError(
            "R5 training scope must be exactly "
            "implicit_goal_regimes"
        )
    ...
    if not schedule:
        raise ValueError(
            "R5 requires at least one training stage"
        )
...
def evaluate_r5(
    ...
    *,
    ...
    if split not in {"dev", "fresh"}:
        raise ValueError(
            "R5 evaluation split must be dev or fresh"
        )
...
def load_r5_checkpoint(
    ...
    if (
        r5_state_dict_sha256(model)
        != payload.get("state_dict_sha256")
    ):
        raise ValueError(
            "R5 checkpoint tensor digest mismatch"
        )
    if (
        state_dict_sha256(
            model.parent.state_dict()
    ...
    if (
        ...
        raise ValueError(
            "R5 checkpoint parameter audit mismatch"
        )
    if (
        model.successor_parameter_count()
        != int(
