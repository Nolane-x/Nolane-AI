...
def test_r5_initialization_is_exactly_r4_equivalent() -> None:
    parent = _r4_parent()
...
def test_visible_target_hard_gate_blocks_r5_residual() -> None:
    parent = _r4_parent()
    model = NativeR5PublicConsistencyPolicy(
        copy.deepcopy(parent),
        consistency_embedding_dim=16,
...
def test_parent_frozen_and_only_r5_parameters_trainable() -> None:
    model = NativeR5PublicConsistencyPolicy(
        _r4_parent(),
        consistency_embedding_dim=16,
        residual_hidden_dim=24,
...
def test_r5_checkpoint_roundtrip_binds_frozen_r4_parent(
    tmp_path: Path,
) -> None:
    model = NativeR5PublicConsistencyPolicy(
        _r4_parent(),
    ...
    restored, observed = load_r5_checkpoint(path)
