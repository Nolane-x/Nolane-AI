...
from consistency_core import NativeR5PublicConsistencyPolicy
...
def main() -> int:
    ...
    if benchmark["fresh_indices"] != [160, 199]:
        raise ValueError("R5 must reserve fresh:160..199")
    if lock["fresh_isolation"]["status"] != "UNOPENED":
        raise ValueError("R5 fresh court must remain unopened")
    ...
    print(json.dumps({
        "status": "R5_PARENT_BASELINE_FRESH_UNOPENED",
    ...
    if selected is None:
        payload = {
            "schema_version": 1,
            "status": "R5_DEV_NO_ELIGIBLE_CANDIDATE_FRESH_UNOPENED",
    ...
    manifest = {
        **metadata,
        "schema_version": 1,
        "status": "R5_DEV_ELIGIBLE_FRESH_UNOPENED",
        "selected_candidate": selected["name"],
        "selected_rank": list(selected["rank"]),
        "selected_eligibility": selected["eligibility"],
        "parent_development": _compact(parent_dev),
