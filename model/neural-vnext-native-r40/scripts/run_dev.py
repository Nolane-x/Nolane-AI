...
from rescue_aligned_proposal_core import NativeR40RescueAlignedProposalEnsemble
...
def main() -> int:
    ...
    if lock.get("candidate") != "Neural-vNext-Native-R40-RescueAlignedProposal":
        raise ValueError("unexpected R40 candidate")
    if (
        lock["fresh_isolation"]["status"] != "UNOPENED"
        or lock["benchmark"]["fresh_indices"] != [280, 319]
    ):
        raise ValueError("R40 fresh isolation changed")
    ...
    print(
        json.dumps(
            {
                "status": "R40_R11_BASELINE_FRESH_UNOPENED",
    ...
    if model.goal_parameter_count() != int(architecture["expected_goal_successor_parameters"]):
        raise AssertionError("R40 goal parameter count changed")
    if model.set_parameter_count() != int(architecture["expected_set_successor_parameters"]):
        raise AssertionError("R40 set parameter count changed")
    if model.parameter_count() != int(architecture["expected_successor_parameters"]):
        raise AssertionError("R40 total parameter count changed")
    ...
    status = (
        "R40_DEV_ELIGIBLE_CONFIRMATION_UNOPENED_FRESH_UNOPENED"
        if eligibility["eligible"]
        else "R40_DEV_REJECTED_CONFIRMATION_FRESH_UNOPENED"
    )
    selected_config = {
        "temperature": temperature,
    ...
    manifest = {
        **metadata,
        ...
        "private_boundary": {
            ...
            "rescue_aligned_joint_proposal": True,
        ...
        "selected_candidate": "rescue_aligned_joint_proposal_ranker",
