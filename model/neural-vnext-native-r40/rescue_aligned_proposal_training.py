...
R11_HORIZON = 1
CHECKPOINT_FORMAT = "nolane-neural-vnext-native-r40-rescue-aligned-proposal-v1"
...
def _training_stratum(
    ...
    *,
    ...
    if not group:
        raise ValueError("empty R40 decision group")
...
def train_decision_set_ensemble(
    ...
    *,
    ...
    if int(stratum_count) < 2:
        raise ValueError("R40 stratum_count must be >=2")
...
def _score_decision_set(
    ...
    candidate_selection = selection_probs[:, 1:]
    joint_proposal = candidate_selection * rescue_probs
    mean_joint = joint_proposal.mean(dim=0)
    candidate_index = int(mean_joint.argmax().item())
    selected_index = candidate_index + 1

    mean_selection = selection_probs.mean(dim=0)
    per_head_joint_choice = joint_proposal.argmax(dim=-1)
    per_head_selection_choice = selection_probs.argmax(dim=-1)
    ...
    return {
        ...
        "min_joint_proposal": float(joint_proposal[:, candidate_index].min().item()),
        "mean_joint_proposal": float(mean_joint[candidate_index].item()),
        ...
        "joint_proposal_heads": int(
            (per_head_joint_choice == candidate_index).sum().item()
        ),
        "selection_preferred_heads": int(
            (per_head_selection_choice == selected_index).sum().item()
        ),
    }
...
def _select_action_row(
    ...
    *,
    ...
    if (
        ...
        or int(score["joint_proposal_heads"]) < model.ensemble_size
    ):
        return None, score
...
def evaluate_r40(
    ...
    *,
    ...
    if split not in {"dev", "fresh"}:
        raise ValueError("R40 evaluation split must be dev/fresh")
    rows: list[dict[str, Any]] = []
