...
def encode_public_goal_features(
    *,
    ...
    if features.shape != (PUBLIC_GOAL_FEATURE_DIM,):
        raise AssertionError("unexpected R40 public goal feature shape")
    return features
...
def encode_parent_latent_context(parent_output: Mapping[str, Tensor]) -> Tensor:
    ...
    if latent.shape != (LATENT_CONTEXT_DIM,):
        raise AssertionError("unexpected R40 parent latent context shape")
...
def encode_pair_features(
    *,
    ...
    if features.shape != (PAIR_FEATURE_DIM,):
        raise AssertionError("unexpected R40 pair feature shape")
    return features
...
class _DecisionSetHead(nn.Module):
    ...
    def forward(self, features: Tensor) -> dict[str, Tensor]:
        if features.ndim != 2 or features.shape[1] != PAIR_FEATURE_DIM:
            raise ValueError("R40 set features must have shape [N,774]")
        if features.shape[0] < 1:
            raise ValueError("R40 decision set cannot be empty")
...
class NativeR40RescueAlignedProposalEnsemble(nn.Module):
    """Set-aware rescue ranker with rescue-aligned candidate proposal consensus."""
    ...
    def architecture(self) -> dict[str, Any]:
        return {
            "r40_ensemble_size": self.ensemble_size,
            "r40_goal_hidden_dim": self.goal_hidden_dim,
            "r40_set_hidden_dim": self.set_hidden_dim,
            "r40_pair_feature_dim": PAIR_FEATURE_DIM,
            "r40_joint_outcome_class_count": JOINT_OUTCOME_CLASS_COUNT,
            "r40_has_keep_r11_null_option": True,
            "r40_has_dedicated_binary_rescue_head": True,
            "mechanism": "permutation_equivariant_set_ranker_with_binary_rescue_aligned_joint_proposal",
        }
