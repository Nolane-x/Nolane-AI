...
CANDIDATE_IDENTITY = "Neural-vNext-Native-R3-AttributedBelief"
LOCKED_FAMILIES = (
    "conditional_regimes",
    "regime_switch",
...
def load_lock(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(lock, dict) or lock.get("schema_version") != 1:
        raise ValueError("unsupported R3 PRE_FRESH_LOCK schema")
    if lock.get("status") != "FROZEN_FRESH_UNOPENED":
        raise ValueError("R3 fresh court requires FROZEN_FRESH_UNOPENED")
    if lock.get("candidate") != CANDIDATE_IDENTITY:
        raise ValueError("unexpected R3 fresh candidate identity")

    court = lock.get("fresh_court")
    if not isinstance(court, dict) or court.get("opened") is not False:
        raise ValueError("R3 fresh court lock must attest opened=false")
    if tuple(court.get("families", ())) != LOCKED_FAMILIES:
        raise ValueError("R3 fresh families differ from preregistration")
    if court.get("index_start") != FRESH_START or court.get("index_end") != FRESH_END:
        raise ValueError("R3 fresh indices differ from preregistration")
    if court.get("episodes_expected") != FRESH_EPISODES:
        raise ValueError("R3 fresh episode count differs from preregistration")
    if lock.get("promotion_gate") != LOCKED_GATE:
        raise ValueError("R3 promotion gate differs from preregistration")

    frozen = lock.get("frozen_candidate")
    if not isinstance(frozen, dict) or frozen.get("fresh_opened") is not False:
        raise ValueError("R3 frozen candidate must attest fresh_opened=false")
    authority = lock.get("candidate_authority")
    if not isinstance(authority, dict):
        raise ValueError("R3 candidate authority missing")
    if authority.get("mode") != "frozen_workflow_artifact" or authority.get("status") != "VERIFIED":
        raise ValueError("R3 requires verified frozen-workflow-artifact authority")
    ...
    if authority.get("checkpoint_sha256") != frozen.get("checkpoint_sha256"):
        raise ValueError("R3 authority checkpoint differs from frozen candidate")
    if authority.get("state_dict_sha256") != frozen.get("state_dict_sha256"):
        raise ValueError("R3 authority state differs from frozen candidate")
    if authority.get("selected_candidate") != frozen.get("selected_candidate"):
        raise ValueError("R3 authority selected candidate differs from frozen candidate")

    reproduction = lock.get("workflow_reproduction")
    if not isinstance(reproduction, dict) or reproduction.get("status") != "BITWISE_REPRODUCED_WORKFLOW_LEVEL":
        raise ValueError("R3 workflow-level reproduction disclosure missing")
    runs = reproduction.get("workflow_run_ids")
    if not isinstance(runs, list) or len(runs) < 2:
        raise ValueError("R3 reproduction requires at least two workflow attempts")
    if reproduction.get("cross_host_guarantee_claimed") is not False:
        raise ValueError("R3 must not claim cross-host guarantee")

    negative = lock.get("phase2_negative_result")
    if not isinstance(negative, dict) or negative.get("status") != "FROZEN":
        raise ValueError("R3 Phase-2 negative result must remain frozen")
    if negative.get("fresh_opened") is not False:
        raise ValueError("R3 Phase-2 negative result must attest fresh unopened")

    secondary = lock.get("secondary_development")
    if not isinstance(secondary, dict) or secondary.get("supports_freeze") is not True:
        raise ValueError("R3 secondary development evidence must support freeze")
    if secondary.get("claim_boundary") != "POST_HOC_DEVELOPMENT_NOT_PROMOTION_COURT":
        raise ValueError("R3 secondary evidence boundary must remain explicit")
    return lock
...
def verify_source_blobs(lock: Mapping[str, Any], repo_root: Path) -> None:
    ...
    if not isinstance(rows, dict) or not rows:
        raise ValueError("R3 PRE_FRESH_LOCK is missing frozen source blobs")
...
def verify_evidence_files(lock: Mapping[str, Any], repo_root: Path) -> None:
    secondary = json.loads(
        (repo_root / "model/neural-vnext-native-r3/evidence/PHASE1_SECONDARY_DEV_001.json").read_text()
    )
    negative = json.loads(
        (repo_root / "model/neural-vnext-native-r3/evidence/PHASE2_NEGATIVE_001.json").read_text()
    )
    expected_secondary = lock["secondary_development"]
    if secondary.get("artifact", {}).get("id") != expected_secondary["artifact_id"]:
        raise ValueError("R3 secondary dev artifact authority mismatch")
    if secondary.get("candidate", {}).get("solved") != expected_secondary["candidate_solved"]:
        raise ValueError("R3 secondary candidate solved mismatch")
    if secondary.get("parent", {}).get("solved") != expected_secondary["parent_solved"]:
        raise ValueError("R3 secondary parent solved mismatch")
    if secondary.get("family_solved_delta_vs_parent") != expected_secondary["family_solved_delta_vs_parent"]:
        raise ValueError("R3 secondary family delta mismatch")
    if secondary.get("supports_freeze") is not True:
        raise ValueError("R3 secondary evidence no longer supports freeze")
    if negative.get("artifact", {}).get("id") != lock["phase2_negative_result"]["artifact_id"]:
        raise ValueError("R3 Phase-2 negative artifact authority mismatch")
    if negative.get("status") != "R3_PHASE2_NEGATIVE_RESULT_FROZEN":
        raise ValueError("R3 Phase-2 negative evidence status changed")
...
def verify_checkpoint(
    ...
    if actual != expected:
        raise ValueError(f"R3 frozen checkpoint authority mismatch: expected {expected}, got {actual}")

    summary = metadata.get("training_summary")
    if not isinstance(summary, dict):
        raise ValueError("R3 checkpoint training summary missing")
    if summary.get("selected_candidate") != frozen["selected_candidate"]:
        raise ValueError("R3 selected candidate mismatch")
    if summary.get("selected_rank") != frozen["selected_rank"]:
        raise ValueError("R3 selected rank mismatch")
    if summary.get("selected_eligibility") != frozen["selected_eligibility"]:
        raise ValueError("R3 selected eligibility mismatch")
    if summary.get("parent_development") != frozen["parent_development"]:
        raise ValueError("R3 frozen parent dev evidence mismatch")
    tournament = summary.get("candidate_tournament")
    if not isinstance(tournament, list):
        raise ValueError("R3 candidate tournament missing")
    selected = [
        row for row in tournament
        if isinstance(row, dict) and row.get("name") == frozen["selected_candidate"]
    ]
    if len(selected) != 1:
        raise ValueError("R3 selected candidate must be unique in tournament")
    if selected[0].get("development") != frozen["development"]:
        raise ValueError("R3 frozen development evidence mismatch")
    if selected[0].get("eligibility") != frozen["selected_eligibility"]:
        raise ValueError("R3 frozen eligibility evidence mismatch")
    if summary.get("fresh_opened") is not False:
        raise ValueError("R3 checkpoint must attest fresh unopened")
...
def expected_identities(lock: Mapping[str, Any]) -> set[tuple[str, int]]:
    ...
    if not isinstance(families, list) or tuple(families) != LOCKED_FAMILIES:
        raise ValueError("R3 fresh families invalid")
    expected = {
        (str(family), index)
        for family in families
        for index in range(start, end + 1)
    }
    if len(expected) != _exact_int(court.get("episodes_expected"), field="fresh episodes"):
        raise ValueError("R3 fresh Cartesian cardinality mismatch")
    return expected
...
def promotion_gate(
    ...
    *,
    ...
    if not isinstance(parent_families, Mapping) or not isinstance(candidate_families, Mapping):
    ...
    candidate_total = _exact_int(candidate.get("solved"), field="candidate solved")
    total_delta = candidate_total - parent_total
    min_total = _exact_int(gate["minimum_total_solved_delta_vs_parent"], field="minimum total delta")
    target = str(gate["target_family"])
    min_target = _exact_int(gate["minimum_target_family_solved_delta"], field="minimum target delta")
    required_visible_delta = _exact_int(
        gate["required_visible_family_solved_delta"], field="required visible delta"
    )

    family_delta: dict[str, int] = {}
    for family in sorted(parent_families):
        p = _exact_int(parent_families[family].get("solved"), field=f"parent {family} solved")
        c = _exact_int(candidate_families[family].get("solved"), field=f"candidate {family} solved")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Open the frozen R3 fresh court exactly once against the exact embedded R2 parent."
    )
    ...
    verify_evidence_files(lock, REPO_ROOT)
    model, metadata = verify_checkpoint(lock, args.checkpoint)

    court = lock["fresh_court"]
    families = tuple(str(v) for v in court["families"])
    indices = (
