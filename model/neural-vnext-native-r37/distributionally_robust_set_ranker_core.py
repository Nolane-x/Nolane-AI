...
def encode_public_goal_features(
    *,
    ...
    if features.shape != (PUBLIC_GOAL_FEATURE_DIM,):
        raise AssertionError("unexpected R37 public goal feature shape")
    return features
...
def encode_parent_latent_context(parent_output: Mapping[str, Tensor]) -> Tensor:
    ...
    if latent.shape != (LATENT_CONTEXT_DIM,):
        raise AssertionError("unexpected R37 parent latent context shape")
...
def encode_pair_features(
    *,
    ...
    if features.shape != (PAIR_FEATURE_DIM,):
        raise AssertionError("unexpected R37 pair feature shape")
    return features
...
class _DecisionSetHead(nn.Module):
    ...
    def forward(self, features: Tensor) -> dict[str, Tensor]:
        if features.ndim != 2 or features.shape[1] != PAIR_FEATURE_DIM:
            raise ValueError("R37 set features must have shape [N,774]")
        if features.shape[0] < 1:
            raise ValueError("R37 decision set cannot be empty")
...
class NativeR37DistributionallyRobustSetRankerEnsemble(nn.Module):
    """Set-aware rescue selector trained on whole decision groups rather than independent rows."""
    ...
    def architecture(self) -> dict[str, Any]:
        return {
            "r36_ensemble_size": self.ensemble_size,
            "r36_goal_hidden_dim": self.goal_hidden_dim,
            "r36_set_hidden_dim": self.set_hidden_dim,
            "r36_pair_feature_dim": PAIR_FEATURE_DIM,
            "r36_joint_outcome_class_count": JOINT_OUTCOME_CLASS_COUNT,
            "r36_has_keep_r11_null_option": True,
            "mechanism": "permutation_equivariant_decision_set_rescue_ranker_with_null_parent_option",
        }
