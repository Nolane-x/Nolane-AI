...
from distributionally_robust_set_ranker_core import (
    BOTH_FAIL_CLASS,
    BOTH_SOLVE_CLASS,
    HARM_CLASS,
    RESCUE_CLASS,
    NativeR37DistributionallyRobustSetRankerEnsemble,
...
R11_HORIZON = 1
CHECKPOINT_FORMAT = "nolane-neural-vnext-native-r37-distributionally-robust-set-ranker-v1"
...
def _training_stratum(
    ...
    *,
    ...
    if not group:
        raise ValueError("empty R37 decision group")
...
def train_decision_set_ensemble(
    ...
    *,
    ...
    if int(stratum_count) < 2:
        raise ValueError("R37 stratum_count must be >=2")
...
def evaluate_r37(
    ...
    *,
    ...
    if split not in {"dev", "fresh"}:
        raise ValueError("R37 evaluation split must be dev/fresh")
    rows: list[dict[str, Any]] = []
