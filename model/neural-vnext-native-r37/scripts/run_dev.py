...
from distributionally_robust_set_ranker_core import NativeR37DistributionallyRobustSetRankerEnsemble
from distributionally_robust_set_ranker_training import (
...
def main() -> int:
    ...
    if lock.get("candidate") != "Neural-vNext-Native-R37-DistributionallyRobustSetRanker":
        raise ValueError("unexpected R37 candidate")
    if (
        lock["fresh_isolation"]["status"] != "UNOPENED"
        or lock["benchmark"]["fresh_indices"] != [280, 319]
    ):
        raise ValueError("R37 fresh isolation changed")
    ...
    print(
        json.dumps(
            {
                "status": "R37_R11_BASELINE_FRESH_UNOPENED",
    ...
    if model.goal_parameter_count() != int(architecture["expected_goal_successor_parameters"]):
        raise AssertionError("R37 goal parameter count changed")
    if model.set_parameter_count() != int(architecture["expected_set_successor_parameters"]):
        raise AssertionError("R37 set parameter count changed")
    if model.parameter_count() != int(architecture["expected_successor_parameters"]):
        raise AssertionError("R37 total parameter count changed")
    ...
    status = (
        ...
        else "R37_DEV_REJECTED_CONFIRMATION_FRESH_UNOPENED"
    )
    selected_config = {
        "temperature": temperature,
    ...
    manifest = {
        **metadata,
        ...
        "selected_candidate": "distributionally_robust_decision_set_ranker",
