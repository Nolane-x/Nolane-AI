...
def test_r6_initialization_is_exactly_r4_equivalent() -> None:
    parent = _r4_parent()
...
def test_visible_target_hard_gate_blocks_r6_residual() -> None:
    parent = _r4_parent()
    model = NativeR6CalibratedConsistencyPolicy(
        copy.deepcopy(parent),
        consistency_embedding_dim=16,
...
def test_parent_frozen_and_only_r6_parameters_trainable() -> None:
    model = NativeR6CalibratedConsistencyPolicy(
        _r4_parent(),
        consistency_embedding_dim=16,
        residual_hidden_dim=24,
...
def test_r6_checkpoint_roundtrip_binds_frozen_r4_parent(
    tmp_path: Path,
) -> None:
    model = NativeR6CalibratedConsistencyPolicy(
        _r4_parent(),
    ...
    restored, observed = load_r6_checkpoint(path)
...
def test_uniform_public_posterior_calibration_gate_falls_back_to_r4() -> None:
    ...
    assert torch.equal(output["action_logits"], expected)
    assert torch.equal(
        output["calibration_gate"],
        torch.zeros_like(output["calibration_gate"]),
    )
...
def test_calibration_gate_is_bounded() -> None:
    ...
    with torch.no_grad():
        output = model.forward_step(*inputs, posterior)
    assert bool((output["calibration_gate"] >= 0.0).all())
    assert bool((output["calibration_gate"] <= 1.0).all())
