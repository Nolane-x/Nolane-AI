...
class NativeR6CalibratedConsistencyPolicy(nn.Module):
    """Frozen accepted R4 plus a confidence-calibrated public-consistency residual."""
    ...
    def __init__(
        ...
        *,
        ...
        if float(calibration_power) <= 0.0:
            raise ValueError("calibration_power must be positive")
        self.parent = parent
    ...
    def train(self, mode: bool = True) -> "NativeR6CalibratedConsistencyPolicy":
        super().train(mode)
        self.parent.eval()
        return self
    ...
    def architecture(self) -> dict[str, int]:
        return {
            **self.parent.architecture(),
            ...
            "calibration_power": self.calibration_power,
    ...
    def forward_step(
        ...
        with torch.no_grad():
            ...
            r4_goal_probabilities = parent_output[
                "goal_probabilities"
            ].detach()
            action_tokens = (
        ...
        consistency_marginals_3d = consistency_marginals.view(
            posterior.shape[0],
            GOAL_DIMENSIONS,
            GOAL_CARDINALITY,
        )
        agreement = (
            ...
            - 0.5
            * (
                consistency_marginals_3d
                - r4_goal_probabilities
            )
            .abs()
            .sum(dim=-1)
            .mean(dim=-1, keepdim=True)
        ...
        residual = residual * calibration_gate
        ...
        return {
            ...
            "consistency_marginals": consistency_marginals_3d,
            "consistency_entropy": entropy.squeeze(-1),
            "consistency_support_fraction":
                support_fraction.squeeze(-1),
            "consistency_agreement": agreement.squeeze(-1),
            "calibration_gate": calibration_gate.squeeze(-1),
