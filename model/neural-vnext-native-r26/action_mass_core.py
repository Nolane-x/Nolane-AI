...
def encode_public_goal_features(
    *,
    ...
    if features.shape != (PUBLIC_GOAL_FEATURE_DIM,):
        raise AssertionError("unexpected R26 public goal feature shape")
    return features
...
class NativeR26GoalBeliefEnsemble(nn.Module):
    ...
    def architecture(self) -> dict[str, Any]:
        return {
            "r26_ensemble_size": self.ensemble_size,
            "r26_hidden_dim": self.hidden_dim,
            "r26_public_goal_feature_dim": PUBLIC_GOAL_FEATURE_DIM,
            "r26_goal_hypothesis_count": GOAL_HYPOTHESIS_COUNT,
    ...
    def per_head_probabilities(
        ...
        *,
        ...
        if features.ndim != 1 or features.shape[0] != PUBLIC_GOAL_FEATURE_DIM:
            raise ValueError("R26 expects one public feature row")
        if support_mask.shape != (GOAL_HYPOTHESIS_COUNT,):
