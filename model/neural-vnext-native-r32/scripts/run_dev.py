...
from factorized_terminal_value_core import NativeR32FactorizedTerminalValueEnsemble
from factorized_terminal_value_training import (
    calibrate_temperature,
    certification_counts,
    collect_goal_dataset,
    collect_terminal_value_pairs,
    evaluate_r32,
    fit_rescue_guard,
    pair_outcome_counts,
    save_checkpoint,
...
def main() -> int:
    ...
    if lock.get("candidate") != "Neural-vNext-Native-R32-FactorizedTerminalValue":
        raise ValueError("unexpected R32 candidate")
    if (
        lock["fresh_isolation"]["status"] != "UNOPENED"
        or lock["benchmark"]["fresh_indices"] != [280, 319]
    ):
        raise ValueError("R32 fresh isolation changed")
    ...
    print(
        json.dumps(
            {
                "status": "R32_R11_BASELINE_FRESH_UNOPENED",
    ...
    model = NativeR32FactorizedTerminalValueEnsemble(
        ...
        value_hidden_dim=int(architecture["value_hidden_dim"]),
    )
    if model.goal_parameter_count() != int(architecture["expected_goal_successor_parameters"]):
        raise AssertionError("R32 goal parameter count changed")
    if model.value_parameter_count() != int(architecture["expected_value_successor_parameters"]):
        raise AssertionError("R32 value parameter count changed")
    if model.parameter_count() != int(architecture["expected_successor_parameters"]):
        raise AssertionError("R32 total parameter count changed")
    ...
    value_train_rows = collect_terminal_value_pairs(
        parent,
        model,
        make_task=make_r18_task,
        indices=tuple(benchmark["value_training_indices"]),
    ...
    value_training_summary = train_value_ensemble(
        model,
        value_train_rows,
        seed=int(training["seed"]),
        epochs=int(training["value_epochs"]),
        batch_size=int(training["batch_size"]),
        learning_rate=float(training["learning_rate"]),
        weight_decay=float(training["weight_decay"]),
    )
    ...
    metadata = save_checkpoint(
        ...
        value_training_summary=value_training_summary,
        rescue_guard=rescue_guard,
    )

    status = (
        "R32_DEV_ELIGIBLE_CONFIRMATION_UNOPENED_FRESH_UNOPENED"
        if eligibility["eligible"]
        else "R32_DEV_REJECTED_CONFIRMATION_FRESH_UNOPENED"
    )
    selected_config = {
        "temperature": temperature,
    ...
    manifest = {
        **metadata,
        ...
        "private_boundary": {
            ...
            "terminal_value_labels_use_paired_terminal_outcomes_only": True,
            "all_valid_alternative_actions_labeled": True,
            "r11_terminal_value_labeled_once_per_decision": True,
            "inference_private_goal_use": False,
        },
        "value_training_pair_rows": len(value_train_rows),
        "value_training_pair_outcomes": pair_outcome_counts(value_train_rows),
        "value_training_certification": certification_counts(value_train_rows),
        "guard_block_row_counts": [len(rows) for rows in guard_block_rows],
        "guard_block_pair_outcomes": [
            pair_outcome_counts(rows) for rows in guard_block_rows
        ],
        "parent_development": _compact(parent_dev),
        "dev_evaluation": _compact(candidate),
        "selected_candidate": "factorized_terminal_action_value",
