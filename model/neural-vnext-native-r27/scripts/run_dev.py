...
from action_mass_core import NativeR27GoalBeliefEnsemble
from action_mass_training import (
    calibrate_temperature,
    collect_goal_dataset,
    evaluate_r27,
    fit_robust_action_mass_guard,
    save_checkpoint,
    train_goal_ensemble,
)
...
def main() -> int:
    ...
    if (
        ...
        raise ValueError("R27 fresh isolation changed")
    ...
    print(
        json.dumps(
            {
                "status": "R27_R11_BASELINE_FRESH_UNOPENED",
    ...
    if model.parameter_count() != int(architecture["expected_successor_parameters"]):
        raise AssertionError("R27 parameter count changed after preregistration")
    ...
    action_guard = fit_robust_action_mass_guard(
        parent,
        model,
        make_task=make_r18_task,
        guard_blocks=[tuple(block) for block in benchmark["guard_blocks"]],
    ...
    status = (
        "R27_DEV_ELIGIBLE_CONFIRMATION_UNOPENED_FRESH_UNOPENED"
        if eligibility["eligible"]
        else "R27_DEV_REJECTED_CONFIRMATION_FRESH_UNOPENED"
    )
    selected_config = {
        "temperature": temperature,
