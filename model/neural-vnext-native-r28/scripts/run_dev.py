...
from counterfactual_rescue_core import NativeR28CounterfactualRescueEnsemble
from counterfactual_rescue_training import (
...
def main() -> int:
    ...
    if lock.get("candidate") != "Neural-vNext-Native-R28-CounterfactualRescueValue":
        raise ValueError("unexpected R28 candidate")
    if (
        lock["fresh_isolation"]["status"] != "UNOPENED"
        or lock["benchmark"]["fresh_indices"] != [280, 319]
    ):
        raise ValueError("R28 fresh isolation changed")
    ...
    print(
        json.dumps(
            {
                "status": "R28_R11_BASELINE_FRESH_UNOPENED",
    ...
    if model.goal_parameter_count() != int(architecture["expected_goal_successor_parameters"]):
        raise AssertionError("R28 goal parameter count changed")
    if model.rescue_parameter_count() != int(architecture["expected_rescue_successor_parameters"]):
        raise AssertionError("R28 rescue parameter count changed")
    if model.parameter_count() != int(architecture["expected_successor_parameters"]):
        raise AssertionError("R28 total parameter count changed")
    ...
    rescue_train_rows = collect_rescue_rows(
        parent,
        model,
        make_task=make_r18_task,
        indices=tuple(benchmark["rescue_training_indices"]),
        temperature=temperature,
        generator_mass_threshold=float(training["generator_mass_threshold"]),
        max_support=int(training["max_support"]),
        max_rows_per_episode=int(training["max_counterfactual_rows_per_episode"]),
    )
    ...
    rescue_guard = fit_rescue_guard(
        ...
        thresholds=[float(value) for value in training["rescue_threshold_candidates"]],
        minimum_precision=float(training["minimum_rescue_precision"]),
        minimum_total_rows=int(training["minimum_total_predicted_rows"]),
        minimum_block_rows=int(training["minimum_block_predicted_rows"]),
        require_zero_harm=bool(training["require_zero_predicted_harm"]),
    )
    rescue_threshold = (
        None
    ...
    status = (
        "R28_DEV_ELIGIBLE_CONFIRMATION_UNOPENED_FRESH_UNOPENED"
        if eligibility["eligible"]
        else "R28_DEV_REJECTED_CONFIRMATION_FRESH_UNOPENED"
    )
    selected_config = {
        "temperature": temperature,
        "generator_mass_threshold": float(training["generator_mass_threshold"]),
    ...
    manifest = {
        **metadata,
        ...
        "rescue_training_positive_rows": sum(int(row["rescue"]) for row in rescue_train_rows),
        "rescue_training_harm_rows": sum(int(row["harm"]) for row in rescue_train_rows),
        ...
        "selected_candidate": "counterfactual_rescue_value",
