...
def main() -> int:
    ...
    if lock.get("candidate") != "Neural-vNext-Native-R29-CertifiedRescueGeometry":
        raise ValueError("unexpected R29 candidate")
    if (
        lock["fresh_isolation"]["status"] != "UNOPENED"
        or lock["benchmark"]["fresh_indices"] != [280, 319]
    ):
        raise ValueError("R29 fresh isolation changed")
    ...
    print(
        json.dumps(
            {
                "status": "R29_R11_BASELINE_FRESH_UNOPENED",
    ...
    if model.goal_parameter_count() != int(architecture["expected_goal_successor_parameters"]):
        raise AssertionError("R29 goal parameter count changed")
    if model.rescue_parameter_count() != int(architecture["expected_rescue_successor_parameters"]):
        raise AssertionError("R29 rescue parameter count changed")
    if model.parameter_count() != int(architecture["expected_successor_parameters"]):
        raise AssertionError("R29 total parameter count changed")
    ...
    status = (
        "R29_DEV_ELIGIBLE_CONFIRMATION_UNOPENED_FRESH_UNOPENED"
        if eligibility["eligible"]
        else "R29_DEV_REJECTED_CONFIRMATION_FRESH_UNOPENED"
    )
    selected_config = {
        "temperature": temperature,
        "generator_mass_threshold": float(training["generator_mass_threshold"]),
    ...
    manifest = {
        **metadata,
        ...
        "rescue_training_class_counts": {
            "neutral": sum(int(row["class_id"] == 0) for row in rescue_train_rows),
            "rescue": sum(int(row["class_id"] == 1) for row in rescue_train_rows),
            "harm": sum(int(row["class_id"] == 2) for row in rescue_train_rows),
        },
        ...
        "selected_candidate": "certified_rescue_geometry",
