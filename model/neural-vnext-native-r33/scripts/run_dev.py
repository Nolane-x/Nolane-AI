...
from joint_terminal_outcome_core import NativeR33JointTerminalOutcomeEnsemble
from joint_terminal_outcome_training import (
...
def main() -> int:
    ...
    if lock.get("candidate") != "Neural-vNext-Native-R33-JointTerminalOutcome":
        raise ValueError("unexpected R33 candidate")
    if (
        lock["fresh_isolation"]["status"] != "UNOPENED"
        or lock["benchmark"]["fresh_indices"] != [280, 319]
    ):
        raise ValueError("R33 fresh isolation changed")
    ...
    print(
        json.dumps(
            {
                "status": "R33_R11_BASELINE_FRESH_UNOPENED",
    ...
    model = NativeR33JointTerminalOutcomeEnsemble(
        ...
        outcome_hidden_dim=int(architecture["outcome_hidden_dim"]),
    )
    if model.goal_parameter_count() != int(architecture["expected_goal_successor_parameters"]):
        raise AssertionError("R33 goal parameter count changed")
    if model.outcome_parameter_count() != int(architecture["expected_outcome_successor_parameters"]):
        raise AssertionError("R33 outcome parameter count changed")
    if model.parameter_count() != int(architecture["expected_successor_parameters"]):
        raise AssertionError("R33 total parameter count changed")
    ...
    joint_train_rows = collect_joint_outcome_rows(
        ...
        model,
        make_task=make_r18_task,
        indices=tuple(benchmark["joint_training_indices"]),
    ...
    outcome_training_summary = train_joint_outcome_ensemble(
        model,
        joint_train_rows,
        seed=int(training["seed"]),
    ...
    metadata = save_checkpoint(
        ...
        outcome_training_summary=outcome_training_summary,
        rescue_guard=rescue_guard,
    )

    status = (
        "R33_DEV_ELIGIBLE_CONFIRMATION_UNOPENED_FRESH_UNOPENED"
        if eligibility["eligible"]
        else "R33_DEV_REJECTED_CONFIRMATION_FRESH_UNOPENED"
    )
    selected_config = {
        "temperature": temperature,
    ...
    manifest = {
        **metadata,
        ...
        "joint_training_rows": len(joint_train_rows),
        "joint_training_class_counts": _class_counts(joint_train_rows),
        "joint_training_certification": _certification_stats(joint_train_rows),
        ...
        "selected_candidate": "four_way_joint_terminal_outcome",
