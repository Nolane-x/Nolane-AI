...
R11_HORIZON = 1
CHECKPOINT_FORMAT = "nolane-neural-vnext-native-r33-joint-terminal-outcome-v1"
...
def _candidate_feature_rows(
    ...
    *,
    ...
    for candidate_action in range(int(action_features.shape[0])):
        ...
        features = encode_latent_action_rescue_features(
...
def train_joint_outcome_ensemble(
    model: NativeR33JointTerminalOutcomeEnsemble,
    ...
    *,
    ...
    class_counts = [
        int((labels == class_id).sum().item()) for class_id in range(4)
    ]
    total = len(rows)
    class_weights = torch.tensor(
        [
            (total / (4.0 * count)) if count > 0 else 0.0
            for count in class_counts
        ],
        dtype=torch.float32,
    )
    summaries: list[dict[str, Any]] = []
    for head_index, head in enumerate(model.outcome_heads):
...
def _score_action_row(
    ...
    probs = model.joint_outcome_probabilities(row["features"])
    return {
...
def evaluate_r33(
    ...
    *,
    ...
    if split not in {"dev", "fresh"}:
        raise ValueError("R33 evaluation split must be dev/fresh")
    rows: list[dict[str, Any]] = []
...
def save_checkpoint(
    ...
    *,
    ...
    outcome_training_summary: Mapping[str, Any],
    ...
    payload = {
        ...
        "outcome_successor_parameters": model.outcome_parameter_count(),
        ...
        "outcome_training_summary": dict(outcome_training_summary),
