...
from binary_rescue_set_ranker_core import NativeR39BinaryRescueSetRankerEnsemble
from binary_rescue_set_ranker_training import (
    calibrate_temperature,
    collect_joint_outcome_rows,
    collect_goal_dataset,
    evaluate_r39,
    fit_rescue_guard,
    save_checkpoint,
    train_goal_ensemble,
    train_decision_set_ensemble,
)
...
def main() -> int:
    ...
    if lock.get("candidate") != "Neural-vNext-Native-R39-BinaryRescueConfidence":
        raise ValueError("unexpected R39 candidate")
    if (
        lock["fresh_isolation"]["status"] != "UNOPENED"
        or lock["benchmark"]["fresh_indices"] != [280, 319]
    ):
        raise ValueError("R39 fresh isolation changed")
    ...
    print(
        json.dumps(
            {
                "status": "R39_R11_BASELINE_FRESH_UNOPENED",
    ...
    model = NativeR39BinaryRescueSetRankerEnsemble(
        ...
        set_hidden_dim=int(architecture["set_hidden_dim"]),
    )
    if model.goal_parameter_count() != int(architecture["expected_goal_successor_parameters"]):
        raise AssertionError("R39 goal parameter count changed")
    if model.set_parameter_count() != int(architecture["expected_set_successor_parameters"]):
        raise AssertionError("R39 set parameter count changed")
    if model.parameter_count() != int(architecture["expected_successor_parameters"]):
        raise AssertionError("R39 total parameter count changed")
    ...
    joint_train_rows = collect_joint_outcome_rows(
        parent,
        model,
        make_task=make_r18_task,
        indices=tuple(benchmark["set_training_indices"]),
    ...
    set_training_summary = train_decision_set_ensemble(
        model,
        joint_train_rows,
        seed=int(training["seed"]),
        epochs=int(training["set_epochs"]),
        batch_size=int(training["batch_size"]),
        learning_rate=float(training["learning_rate"]),
        weight_decay=float(training["weight_decay"]),
        listwise_loss_weight=float(training["listwise_loss_weight"]),
        harm_mass_weight=float(training["harm_mass_weight"]),
        stratum_count=int(training["stratum_count"]),
        group_dro_eta=float(training["group_dro_eta"]),
        rescue_group_listwise_multiplier=float(
            training["rescue_group_listwise_multiplier"]
        ),
        rescue_binary_loss_weight=float(training["rescue_binary_loss_weight"]),
    )
    ...
    rescue_guard = fit_rescue_guard(
        ...
        selection_thresholds=[
            float(value) for value in training["selection_threshold_candidates"]
        ],
        rescue_thresholds=[
    ...
    selection_threshold = (
        ...
        else float(rescue_guard["selected"]["selection_threshold"])
    )
    rescue_threshold = (
        None
        if not rescue_guard["enabled"]
    ...
    metadata = save_checkpoint(
        ...
        set_training_summary=set_training_summary,
        rescue_guard=rescue_guard,
    )

    status = (
        "R39_DEV_ELIGIBLE_CONFIRMATION_UNOPENED_FRESH_UNOPENED"
        if eligibility["eligible"]
        else "R39_DEV_REJECTED_CONFIRMATION_FRESH_UNOPENED"
    )
    selected_config = {
        "temperature": temperature,
    ...
    manifest = {
        **metadata,
        ...
        "private_boundary": {
            ...
            "decision_set_listwise_labels_use_terminal_outcomes_only": True,
            "training_identity_strata_not_used_at_inference": True,
            "runtime_uses_full_alternative_set": True,
            "dedicated_binary_rescue_probability_gate": True,
            "all_alternative_actions_labeled": True,
            "inference_private_goal_use": False,
        },
        "set_training_rows": len(joint_train_rows),
        "set_training_class_counts": _class_counts(joint_train_rows),
        "set_training_certification": _certification_stats(joint_train_rows),
        ...
        "selected_candidate": "binary_rescue_confidence_decision_set_ranker",
