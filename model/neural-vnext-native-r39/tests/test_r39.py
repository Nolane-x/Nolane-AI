...
def test_parameter_count_includes_binary_rescue_head() -> None:
...
def test_round_robin_order_interleaves_strata_deterministically() -> None:
    groups = [_group(index) for index in range(23424, 23424 + 8 * 2)]
...
class _FakeSetModel:
    ...
    def decision_set_outputs(self, features: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        outcome[:, 0, :] = torch.tensor([0.60, 0.10, 0.02, 0.28])
...
def test_binary_rescue_selector_accepts_dedicated_confidence() -> None:
    rows = [
