...
from threshold_core import (
    ...
    r7_state_dict_sha256,
)

CHECKPOINT_FORMAT = (
    "nolane-neural-vnext-native-r7-threshold-consistency-v1"
)
...
def load_lock(path: str | Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("unsupported R7 PREDEV lock")
    if (
        payload.get("candidate")
        != "Neural-vNext-Native-R7-ThresholdConsistency"
    ):
        raise ValueError("unexpected R7 candidate identity")
    ...
    if (
        ...
        raise ValueError(
            "R7 PREDEV lock must keep fresh unopened"
        )
    ...
    if (
        ...
        raise ValueError(
            "R7 must reserve untouched fresh:200..239"
        )
    return payload
...
def _task_order(
    *,
    ...
    if start < 0 or end < start:
        raise ValueError("invalid R7 training index range")
    rows = [
        (str(family), index)
...
def train_episode(
    ...
    *,
    ...
    if getattr(task, "split", None) != "train":
        raise ValueError("R7 training is train-split only")
    if (
        getattr(task, "family", None)
        != "implicit_goal_regimes"
    ):
        raise ValueError(
            "R7 training is hidden-goal-family only"
        )
    ...
    if not trainable:
        raise ValueError(
            "R7 exposes no trainable successor gradients"
        )
...
def train_r7_policy(
    ...
    *,
    ...
    if tuple(families) != (
        ...
        raise ValueError(
            "R7 training scope must be exactly "
            "implicit_goal_regimes"
        )
    ...
    if not schedule:
        raise ValueError(
            "R7 requires at least one training stage"
        )
...
def evaluate_r7(
    ...
    *,
    ...
    if split not in {"dev", "fresh"}:
        raise ValueError(
            "R7 evaluation split must be dev or fresh"
        )
...
def load_r7_checkpoint(
    ...
    model = NativeR7ThresholdConsistencyPolicy(
        ...
        max_support=int(
            architecture["max_support"]
        ),
        confidence_power=float(
            architecture["confidence_power"]
        ),
    )
    model.load_state_dict(state, strict=True)
    if (
        r7_state_dict_sha256(model)
        != payload.get("state_dict_sha256")
    ):
        raise ValueError(
            "R7 checkpoint tensor digest mismatch"
        )
    if (
        state_dict_sha256(
            model.parent.state_dict()
    ...
    if (
        ...
        raise ValueError(
            "R7 checkpoint parameter audit mismatch"
        )
    if (
        model.successor_parameter_count()
        != int(
