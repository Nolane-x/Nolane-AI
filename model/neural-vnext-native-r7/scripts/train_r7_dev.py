...
from threshold_core import NativeR7ThresholdConsistencyPolicy
...
def main() -> int:
    ...
    if benchmark["fresh_indices"] != [200, 239]:
        raise ValueError("R7 must reserve fresh:200..239")
    if lock["fresh_isolation"]["status"] != "UNOPENED":
        raise ValueError("R7 fresh court must remain unopened")
    ...
    print(json.dumps({
        "status": "R7_PARENT_BASELINE_FRESH_UNOPENED",
    ...
    if selected is None:
        payload = {
            "schema_version": 1,
            "status": "R7_DEV_NO_ELIGIBLE_CANDIDATE_FRESH_UNOPENED",
    ...
    manifest = {
        **metadata,
        "schema_version": 1,
        "status": "R7_DEV_ELIGIBLE_FRESH_UNOPENED",
        "selected_candidate": selected["name"],
        "selected_rank": list(selected["rank"]),
        "selected_eligibility": selected["eligibility"],
        "parent_development": _compact(parent_dev),
