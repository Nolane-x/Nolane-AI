...
class NativeR7ThresholdConsistencyPolicy(nn.Module):
    """Frozen accepted R4 plus a public-evidence thresholded consistency residual."""
    ...
    def train(self, mode: bool = True) -> "NativeR7ThresholdConsistencyPolicy":
        super().train(mode)
        self.parent.eval()
        return self
    ...
    def forward_step(
        ...
        sharp_support_gate = (
            support_count <= float(self.max_support)
        ).to(posterior.dtype)
        evidence_gate = (
            sharp_support_gate
            * (1.0 - entropy).clamp(0.0, 1.0).pow(
        ...
        residual = residual * evidence_gate
        ...
        return {
            ...
            "evidence_gate": evidence_gate.squeeze(-1),
