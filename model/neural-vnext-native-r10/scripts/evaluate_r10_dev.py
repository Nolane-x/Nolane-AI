...
from multistep_runtime import evaluate_r10
...
def main() -> int:
    ...
    if lock.get("candidate")!="Neural-vNext-Native-R10-MultistepCounterfactual":
        raise ValueError("unexpected R10 candidate identity")
    if lock["fresh_isolation"]["status"]!="UNOPENED":
        raise ValueError("R10 fresh must remain unopened")
    if lock["benchmark"]["development_indices"]!=[480,511]:
        raise ValueError("R10 primary dev identities changed")
    if lock["benchmark"]["confirmation_indices"]!=[512,543]:
        raise ValueError("R10 confirmation identities changed")
    if lock["benchmark"]["fresh_indices"]!=[240,279]:
        raise ValueError("R10 fresh identities changed")
    ...
    print(json.dumps({
        "status":"R10_ACCEPTED_R9_BASELINE_FRESH_UNOPENED",
    ...
    payload={
        "schema_version":1,
        "status":"R10_DEV_ELIGIBLE_FRESH_UNOPENED" if selected else "R10_DEV_NO_ELIGIBLE_CANDIDATE_FRESH_UNOPENED",
        "candidate":"Neural-vNext-Native-R10-MultistepCounterfactual",
