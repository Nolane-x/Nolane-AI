...
def rollout_r10(
    ...
    *,
    ...
    while not task.done:
        ...
        action, decision = choose_public_multistep_action(
        ...
        multistep_decisions += int(
            decision.get("reason") == "certified_multistep_advantage"
        )
    ...
    return {
        ...
        "multistep_decisions": multistep_decisions,
...
def evaluate_r10(
    ...
    *,
    ...
    if split not in {"dev", "fresh"}:
        raise ValueError("R10 evaluation split must be dev or fresh")
    ...
    for family in families:
        ...
        for index in range(int(indices[0]), int(indices[1]) + 1):
            ...
            fdec += int(result["multistep_decisions"])
            solved += int(result["solved"])
            steps += int(result["steps"])
            overrides += int(result["overrides_vs_r9"])
            decisions += int(result["multistep_decisions"])
            episodes += 1
            rows.append({
                "family": str(family),
                "split": str(split),
