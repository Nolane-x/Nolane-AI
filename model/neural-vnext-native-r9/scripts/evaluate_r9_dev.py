...
def main()->int:
    ...
    if lock["candidate"]!="Neural-vNext-Native-R9-PublicCounterfactualPlanner":
        raise ValueError("unexpected R9 candidate")
    ...
    baseline=evaluate_r4(parent,make_task=make_r18_task,families=families,split="dev",indices=indices)
    print(json.dumps({"status":"R9_PARENT_BASELINE_FRESH_UNOPENED","parent_dev_solved":baseline["solved"],"parent_dev_families":baseline["families"],"dev_indices":list(indices)},sort_keys=True))
    rows=[]; selected=None
    for cfg in lock["selection"]["candidates"]:
        result=evaluate_r9(parent,make_task=make_r18_task,families=families,split="dev",indices=indices,max_support=int(cfg["max_support"]))
        eligibility=_eligibility(baseline,result)
        rank=(int(result["families"]["implicit_goal_regimes"]["solved"]),int(result["solved"]),-int(result["steps"]),str(cfg["name"]))
        row={"name":cfg["name"],"config":{"max_support":int(cfg["max_support"])},"development":_compact(result),"eligibility":eligibility,"rank":list(rank)}
        rows.append(row)
        print(json.dumps({"candidate":cfg["name"],"dev_solved":result["solved"],"dev_families":result["families"],"eligibility":eligibility,"rank":list(rank),"fresh_opened":False},sort_keys=True))
        if eligibility["eligible"] and (selected is None or rank>selected["rank"]):
            selected={"row":row,"rank":rank,"result":result}
    payload={
      "schema_version":1,
      "status":"R9_DEV_ELIGIBLE_FRESH_UNOPENED" if selected else "R9_DEV_NO_ELIGIBLE_CANDIDATE_FRESH_UNOPENED",
      "candidate":"Neural-vNext-Native-R9-PublicCounterfactualPlanner",
      ...
      "physical_parameters":lock["parent"]["parameters"],
      "fresh_opened":False,
    }
    args.manifest.parent.mkdir(parents=True,exist_ok=True)
    args.manifest.write_text(json.dumps(payload,indent=2,sort_keys=True)+"\n")
    args.dev_result.write_text(json.dumps({"parent":baseline,"selected":selected["result"] if selected else None,"candidates":rows,"fresh_opened":False},indent=2,sort_keys=True)+"\n")
    print(json.dumps({"status":payload["status"],"selected_candidate":payload["selected_candidate"],"selected_config":payload["selected_config"],"parent_dev_solved":baseline["solved"],"selected_dev_solved":selected["result"]["solved"] if selected else None,"fresh_opened":False},sort_keys=True))
