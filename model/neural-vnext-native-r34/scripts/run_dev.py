...
from trajectory_mechanism_core import NativeR34TrajectoryMechanismEnsemble
from trajectory_mechanism_training import (
    ...
    save_checkpoint,
    train_goal_ensemble,
    train_trajectory_mechanism_ensemble,
)
...
def main() -> int:
    ...
    if lock.get("candidate") != "Neural-vNext-Native-R34-TrajectoryMechanism":
        raise ValueError("unexpected R34 candidate")
    if (
        lock["fresh_isolation"]["status"] != "UNOPENED"
        or lock["benchmark"]["fresh_indices"] != [280, 319]
    ):
        raise ValueError("R34 fresh isolation changed")
    ...
    print(
        json.dumps(
            {
                "status": "R34_R11_BASELINE_FRESH_UNOPENED",
    ...
    model = NativeR34TrajectoryMechanismEnsemble(
        ...
        mechanism_hidden_dim=int(architecture["mechanism_hidden_dim"]),
    )
    if model.goal_parameter_count() != int(architecture["expected_goal_successor_parameters"]):
        raise AssertionError("R34 goal parameter count changed")
    if model.mechanism_parameter_count() != int(architecture["expected_mechanism_successor_parameters"]):
        raise AssertionError("R34 mechanism parameter count changed")
    if model.parameter_count() != int(architecture["expected_successor_parameters"]):
        raise AssertionError("R34 total parameter count changed")
    ...
    joint_train_rows = collect_joint_outcome_rows(
        ...
        model,
        make_task=make_r18_task,
        indices=tuple(benchmark["mechanism_training_indices"]),
    ...
    mechanism_training_summary = train_trajectory_mechanism_ensemble(
        model,
        joint_train_rows,
        seed=int(training["seed"]),
        epochs=int(training["mechanism_epochs"]),
        batch_size=int(training["batch_size"]),
        learning_rate=float(training["learning_rate"]),
        weight_decay=float(training["weight_decay"]),
        mechanism_loss_weight=float(training["mechanism_loss_weight"]),
        trajectory_loss_weight=float(training["trajectory_loss_weight"]),
    )
    ...
    metadata = save_checkpoint(
        ...
        mechanism_training_summary=mechanism_training_summary,
        rescue_guard=rescue_guard,
    )

    status = (
        "R34_DEV_ELIGIBLE_CONFIRMATION_UNOPENED_FRESH_UNOPENED"
        if eligibility["eligible"]
        else "R34_DEV_REJECTED_CONFIRMATION_FRESH_UNOPENED"
    )
    selected_config = {
        "temperature": temperature,
    ...
    manifest = {
        **metadata,
        ...
        "mechanism_training_rows": len(joint_train_rows),
        "mechanism_training_class_counts": _class_counts(joint_train_rows),
        "mechanism_training_certification": _certification_stats(joint_train_rows),
        ...
        "selected_candidate": "trajectory_mechanism_supervised_joint_outcome",
