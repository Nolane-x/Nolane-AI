...
R11_HORIZON = 1
CHECKPOINT_FORMAT = "nolane-neural-vnext-native-r34-trajectory-mechanism-v1"
...
def _terminal_mechanism(task: Any) -> int:
    event = str(task.observe().get("last_event", ""))
    if bool(task.solved):
        return MECHANISM_SOLVED
    if event == "submission rejected":
        return MECHANISM_REJECTED_SUBMIT
    if event == "action budget exhausted":
        return MECHANISM_BUDGET_EXHAUSTED
    raise ValueError(f"unexpected terminal mechanism: {event!r}")


def _update_trajectory_stats(stats: dict[str, float], result: Any) -> None:
    stats["steps"] += 1.0
    stats["progress_sum"] += float(result.progress_delta)
    stats["positive"] += float(float(result.progress_delta) > 1.0e-9)
    stats["information_sum"] += float(result.information_gain)


def _finish_with_r11(
    parent: Any,
    task: Any,
    state: PublicEpisodeState,
    ...
    while not task.done:
        ...
        _update_trajectory_stats(stats, result)
    steps = max(1.0, float(stats["steps"]))
    return {
        "solved": bool(task.solved),
        "absolute_steps": int(task.step_count),
        "branch_steps": int(stats["steps"]),
        "mechanism": int(_terminal_mechanism(task)),
        "progress_sum": float(stats["progress_sum"]),
        "positive_fraction": float(stats["positive"]) / steps,
        "information_mean": float(stats["information_sum"]) / steps,
    }
...
def _branch_outcome(
    ...
    *,
    ...
    _update_trajectory_stats(stats, result)
    return _finish_with_r11(parent, branch_task, branch_state, stats)
...
def _trajectory_target(
    ...
    return torch.tensor(
        [
            min(1.0, float(baseline["branch_steps"]) / 32.0),
            min(1.0, float(candidate["branch_steps"]) / 32.0),
            max(-1.0, min(1.0, float(baseline["progress_sum"]) / 8.0)),
            max(-1.0, min(1.0, float(candidate["progress_sum"]) / 8.0)),
            float(baseline["positive_fraction"]),
            float(candidate["positive_fraction"]),
            float(baseline["information_mean"]),
            float(candidate["information_mean"]),
        ],
        dtype=torch.float32,
    )
            ...
            if (
                ...
                if candidates:
                    ...
                    for candidate in candidates:
                        candidate_outcome = _branch_outcome(
                            parent,
                            task=task,
                            state=state,
                            action_features=action_features,
                        ...
                        candidate_solved = bool(candidate_outcome["solved"])
                        ...
                        rows.append(
                            {
                                **candidate,
                                ...
                                "baseline_mechanism": int(baseline["mechanism"]),
                                "candidate_mechanism": int(candidate_outcome["mechanism"]),
                                "trajectory_target": _trajectory_target(
                                    baseline, candidate_outcome
                                ),
                                "baseline_steps": int(baseline["absolute_steps"]),
                                "candidate_steps": int(candidate_outcome["absolute_steps"]),
...
def train_trajectory_mechanism_ensemble(
    model: NativeR34TrajectoryMechanismEnsemble,
    ...
    *,
    ...
    joint_labels = torch.tensor([int(row["class_id"]) for row in rows], dtype=torch.long)
    baseline_mechanism = torch.tensor(
        [int(row["baseline_mechanism"]) for row in rows], dtype=torch.long
    )
    candidate_mechanism = torch.tensor(
        [int(row["candidate_mechanism"]) for row in rows], dtype=torch.long
    )
    trajectory_targets = torch.stack(
        [row["trajectory_target"] for row in rows], dim=0
    ).float()

    joint_weights = _balanced_weights(joint_labels, 4)
    baseline_weights = _balanced_weights(baseline_mechanism, 3)
    candidate_weights = _balanced_weights(candidate_mechanism, 3)
    class_counts = [
        int((joint_labels == class_id).sum().item()) for class_id in range(4)
    ]

    summaries: list[dict[str, Any]] = []
    for head_index, head in enumerate(model.mechanism_heads):
        ...
        for epoch in range(int(epochs)):
            ...
            rng.shuffle(order)
            losses: list[float] = []
            joint_correct = baseline_correct = candidate_correct = 0
            trajectory_abs = 0.0
            trajectory_elements = 0
            for start in range(0, len(order), int(batch_size)):
                ids = order[start : start + int(batch_size)]
                xb = features[ids]
                y_joint = joint_labels[ids]
                y_baseline = baseline_mechanism[ids]
                y_candidate = candidate_mechanism[ids]
                y_trajectory = trajectory_targets[ids]
                output = head(xb)

                joint_loss = F.cross_entropy(
                    output["joint_logits"], y_joint, weight=joint_weights
                )
                baseline_loss = F.cross_entropy(
                    output["baseline_mechanism_logits"],
                    y_baseline,
                    weight=baseline_weights,
                )
                candidate_loss = F.cross_entropy(
                    output["candidate_mechanism_logits"],
                    y_candidate,
                    weight=candidate_weights,
                )
                trajectory_loss = F.smooth_l1_loss(
                    output["trajectory"], y_trajectory
                )
                loss = (
                    joint_loss
                    + float(mechanism_loss_weight)
                    * 0.5
                    * (baseline_loss + candidate_loss)
                    + float(trajectory_loss_weight) * trajectory_loss
                )
                ...
                losses.append(float(loss.detach().item()))
                joint_correct += int(
                    (output["joint_logits"].detach().argmax(dim=-1) == y_joint)
                    .sum()
                    .item()
                )
                baseline_correct += int(
                    (
                        output["baseline_mechanism_logits"].detach().argmax(dim=-1)
                        == y_baseline
                    )
                    .sum()
                    .item()
                ...
                candidate_correct += int(
                    (
                        output["candidate_mechanism_logits"].detach().argmax(dim=-1)
                        == y_candidate
                    )
                    .sum()
                    .item()
                ...
                trajectory_abs += float(
                    (output["trajectory"].detach() - y_trajectory).abs().sum().item()
                )
                trajectory_elements += int(y_trajectory.numel())

            history.append(
                {
                    "epoch": float(epoch + 1),
                    "mean_loss": sum(losses) / max(1, len(losses)),
                    "joint_accuracy": joint_correct / max(1, len(rows)),
                    "baseline_mechanism_accuracy": baseline_correct / max(1, len(rows)),
                    "candidate_mechanism_accuracy": candidate_correct / max(1, len(rows)),
                    "trajectory_mae": trajectory_abs / max(1, trajectory_elements),
                }
            )
        summaries.append({"head": head_index, "history": history})
    model.eval()
    return {
        ...
        "joint_class_weights": [float(v) for v in joint_weights.tolist()],
        "baseline_mechanism_weights": [float(v) for v in baseline_weights.tolist()],
        "candidate_mechanism_weights": [float(v) for v in candidate_weights.tolist()],
        "mechanism_loss_weight": float(mechanism_loss_weight),
...
def _score_action_row(
    ...
    output = model.mechanism_outputs(row["features"])
    probs = output["joint_probabilities"]
    baseline_mechanism = output["baseline_mechanism_probabilities"]
    candidate_mechanism = output["candidate_mechanism_probabilities"]
    return {
        ...
        "mean_baseline_fail": float(
            (1.0 - baseline_mechanism[:, MECHANISM_SOLVED]).mean().item()
        ),
        "mean_candidate_solve": float(
            candidate_mechanism[:, MECHANISM_SOLVED].mean().item()
        ),
        "trajectory_disagreement": float(
...
def evaluate_r34(
    ...
    *,
    ...
    if split not in {"dev", "fresh"}:
        raise ValueError("R34 evaluation split must be dev/fresh")
    rows: list[dict[str, Any]] = []
...
def save_checkpoint(
    ...
    *,
    ...
    mechanism_training_summary: Mapping[str, Any],
    ...
    payload = {
        ...
        "mechanism_successor_parameters": model.mechanism_parameter_count(),
        ...
        "mechanism_training_summary": dict(mechanism_training_summary),
