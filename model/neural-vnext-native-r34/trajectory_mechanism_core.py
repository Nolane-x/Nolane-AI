...
TERMINAL_MECHANISM_CLASS_COUNT = 3
MECHANISM_SOLVED = 0
MECHANISM_REJECTED_SUBMIT = 1
MECHANISM_BUDGET_EXHAUSTED = 2

TRAJECTORY_REGRESSION_DIM = 8
...
def encode_public_goal_features(
    *,
    ...
    if features.shape != (PUBLIC_GOAL_FEATURE_DIM,):
        raise AssertionError("unexpected R34 public goal feature shape")
    return features
...
def encode_parent_latent_context(parent_output: Mapping[str, Tensor]) -> Tensor:
    ...
    if latent.shape != (LATENT_CONTEXT_DIM,):
        raise AssertionError("unexpected R34 parent latent context shape")
...
def encode_pair_features(
    *,
    ...
    if features.shape != (PAIR_FEATURE_DIM,):
        raise AssertionError("unexpected R34 pair feature shape")
    return features
...
class _TrajectoryMechanismHead(nn.Module):
    """Shared representation supervised by outcome, failure mechanism and trajectory shape."""
    ...
    def __init__(self, hidden_dim: int) -> None:
        ...
        self.joint_outcome = nn.Linear(hidden_dim, JOINT_OUTCOME_CLASS_COUNT)
        self.baseline_mechanism = nn.Linear(hidden_dim, TERMINAL_MECHANISM_CLASS_COUNT)
        self.candidate_mechanism = nn.Linear(hidden_dim, TERMINAL_MECHANISM_CLASS_COUNT)
        self.trajectory = nn.Linear(hidden_dim, TRAJECTORY_REGRESSION_DIM)

    def forward(self, features: Tensor) -> dict[str, Tensor]:
        hidden = self.trunk(features)
        return {
            "joint_logits": self.joint_outcome(hidden),
            "baseline_mechanism_logits": self.baseline_mechanism(hidden),
            "candidate_mechanism_logits": self.candidate_mechanism(hidden),
            "trajectory": self.trajectory(hidden),
        }


class NativeR34TrajectoryMechanismEnsemble(nn.Module):
    """R33 decision surface with explicit terminal-mechanism and trajectory supervision."""
    ...
    def __init__(
        ...
        *,
        ...
        self.mechanism_hidden_dim = int(mechanism_hidden_dim)
        ...
        self.mechanism_heads = nn.ModuleList(
            [
                _TrajectoryMechanismHead(self.mechanism_hidden_dim)
    ...
    def architecture(self) -> dict[str, Any]:
        return {
            "r34_ensemble_size": self.ensemble_size,
            "r34_goal_hidden_dim": self.goal_hidden_dim,
            "r34_mechanism_hidden_dim": self.mechanism_hidden_dim,
            "r34_pair_feature_dim": PAIR_FEATURE_DIM,
            "r34_joint_outcome_class_count": JOINT_OUTCOME_CLASS_COUNT,
            "r34_terminal_mechanism_class_count": TERMINAL_MECHANISM_CLASS_COUNT,
            "r34_trajectory_regression_dim": TRAJECTORY_REGRESSION_DIM,
            "mechanism": "shared_pair_trunk_with_joint_outcome_terminal_mechanism_and_trajectory_auxiliary_supervision",
        }
    ...
    def mechanism_parameter_count(self) -> int:
        return sum(
            parameter.numel()
    ...
    def mechanism_outputs(self, features: Tensor) -> dict[str, Tensor]:
        if features.shape != (PAIR_FEATURE_DIM,):
            raise ValueError("R34 pair feature shape mismatch")
        outputs = [head(features.unsqueeze(0)) for head in self.mechanism_heads]
        return {
            "joint_probabilities": torch.softmax(
                torch.stack([out["joint_logits"][0] for out in outputs], dim=0),
                dim=-1,
            ),
            "baseline_mechanism_probabilities": torch.softmax(
                torch.stack(
                    [out["baseline_mechanism_logits"][0] for out in outputs], dim=0
                ),
                dim=-1,
            ),
            "candidate_mechanism_probabilities": torch.softmax(
                torch.stack(
                    [out["candidate_mechanism_logits"][0] for out in outputs], dim=0
                ),
                dim=-1,
            ),
            "trajectory": torch.stack([out["trajectory"][0] for out in outputs], dim=0),
        }
