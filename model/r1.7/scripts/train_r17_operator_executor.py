...
def main():
    ...
    best=None;best_epoch=0;best_state=None;history=[]
    ...
    for epoch in range(1,epochs+1):
        ...
        if operator_executor_internal_gate(metrics):
            ...
            if key>old:
                best=copy.deepcopy(metrics);best_epoch=epoch;best_state=copy.deepcopy(model.state_dict())
    accepted=best is not None
    if accepted: model.load_state_dict(best_state)
    report={'version':'r1.7-operator-executor-internal-v1','protocol':{'fit_indices':[282,482],'val_indices':[482,522],'seed':seed,'epochs':epochs,'lr':lr,'weight_decay':1e-4,'batch_size':batch_size},'parent_sha256':sha256_file(parent),'parent_candidate_effective_parameters':parent_meta['candidate_effective_parameters'],'trainable_parameters':sum(p.numel() for p in params),'fit_rows':len(fit),'val_rows':len(val),'initial_validation':initial,'best_epoch':best_epoch,'best_validation':best,'accepted_for_program_search':accepted,'history':history}
