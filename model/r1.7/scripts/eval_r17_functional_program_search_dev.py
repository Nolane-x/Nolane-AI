...
def main():
    ...
    for index in range(60):
        task=make_r17_task('composition_holdout','dev',index)
    ...
    report={'version':'r1.7-functional-program-search-dev-v1','protocol':{'split':'dev','family':'composition_holdout','indices':[0,60],'max_horizon':4,'trainable_parameters':0},'operator_executor_sha256':sha256_file(parent),'operator_executor_candidate_effective_parameters':meta['candidate_effective_parameters'],'worlds':total,'demo_exact_rate':exact/total,'task_solve_rate':solved/total,'false_exact_rate':false_exact/total,'mean_action_efficiency':eff_sum/max(1,eff_n),'templates':templates,'rows':rows}
    report['accepted']=report['demo_exact_rate']>=.90 and report['task_solve_rate']>=.80 and report['false_exact_rate']<=.05 and all(v['solve_rate']>=.70 for v in templates.values())
    path=root/'results/r1_7_functional_program_search_dev.json';path.write_text(json.dumps(report,indent=2,sort_keys=True)+'\n')
