...
from selective_neural_core import NativeR13SelectiveNeuralCausalPolicy
from selective_neural_training import (
    evaluate_r13,
    load_lock,
    save_r13_checkpoint,
    train_r13_policy,
    calibrate_override_margin,
)
...
def main()->int:
    ...
    print(json.dumps({
        "status":"R13_ACCEPTED_R11_BASELINE_FRESH_UNOPENED",
    ...
    for i,cfg in enumerate(training["candidates"]):
        ...
        calibration=calibrate_override_margin(
            model,
            make_task=make_r18_task,
            oracle_plan=oracle_plan,
            calibration_indices=calibration_indices,
            precision_floor=float(training["calibration_precision_floor"]),
        )
        ...
        row={
            ...
            "architecture":{
                ...
                "max_support":int(cfg["max_support"]),
                "parent_margin":float(cfg["parent_margin"]),
                "override_margin_threshold":float(model.override_margin_threshold),
            },
            "training":summary,
    ...
    if selected is None:
        payload={
            "schema_version":1,
            "status":"R13_DEV_NO_ELIGIBLE_CANDIDATE_FRESH_UNOPENED",
            "runtime":runtime,
    ...
    manifest={
        **metadata,
        "schema_version":1,
        "status":"R13_DEV_ELIGIBLE_FRESH_UNOPENED",
        "selected_candidate":selected["name"],
        "selected_rank":list(selected["rank"]),
        "selected_eligibility":selected["eligibility"],
        "parent_development":_compact(parent_dev),
