...
from selective_neural_core import CAUSAL_FEATURE_DIM, NativeR13SelectiveNeuralCausalPolicy
...
def test_zero_init_hidden_action_matches_r11_anchor() -> None:
    parent=_r4()
    model=NativeR13SelectiveNeuralCausalPolicy(parent,posterior_embedding_dim=16,residual_hidden_dim=24,max_support=6,parent_margin=1.0,override_margin_threshold=1.0e9)
...
def test_visible_target_returns_exact_r4_logits() -> None:
    parent=_r4()
    model=NativeR13SelectiveNeuralCausalPolicy(parent,posterior_embedding_dim=16,residual_hidden_dim=24,max_support=125,parent_margin=1.0,override_margin_threshold=1.0e9)
    with torch.no_grad():
...
def test_broad_support_disables_residual() -> None:
    parent=_r4()
    model=NativeR13SelectiveNeuralCausalPolicy(parent,posterior_embedding_dim=16,residual_hidden_dim=24,max_support=3,parent_margin=1.0,override_margin_threshold=1.0e9)
    with torch.no_grad():
        model.residual_score[-1].bias.fill_(10.0)
    g=torch.zeros(1,GLOBAL_FEATURE_DIM)
    a=torch.randn(1,4,ACTION_FEATURE_DIM)
...
def test_only_r13_modules_are_trainable() -> None:
    model=NativeR13SelectiveNeuralCausalPolicy(_r4(),posterior_embedding_dim=16,residual_hidden_dim=24,max_support=3,parent_margin=1.0,override_margin_threshold=1.0e9)
...
def test_margin_gate_can_block_nonzero_residual_override() -> None:
    parent=_r4()
    model=NativeR13SelectiveNeuralCausalPolicy(
        parent,
        posterior_embedding_dim=16,
        residual_hidden_dim=24,
        max_support=125,
        parent_margin=1.0,
        override_margin_threshold=1.0e9,
    )
    with torch.no_grad():
        model.residual_score[-1].bias.fill_(5.0)
    g=torch.zeros(1,GLOBAL_FEATURE_DIM)
    a=torch.randn(1,4,ACTION_FEATURE_DIM)
    valid=torch.ones(1,4,dtype=torch.bool)
    with torch.no_grad():
    ...
    assert int(out["action_logits"].argmax(-1).item())==1
    assert not bool(out["override_allowed"].item())
