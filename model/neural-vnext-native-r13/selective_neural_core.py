...
class NativeR13SelectiveNeuralCausalPolicy(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        override_margin_threshold: float = 1.0e9,
    ) -> None:
        super().__init__()
        ...
        if float(override_margin_threshold) < 0.0:
            raise ValueError("override_margin_threshold must be non-negative")
        self.parent = parent
        ...
        self.override_margin_threshold = float(override_margin_threshold)
    ...
    def train(self, mode: bool = True) -> "NativeR13SelectiveNeuralCausalPolicy":
        super().train(mode)
        self.parent.eval()
        return self
    ...
    def architecture(self) -> dict[str, Any]:
        return {
            **self.parent.architecture(),
            "r13_posterior_embedding_dim": self.posterior_embedding_dim,
            "r13_residual_hidden_dim": self.residual_hidden_dim,
            "r13_max_support": self.max_support,
            "r13_parent_margin": self.parent_margin,
            "r13_override_margin_threshold": self.override_margin_threshold,
            "r13_causal_feature_dim": CAUSAL_FEATURE_DIM,
            "r13_goal_hypothesis_count": GOAL_HYPOTHESIS_COUNT,
        }

    def set_override_margin_threshold(self, value: float) -> None:
        if float(value) < 0.0:
            raise ValueError("override margin threshold must be non-negative")
        self.override_margin_threshold = float(value)
    ...
    def load_successor_state_dict(self, state: dict[str, Tensor]) -> None:
        ...
        if set(state) != expected:
            raise ValueError("R13 successor state keys mismatch")
    ...
    def forward_from_parent(
        ...
        *,
        ...
        if action_features.shape[:2] != (batch, actions):
            raise ValueError("R13 action/causal shape mismatch")
        ...
        proposal_logits = (anchored + residual).masked_fill(
            ~valid_actions,
            torch.finfo(parent_logits.dtype).min,
        )
        proposed_actions = proposal_logits.argmax(dim=-1)
        proposed_scores = proposal_logits.gather(
            1, proposed_actions[:, None]
        ).squeeze(1)
        r11_scores = proposal_logits.gather(
            1, r11_actions[:, None]
        ).squeeze(1)
        proposal_margin = proposed_scores - r11_scores
        override_allowed = (
            (target_visible.squeeze(-1) < 0.5)
            & (support_gate.squeeze(-1) > 0.5)
            & (proposed_actions != r11_actions)
            & (
                proposal_margin
                >= float(self.override_margin_threshold)
            )
        )
        selective_hidden_logits = torch.where(
