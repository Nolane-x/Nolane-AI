...
from branch_sequence_decoder_core import NativeR35BranchSequenceDecoderEnsemble
from branch_sequence_decoder_training import (
    ...
    save_checkpoint,
    train_goal_ensemble,
    train_branch_sequence_ensemble,
)
...
def main() -> int:
    ...
    if lock.get("candidate") != "Neural-vNext-Native-R35-BranchSequenceDecoder":
        raise ValueError("unexpected R35 candidate")
    if (
        lock["fresh_isolation"]["status"] != "UNOPENED"
        or lock["benchmark"]["fresh_indices"] != [280, 319]
    ):
        raise ValueError("R35 fresh isolation changed")
    ...
    print(
        json.dumps(
            {
                "status": "R35_R11_BASELINE_FRESH_UNOPENED",
    ...
    model = NativeR35BranchSequenceDecoderEnsemble(
        ...
        decoder_hidden_dim=int(architecture["decoder_hidden_dim"]),
        horizon=int(architecture["branch_sequence_horizon"]),
        time_embedding_dim=int(architecture["time_embedding_dim"]),
    )
    if model.goal_parameter_count() != int(architecture["expected_goal_successor_parameters"]):
        raise AssertionError("R35 goal parameter count changed")
    if model.sequence_parameter_count() != int(architecture["expected_sequence_successor_parameters"]):
        raise AssertionError("R35 sequence parameter count changed")
    if model.parameter_count() != int(architecture["expected_successor_parameters"]):
        raise AssertionError("R35 total parameter count changed")
    ...
    joint_train_rows = collect_joint_outcome_rows(
        ...
        model,
        make_task=make_r18_task,
        indices=tuple(benchmark["sequence_training_indices"]),
    ...
    sequence_training_summary = train_branch_sequence_ensemble(
        model,
        joint_train_rows,
        seed=int(training["seed"]),
        epochs=int(training["sequence_epochs"]),
        batch_size=int(training["batch_size"]),
        learning_rate=float(training["learning_rate"]),
        weight_decay=float(training["weight_decay"]),
        sequence_loss_weight=float(training["sequence_loss_weight"]),
    )
    ...
    metadata = save_checkpoint(
        ...
        sequence_training_summary=sequence_training_summary,
        rescue_guard=rescue_guard,
    )

    status = (
        "R35_DEV_ELIGIBLE_CONFIRMATION_UNOPENED_FRESH_UNOPENED"
        if eligibility["eligible"]
        else "R35_DEV_REJECTED_CONFIRMATION_FRESH_UNOPENED"
    )
    selected_config = {
        "temperature": temperature,
    ...
    manifest = {
        **metadata,
        ...
        "private_boundary": {
            ...
            "branch_sequence_labels_are_training_only": True,
            "future_branch_sequence_used_as_input": False,
            "all_alternative_actions_labeled": True,
            "inference_private_goal_use": False,
        },
        "sequence_training_rows": len(joint_train_rows),
        "sequence_training_class_counts": _class_counts(joint_train_rows),
        "sequence_training_certification": _certification_stats(joint_train_rows),
        ...
        "selected_candidate": "fixed_policy_branch_sequence_decoder",
