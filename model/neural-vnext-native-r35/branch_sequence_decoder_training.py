...
R11_HORIZON = 1
CHECKPOINT_FORMAT = "nolane-neural-vnext-native-r35-branch-sequence-decoder-v1"
...
def _step_signature(result: Any) -> Tensor:
    return torch.tensor(
        [
            float(result.progress_delta),
            float(result.information_gain),
            float(bool(result.done)),
            float(bool(result.solved)),
        ],
        dtype=torch.float32,
    )


def _sequence_target(signatures: Sequence[Tensor], solved: bool) -> Tensor:
    rows = [row.detach().clone().float() for row in signatures[:BRANCH_SEQUENCE_HORIZON]]
    while len(rows) < BRANCH_SEQUENCE_HORIZON:
        rows.append(
            torch.tensor(
                [0.0, 0.0, 1.0, float(bool(solved))],
                dtype=torch.float32,
            )
        )
    target = torch.stack(rows, dim=0)
    if target.shape != (BRANCH_SEQUENCE_HORIZON, BRANCH_SEQUENCE_CHANNELS):
        raise AssertionError("unexpected R35 branch sequence target shape")
    return target
...
def _finish_with_r11(
    ...
    while not task.done:
        ...
        signatures.append(_step_signature(result))
    return {
        "solved": bool(task.solved),
        "absolute_steps": int(task.step_count),
        "sequence_target": _sequence_target(signatures, bool(task.solved)),
    }
...
def _branch_outcome(
    ...
    *,
    ...
    signatures.append(_step_signature(result))
    return _finish_with_r11(parent, branch_task, branch_state, signatures)
                        ...
                        rows.append(
                            {
                                **candidate,
                                ...
                                "baseline_sequence_target": baseline["sequence_target"],
                                "candidate_sequence_target": candidate_outcome["sequence_target"],
...
def _branch_sequence_loss(prediction: Tensor, target: Tensor) -> Tensor:
    continuous = F.smooth_l1_loss(prediction[..., :2], target[..., :2])
...
def train_branch_sequence_ensemble(
    model: NativeR35BranchSequenceDecoderEnsemble,
    ...
    *,
    ...
    baseline_targets = torch.stack(
        [row["baseline_sequence_target"] for row in rows], dim=0
    ).float()
    candidate_targets = torch.stack(
        [row["candidate_sequence_target"] for row in rows], dim=0
    ).float()
    ...
    summaries: list[dict[str, Any]] = []
    for head_index, head in enumerate(model.sequence_heads):
        ...
        for epoch in range(int(epochs)):
            ...
            binary_correct = 0
            binary_elements = 0
            for start_index in range(0, len(order), int(batch_size)):
                ids = order[start_index : start_index + int(batch_size)]
                xb = features[ids]
                y_joint = joint_labels[ids]
                y_baseline = baseline_targets[ids]
                y_candidate = candidate_targets[ids]
                output = head(xb)

                joint_loss = F.cross_entropy(
                    output["joint_logits"], y_joint, weight=joint_weights
                )
                baseline_loss = _branch_sequence_loss(
                    output["baseline_sequence"], y_baseline
                )
                candidate_loss = _branch_sequence_loss(
                    output["candidate_sequence"], y_candidate
                )
                loss = joint_loss + float(sequence_loss_weight) * 0.5 * (
                ...
                for prediction, target in (
                    (output["baseline_sequence"].detach(), y_baseline),
                    (output["candidate_sequence"].detach(), y_candidate),
                ):
                    continuous_abs += float(
                        (prediction[..., :2] - target[..., :2]).abs().sum().item()
                    )
                    continuous_elements += int(target[..., :2].numel())
                    binary_prediction = torch.sigmoid(prediction[..., 2:]) >= 0.5
                    binary_correct += int(
                        (binary_prediction == target[..., 2:].bool()).sum().item()
                    )
                    binary_elements += int(target[..., 2:].numel())
            history.append(
                {
                    "epoch": float(epoch + 1),
                    "mean_loss": sum(losses) / max(1, len(losses)),
                    "joint_accuracy": joint_correct / max(1, len(rows)),
                    "sequence_continuous_mae": continuous_abs / max(1, continuous_elements),
                    "sequence_binary_accuracy": binary_correct / max(1, binary_elements),
                }
            )
        summaries.append({"head": head_index, "history": history})
    model.eval()
...
def _score_action_row(
    ...
    output = model.branch_outputs(row["features"])
    ...
    return {
        ...
        "sequence_disagreement": float(
            torch.cat(
                (output["baseline_sequences"], output["candidate_sequences"]),
                dim=1,
            ).std(dim=0, unbiased=False).mean().item()
        ),
    }
...
def evaluate_r35(
    ...
    *,
    ...
    if split not in {"dev", "fresh"}:
        raise ValueError("R35 evaluation split must be dev/fresh")
    rows: list[dict[str, Any]] = []
...
def save_checkpoint(
    ...
    *,
    ...
    sequence_training_summary: Mapping[str, Any],
    ...
    payload = {
        ...
        "sequence_successor_parameters": model.sequence_parameter_count(),
        ...
        "sequence_training_summary": dict(sequence_training_summary),
