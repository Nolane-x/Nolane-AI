...
from goal_belief_core import NativeR22GoalBeliefEnsemble
from conformal_training import (
...
def _configure(seed: int) -> dict[str, Any]:
    ...
    return {
        "seed":int(seed),"deterministic_algorithms":bool(torch.are_deterministic_algorithms_enabled()),
        "mkldnn_enabled":bool(torch.backends.mkldnn.enabled),"num_threads":int(torch.get_num_threads()),
    }
...
def main() -> int:
    ...
    if lock.get("candidate")!="Neural-vNext-Native-R22-ConformalGoalBelief":
        raise ValueError("unexpected R22 candidate")
    if lock["fresh_isolation"]["status"]!="UNOPENED" or lock["benchmark"]["fresh_indices"]!=[280,319]:
        raise ValueError("R22 fresh isolation changed")
    ...
    print(json.dumps({"status":"R22_R11_BASELINE_FRESH_UNOPENED","parent_dev_solved":parent_dev["solved"],"parent_dev_families":parent_dev["families"],"dev_indices":list(dev_indices)},sort_keys=True))
    ...
    validation_data=collect_goal_dataset(parent,make_task=make_r18_task,indices=tuple(benchmark["guard_validation_indices"]))

    torch.manual_seed(int(training["seed"]))
    model=NativeR22GoalBeliefEnsemble(ensemble_size=int(training["ensemble_size"]),hidden_dim=int(training["hidden_dim"]))
    ...
    temperature_calibration=calibrate_temperature(model,fit_data,candidates=[float(v) for v in training["temperature_candidates"]])
    temperature=float(temperature_calibration["selected_temperature"])
    guard=fit_conformal_guard(
        model,fit_data,validation_data,temperature=temperature,
        alpha_candidates=[float(v) for v in training["alpha_candidates"]],
        minimum_precision=float(training["minimum_guard_precision"]),
        minimum_rows=int(training["minimum_guard_rows"]),
    )
    q=None if not guard["enabled"] else float(guard["selected"]["q"])
    candidate=evaluate_r22(
    ...
    metadata=save_checkpoint(
        ...
        temperature_calibration=temperature_calibration,conformal_guard=guard,
    )
    status="R22_DEV_ELIGIBLE_FRESH_UNOPENED" if eligibility["eligible"] else "R22_DEV_REJECTED_FRESH_UNOPENED"
    selected_config={
    ...
    manifest={
        **metadata,"schema_version":1,"status":status,"runtime":runtime,
        ...
        "selected_candidate":"conformal_singleton","selected_config":selected_config,
        "selected_eligibility":eligibility,"fresh_opened":False,
    }
    args.manifest.parent.mkdir(parents=True,exist_ok=True)
    args.manifest.write_text(json.dumps(manifest,indent=2,sort_keys=True)+"\n",encoding="utf-8")
    args.dev_result.write_text(json.dumps({
        "parent":parent_dev,"candidate":candidate,"temperature_calibration":temperature_calibration,
        "conformal_guard":guard,"selected_config":selected_config,"eligibility":eligibility,"fresh_opened":False,
    ...
    print(json.dumps({
        ...
        "selected_config":selected_config,"guard":guard,"successor_parameters":metadata["successor_parameters"],
