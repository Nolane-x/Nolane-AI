...
def encode_public_goal_features(
    *,
    ...
    if features.shape != (PUBLIC_GOAL_FEATURE_DIM,):
        raise AssertionError("unexpected R22 public goal feature shape")
    return features
...
class NativeR22GoalBeliefEnsemble(nn.Module):
    ...
    def architecture(self) -> dict[str, Any]:
        return {
            "r22_ensemble_size": self.ensemble_size,
            "r22_hidden_dim": self.hidden_dim,
            "r22_public_goal_feature_dim": PUBLIC_GOAL_FEATURE_DIM,
            "r22_goal_hypothesis_count": GOAL_HYPOTHESIS_COUNT,
        }
    ...
    def ensemble_logits(self, features: Tensor) -> Tensor:
        if features.ndim != 2 or features.shape[-1] != PUBLIC_GOAL_FEATURE_DIM:
            raise ValueError("R22 features must be [batch,dim]")
    ...
    def probabilities(
        ...
        *,
        ...
        if features.ndim != 1 or features.shape[0] != PUBLIC_GOAL_FEATURE_DIM:
            raise ValueError("R22 probabilities expect one feature row")
        if support_mask.shape != (GOAL_HYPOTHESIS_COUNT,):
