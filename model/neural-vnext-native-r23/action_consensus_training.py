...
from goal_belief_core import (
    NativeR23GoalBeliefEnsemble,
    encode_public_goal_features,
    goal_index,
)

R11_HORIZON = 1
CHECKPOINT_FORMAT = "nolane-neural-vnext-native-r23-conformal-action-consensus-v1"
...
def train_goal_ensemble(
    model: NativeR23GoalBeliefEnsemble,
...
def calibrate_temperature(
    ...
    *,
    ...
    for value in candidates:
        mean=_batch_probabilities(model,dataset,temperature=float(value))
...
def fit_conformal_thresholds(
    ...
    *,
    ...
    mean=_batch_probabilities(model,dataset,temperature=float(temperature))
    labels=dataset["labels"].long(); support=dataset["support_mask"].bool()
    scores=[]
    for i in range(int(mean.shape[0])):
        count=int(support[i].sum().item())
        if 2 <= count <= 3:
            scores.append(1.0-float(mean[i,int(labels[i])].item()))
    return [{"alpha":float(alpha),"q":_conformal_q(scores,alpha=float(alpha))} for alpha in alpha_candidates]
...
def _consensus_action(
    ...
    *,
    ...
    goals=[int(row[0].item()) for row in prediction_set.nonzero(as_tuple=False)]
    if not goals:
        return None
    actions={
        _action_for_goal(
            goal_index_value=goal,
            observation=observation,
            state=state,
            parent_logits=parent_logits,
        )
    ...
    if len(actions)!=1:
        return None
    return int(next(iter(actions)))


def fit_action_consensus_guard(
    parent: Any,
    model: NativeR23GoalBeliefEnsemble,
    *,
    make_task: Any,
    ...
    temperature: float,
    thresholds: Sequence[Mapping[str,float]],
    max_support: int,
    ...
    stats=[
        {
            "alpha":float(row["alpha"]),"q":float(row["q"]),
            "consensus_rows":0,"override_rows":0,"correct_override_rows":0,
        }
    ...
    for index in range(int(indices[0]),int(indices[1])+1):
        ...
        while not task.done:
            ...
            if (
                ...
                mean,_=model.probabilities(
                    features=features,support_mask=support_mask,temperature=float(temperature)
                )
                teacher_action=_action_for_goal(
                    goal_index_value=label,observation=observation,state=state,parent_logits=parent_output["action_logits"][0]
                )
                for row in stats:
                    pset=conformal_prediction_set(mean,support_mask,q=float(row["q"]))
                    consensus=_consensus_action(
                        pset,observation=observation,state=state,parent_logits=parent_output["action_logits"][0]
                    )
                    if consensus is None:
                        continue
                    row["consensus_rows"]+=1
                    if int(consensus)!=int(r11_action):
                        row["override_rows"]+=1
                        row["correct_override_rows"]+=int(int(consensus)==int(teacher_action))
            _advance(state,task,observation,action_features,r11_action)
    candidates=[]
    for row in stats:
    ...
    selected=max(
        eligible,
        key=lambda row:(row["override_rows"],row["override_precision"],-row["alpha"]),
    ...
    *,
    ...
    while not task.done:
        ...
        if (
            ...
            mean,_=model.probabilities(features=features,support_mask=support_mask,temperature=float(temperature))
...
def evaluate_r23(
    ...
    *,
    ...
    if split not in {"dev","fresh"}:
        raise ValueError("R23 evaluation split must be dev/fresh")
    rows=[]; families_result={}; solved=steps=overrides=consensus_steps=0
    for family in families:
        fs=fst=fov=fcs=episodes=0
        for index in range(int(indices[0]),int(indices[1])+1):
            result=rollout_r23(
                parent,model,make_task(str(family),str(split),int(index)),
                temperature=float(temperature),q=q,max_support=int(max_support),
            )
            fs+=int(result["solved"]); fst+=int(result["steps"]); fov+=int(result["overrides_vs_r11"]); fcs+=int(result["consensus_steps"])
            solved+=int(result["solved"]); steps+=int(result["steps"]); overrides+=int(result["overrides_vs_r11"]); consensus_steps+=int(result["consensus_steps"])
            episodes+=1; rows.append({"family":str(family),"split":str(split),"index":int(index),**result})
        families_result[str(family)]={"episodes":episodes,"solved":fs,"steps":fst,"overrides_vs_r11":fov,"consensus_steps":fcs}
    return {
        "split":str(split),"indices":list(indices),"episodes":len(rows),"solved":solved,"steps":steps,
        "overrides_vs_r11":overrides,"consensus_steps":consensus_steps,"families":families_result,"rows":rows,
    }
...
def load_checkpoint(path: str | Path) -> tuple[NativeR23GoalBeliefEnsemble, dict[str, Any]]:
    payload=torch.load(Path(path),map_location="cpu",weights_only=True)
    if not isinstance(payload,dict) or payload.get("format")!=CHECKPOINT_FORMAT:
        raise ValueError("unsupported R23 checkpoint")
    architecture=payload["architecture"]
    model=NativeR23GoalBeliefEnsemble(
        ensemble_size=int(architecture["r23_ensemble_size"]),hidden_dim=int(architecture["r23_hidden_dim"])
    )
    model.load_state_dict(payload["state_dict"],strict=True)
    if model.state_sha256()!=payload["state_dict_sha256"]:
        raise ValueError("R23 state digest mismatch")
