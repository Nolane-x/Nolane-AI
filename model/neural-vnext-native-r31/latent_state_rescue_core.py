...
def encode_public_goal_features(
    *,
    ...
    if features.shape != (PUBLIC_GOAL_FEATURE_DIM,):
        raise AssertionError("unexpected R31 public goal feature shape")
    return features
...
def encode_parent_latent_context(parent_output: Mapping[str, Tensor]) -> Tensor:
    ...
    if latent.shape != (LATENT_CONTEXT_DIM,):
        raise AssertionError("unexpected R31 parent latent context shape")
...
def encode_latent_action_rescue_features(
    *,
    ...
    if features.shape != (LATENT_ACTION_RESCUE_FEATURE_DIM,):
        raise AssertionError("unexpected R31 latent action rescue feature shape")
    return features
...
class _LatentActionRescueHead(nn.Module):
    def __init__(self, hidden_dim: int) -> None:
        super().__init__()
        self.net = nn.Sequential(
...
class NativeR31LatentStateRescueEnsemble(nn.Module):
    """R30 all-action rescue policy augmented only by frozen accepted-R4 latent state."""
    ...
    def __init__(
        ...
        *,
        ...
        self.rescue_heads = nn.ModuleList(
            [
                _LatentActionRescueHead(self.rescue_hidden_dim)
    ...
    def architecture(self) -> dict[str, Any]:
        return {
            "r31_ensemble_size": self.ensemble_size,
            "r31_goal_hidden_dim": self.goal_hidden_dim,
            "r31_rescue_hidden_dim": self.rescue_hidden_dim,
            "r31_public_goal_feature_dim": PUBLIC_GOAL_FEATURE_DIM,
            "r31_base_action_rescue_feature_dim": BASE_ACTION_RESCUE_FEATURE_DIM,
            "r31_latent_context_dim": LATENT_CONTEXT_DIM,
            "r31_latent_action_rescue_feature_dim": LATENT_ACTION_RESCUE_FEATURE_DIM,
            "r31_rescue_class_count": RESCUE_CLASS_COUNT,
            "mechanism": "r30_all_action_rescue_plus_frozen_accepted_r4_latent_state",
        }
    ...
    def rescue_class_probabilities(self, features: Tensor) -> Tensor:
        if features.shape != (LATENT_ACTION_RESCUE_FEATURE_DIM,):
            raise ValueError("R31 rescue feature shape mismatch")
        logits = torch.stack(
