...
from latent_state_rescue_core import NativeR31LatentStateRescueEnsemble
from latent_state_rescue_training import (
...
def main() -> int:
    ...
    if lock.get("candidate") != "Neural-vNext-Native-R31-LatentStateRescue":
        raise ValueError("unexpected R31 candidate")
    if (
        lock["fresh_isolation"]["status"] != "UNOPENED"
        or lock["benchmark"]["fresh_indices"] != [280, 319]
    ):
        raise ValueError("R31 fresh isolation changed")
    ...
    print(
        json.dumps(
            {
                "status": "R31_R11_BASELINE_FRESH_UNOPENED",
    ...
    if model.goal_parameter_count() != int(architecture["expected_goal_successor_parameters"]):
        raise AssertionError("R31 goal parameter count changed")
    if model.rescue_parameter_count() != int(architecture["expected_rescue_successor_parameters"]):
        raise AssertionError("R31 rescue parameter count changed")
    if model.parameter_count() != int(architecture["expected_successor_parameters"]):
        raise AssertionError("R31 total parameter count changed")
    ...
    status = (
        "R31_DEV_ELIGIBLE_CONFIRMATION_UNOPENED_FRESH_UNOPENED"
        if eligibility["eligible"]
        else "R31_DEV_REJECTED_CONFIRMATION_FRESH_UNOPENED"
    )
    selected_config = {
        "temperature": temperature,
    ...
    manifest = {
        **metadata,
        ...
        "selected_candidate": "latent_state_action_conditioned_rescue",
