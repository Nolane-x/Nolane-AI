...
from cogcoder.arc_score_local import score_local
...
def measure(directory,max_attempts=2,max_programs=64):
    ...
    for path in paths:
        try: score=score_local(load_task(path),max_attempts=max_attempts,max_programs=max_programs)
        except Exception: errors+=1; continue
        solved+=int(score.solved); candidates+=score.candidate_programs; attempts+=score.attempts_emitted
